...
print('Gresho_2d: checking simulation output in directory ' +
...
while True:
    ...
    Pressure_ref[i2] = 9.0 + 12.5 * Radius[i2] * Radius[i2] - 20.0 * Radius[
        i2] + 4.0 * np.log(Radius[i2] / 0.2)
    ...
    if makeplots:
        ...
        pc = ax.pcolormesh(Edges1d,
                           Edges1d,
                           RotationVelocity[cells].reshape((Nplot, Nplot)),
                           rasterized=True)
        ...
        ax.set_title(
            r'$\mathrm{Gresho\_2d:}\ \mathrm{N}=%d^2,\ \mathrm{L1}=%4.1e$' %
            (np.int32(CellsPerDimension), L1_vel),
            loc='right',
            size=8)
