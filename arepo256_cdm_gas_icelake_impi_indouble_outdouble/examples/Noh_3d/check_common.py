...
DeltaMaxAllowed = check_parameters.DeltaMaxAllowed
...
while True:
    ...
    Density_ref = density_0 * (1.0 + time / Radius) * (1.0 + time / Radius
                                                       )  ## in 3d
    ...
    if makeplots:
        ...
        Edges1d = np.linspace(0.5 * Boxsize - 1.3 * r_shock,
        ...
        ax.set_xlim(0.5 * Boxsize - 1.3 * r_shock,
                    0.5 * Boxsize + 1.3 * r_shock)
        ax.set_ylim(0.5 * Boxsize - 1.3 * r_shock,
                    0.5 * Boxsize + 1.3 * r_shock)
        ...
        pc = ax.pcolormesh(Edges1d,
                           Edges1d,
                           vRad[cells].reshape((Nplot, Nplot)),
                           rasterized=True,
        ...
        pc = ax.pcolormesh(Edges1d,
                           Edges1d,
                           Uthermal[cells].reshape((Nplot, Nplot)),
                           rasterized=True,
    ...
    i_compare, = np.where(
        Radius < 0.8
    )  ## only check inner region; boundary has spurious effects in this testcase
    abs_delta_dens = np.abs(Density[i_compare] -
                            Density_ref[i_compare]) / Density_ref[i_compare]
    L1_dens = np.average(abs_delta_dens, weights=CellVolume[i_compare])

    L1_max = DeltaMaxAllowed * time
    print(__file__ + ' snapshot %d: L1_dens = %g, DeltaMaxAllowed = %g!' %
          (i_file, L1_dens, L1_max))

    if not L1_dens <= L1_max:
        sys.exit(1)
