...
print(
    'examples/isolated_galaxy_collisionless_3d/check.py: checking simulation output in directory '
...
while True:
    ...
    Radius1 = np.sqrt(xPosFromCenter1**2 + yPosFromCenter1**2 +
                      zPosFromCenter1**2)
    Radius2 = np.sqrt(xPosFromCenter2**2 + yPosFromCenter2**2 +
                      zPosFromCenter2**2)
    Radius3 = np.sqrt(xPosFromCenter3**2 + yPosFromCenter3**2 +
                      zPosFromCenter3**2)
    # disk height
    ...
    if createFigures:
        ...
        H1, xgrid, ygrid = np.histogram2d(xPosFromCenter2,
                                          yPosFromCenter2,
                                          bins=[binsx, binsy],
                                          density=True)
        H2, xgrid, ygrid = np.histogram2d(xPosFromCenter3,
                                          yPosFromCenter3,
                                          bins=[binsx, binsy],
                                          density=True)
        He1, xgrid, ygrid = np.histogram2d(zPosFromCenter2,
                                           yPosFromCenter2,
                                           bins=[binsz, binsy],
                                           density=True)
        He2, xgrid, ygrid = np.histogram2d(zPosFromCenter3,
                                           yPosFromCenter3,
                                           bins=[binsz, binsy],
                                           density=True)
        ...
        fig.savefig(os.path.join(directory, 'positions_%03d.pdf' % i_file))
