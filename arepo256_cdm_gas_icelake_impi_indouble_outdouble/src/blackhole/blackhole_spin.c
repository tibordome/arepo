...
void blackhole_new_accretion_episode(int i, MyFloat CurrentTime, MyFloat BH_Mass)
{
  ...
  // BPP(i).BH_DTimeAccretion_Current
  // = 5.3e5*pow(fabs(a),7/8.)*pow(Mass,11/8.)*pow(eps01/Xi,3/4.)*pow(v2v1,-7/8.)*pow(Alpha,-3/2.)*SEC_PER_YEAR/SEC_PER_GIGAYEAR; //in
  // yrs BPP(i).BH_DMass_Current = 6.0e5*pow(Alpha/0.1,-1/45.)*pow(Mass,34/45.)*pow(Xi/eps01,4/45.)*SOLAR_MASS/All.UnitMass_in_g;

  // Updating accretion time in prolonged mode and accreted mass in chaotic mode (Fanidakis, et al. 2010)
  BPP(i).BH_DTimeAccretion_Current = 3.0e6 * pow(fabs(a), 7 / 8.) * pow(Mass, 11 / 8.) * pow(LambEdd, -3 / 4.) * pow(v2v1, -7 / 8.) *
                                     pow(Alpha, -3 / 2.) * SEC_PER_YEAR / SEC_PER_GIGAYEAR;  // in yrs
  BPP(i).BH_DMass_Current = 2.13e5 * pow(eps, -5 / 27.) * pow(Mass, 23 / 27.) * pow(LambEdd, 5 / 27.) * pow(Alpha, -2 / 17.) *
...
int blackhole_do_prolonged_spin_evolution(int i, MyFloat CurrentTime)
{
  ...
  if(DTimeCurrentEpisode >= BPP(i).BH_DTimeAccretion_Current)
    {
      BH_Mass = BPP(i).BH_Mass_Previous +
                (BPP(i).BH_Mass - BPP(i).BH_Mass_Previous) * BPP(i).BH_DTimeAccretion_Current / DTimeCurrentEpisode;
        ...
        while(TimeBreak == 1)
          {
            fprintf(FdBlackHolesSpin, "BH=%llu %g %g %g %g %g %g %g %g %g %g %d\n", (long long)P[i].ID, All.Time,
                    BPP(i).BH_SpinParameter, BPP(i).BH_Mass, MdotNorm, BPP(i).BH_SpinOrientation[0], BPP(i).BH_SpinOrientation[1],
                    BPP(i).BH_SpinOrientation[2], BPP(i).BH_AngMomGasCells[0], BPP(i).BH_AngMomGasCells[1],
                    BPP(i).BH_AngMomGasCells[2], BPP(i).BH_SpinModel);
#if BH_SPIN_MODEL == 0
            BPP(i).BH_SpinModel = 0;
