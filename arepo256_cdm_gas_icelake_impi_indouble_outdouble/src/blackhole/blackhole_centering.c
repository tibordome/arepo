...
void blackhole_centering(void)
{
  ...
  BHPaux         = (struct bh_aux_data *)mymalloc("BHPaux", All.MaxPartBHs * sizeof(struct bh_aux_data));
          ...
          if(blackhole_isactive(i))
            {
              ...
              if(BPPaux(i).Ngb < 0.95 * All.BlackHoleCenteringMassMultiplier ||
                 BPPaux(i).Ngb > 1.05 * All.BlackHoleCenteringMassMultiplier)
                {
                  ...
                  if(BPPaux(i).Ngb > 0)
                    {
                      BPPaux(i).Dhsmlrho *=
                          BPP(i).HsmlCentering / (NUMDIMS * BPPaux(i).Ngb / (NORM_COEFF * pow(BPP(i).HsmlCentering, 3)));
                  ...
                  if(BPPaux(i).Ngb < 0.95 * All.BlackHoleCenteringMassMultiplier)
                    BPPaux(i).Left = fmax(BPP(i).HsmlCentering, BPPaux(i).Left);
                  else
                    {
                      if(BPPaux(i).Right != 0)
                        {
                      ...
                      if(BPPaux(i).Right == 0 && BPPaux(i).Left > 0)
                        {
                          ...
                          if(fabs(BPPaux(i).Ngb - All.BlackHoleCenteringMassMultiplier) < 0.5 * All.BlackHoleCenteringMassMultiplier)
                            {
                              fac = 1 - (BPPaux(i).Ngb - All.BlackHoleCenteringMassMultiplier) / (NUMDIMS * BPPaux(i).Ngb) *
                                            BPPaux(i).Dhsmlrho;
...
static void blackhole_centering_provide_treeexported_potential_values(void)
{
  struct treepoint_auxdata *export_Tree_Points =
      (struct treepoint_auxdata *)mymalloc("export_Tree_Points", Tree_NumPartExported * sizeof(struct treepoint_auxdata));
  ...
  for(int ngrp = 1; ngrp < (1 << PTask); ngrp++)
    {
      ...
      if(recvTask < NTask)
        if(Force_Send_count[recvTask] > 0 || Force_Recv_count[recvTask] > 0)
          MPI_Sendrecv(&export_Tree_Points[Force_Send_offset[recvTask]], Force_Send_count[recvTask] * sizeof(struct treepoint_auxdata),
                       MPI_BYTE, recvTask, TAG_DENS_A, &TreeAux_Points[Force_Recv_offset[recvTask]],
                       Force_Recv_count[recvTask] * sizeof(struct treepoint_auxdata), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                       MPI_STATUS_IGNORE);
    }
