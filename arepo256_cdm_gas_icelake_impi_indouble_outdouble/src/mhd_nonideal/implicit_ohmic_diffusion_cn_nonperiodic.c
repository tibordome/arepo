 ...
 *
 * \file        src/mhd_nonideal/implicit_ohmic_diffusion_cn_noperiodic.c
 * \date        10/2017
...
void set_coeff(HYPRE_IJMatrix *A, HYPRE_IJVector *x, HYPRE_IJVector *b, int *fld_offsets, double dt, int direction)
{
  ...
  memset(bval, 0, NumGas * sizeof(double));
      ...
      while(q >= 0)
        {
          ...
          sum += dval;
          bval[i] -= 0.5 * dval * (bfield - SphP[i].B[direction]);

          /* it's a boundary face */
          if(is_boundary > 0)
            {
              B0[i] -= 0.5 * dval * B_bcs[nghostloc];
              nghostloc++;
            }
          else
            {
