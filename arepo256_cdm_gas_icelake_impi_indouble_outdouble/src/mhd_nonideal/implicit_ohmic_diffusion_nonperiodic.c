 ...
 *
 * \file        src/mhd_nonideal/implicit_ohmic_diffusion_noperiodic.c
 * \date        10/2017
...
/*
 * ! \file implicit_ohmic_diffusion_noperiodic.c
...
#if defined(IMPLICIT_OHMIC_DIFFUSION) && defined(MHD_POWELL) && !defined(OHM_CRANK_NICHOLSON) && defined(REFLECTIVE_X) && \
    defined(REFLECTIVE_Y) && defined(REFLECTIVE_Z)
...
static MyFloat *Eta;
static MyFloat *B0;
static MyFloat *B_bcs;

static int NGhost;

static void count_ghost(void);
static void set_ghost(int direction);
...
void ohmic_diffusion(void)
{
  ...
  B0  = (MyFloat *)mymalloc("B0", NumGas * sizeof(MyFloat));
  ...
  count_ghost();
  B_bcs = (MyFloat *)mymalloc("ghostindex", NGhost * sizeof(MyFloat));
...
void update_B_and_Energy(void)
{
  ...
  for(i = 0; i < NumGas; i++)
    for(k = 0; k < 3; k++)
      SphP[i].BConserved[k] = SphP[i].B[k] * SphP[i].Volume;
#else
  for(i = 0; i < NumGas; i++)
    {
      double dEmag = -0.5 * (SphP[i].BConserved[0] * SphP[i].BConserved[0] + SphP[i].BConserved[1] * SphP[i].BConserved[1] +
                             SphP[i].BConserved[2] * SphP[i].BConserved[2]);

      for(k = 0; k < 3; k++)
        SphP[i].BConserved[k] = SphP[i].B[k] * SphP[i].Volume;

      dEmag += 0.5 * (SphP[i].BConserved[0] * SphP[i].BConserved[0] + SphP[i].BConserved[1] * SphP[i].BConserved[1] +
                      SphP[i].BConserved[2] * SphP[i].BConserved[2]);

      SphP[i].Energy += dEmag / SphP[i].Volume * pvd.atime;
...
void initialize_eta(int direction)
{
  ...
  for(i = 0; i < NumGas; i++)
    {
      B0[i]  = SphP[i].B[direction];
      Eta[i] = All.OhmicDiffusionCoefficient / SphP[i].Volume;

      if(All.ComovingIntegrationOn)
        Eta[i] /= All.cf_atime * All.cf_atime;
    }
}
...
void prepare_linear_iterations(double dt, int direction) { linear_iterations(dt, direction); }
      ...
      while(q >= 0)
        {
          ...
          /* check if the face divides 2 boundary points */
          /* it is now not important on what side they are */
          for(int k = 0; k < 3; k++)
            {
              p0[k] = WRAP(k, p0[k]);
              p1[k] = WRAP(k, p1[k]);

              if((p1[k] - p0[k]) > 0.5 * All.BoxSizes[k] || (p1[k] - p0[k]) < -0.5 * All.BoxSizes[k])
                {
                  is_boundary = 1;
                  // mpi_printf("NON-IDEAL: dp %d, dp_other %d, dx %e, dy %e, dz %e, p0=%e|%e|%e, p1=%e|%e|%e\n", dp, dp_other, dx, dy,
                  // dz, p0tmp[0], p0tmp[1], p0tmp[2], p1tmp[0], p1tmp[1], p1tmp[2]);
                }
            }

          dx = NEAREST_X(p1[0] - p0[0]);
          dy = NEAREST_Y(p1[1] - p0[1]);
          dz = NEAREST_Z(p1[2] - p0[2]);
          ...
          // mpi_printf("NON-IDEAL: i %d, Eta %e, eta_q %e eta_ij %e, dr %e, dt %e, area %e\n", i, Eta[i], eta_q, eta_ij, dr, dt,
          // VF[vf].area); mpi_printf("NON-IDEAL: i %d, dval %e\n", i, dval);

          /* it's a boundary face */
          if(is_boundary > 0)
            {
              B0[i] -= dval * B_bcs[nghostloc];
              nghostloc++;
            }
          else
            {
          ...
          if((task = ListExports[listp].origin) != ThisTask)
            {
              ...
              tmpInExch[off].B   = SphP[place].B[direction];
...
{
//#if defined(ONLY_OHMIC_DIFFUSION) || defined(BAR_EQS)
#if defined(ONLY_OHMIC_DIFFUSION)
  return;
#else
  struct pv_update_data pvd;
...
void set_ghost(int direction)
{
      ...
      while(q >= 0)
        {
          ...
          /* dp_other is the index of the SphP[i] particle in the Delaunay triangulation */
          int dp_other = VF[vf].p1;
          if(dp_other == dp)
            dp_other = VF[vf].p2;
          ...
          for(int k = 0; k < 3; k++)
            {
              ...
              if((p1[k] - p0[k] > 0.5 * All.BoxSizes[k]) || (p1[k] - p0[k] < -0.5 * All.BoxSizes[k]))
                {
                  B_bcs[nghostloc] = SphP[i].B[direction];
                  nghostloc++;
                  break;
                }
            }
  ...
  if(nghostloc != NGhost)
    terminate("ThisTask = %d, NGhost = %d, nghostloc = %d\n", ThisTask, NGhost, nghostloc);
}

void count_ghost(void)
{
  point *DP = Mesh.DP;
  face *VF  = Mesh.VF;
