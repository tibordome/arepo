  ...
  if(no < Ngb_MaxPart)
    {
      ...
      if(fabs(SphP[no].Weights[1][W_RHO]) > All.DG_SlopeRangeFactor * All.DG_TargetSlope  // x slope
         || fabs(SphP[no].Weights[2][W_RHO]) >
                All.DG_SlopeRangeFactor *
                    All.DG_TargetSlope                                                                             // y slope
...
#if defined(DEREFINE_X_LARGER) || defined(DEREFINE_X_SMALLER) || defined(DEREFINE_Y_LARGER) || defined(DEREFINE_Y_SMALLER) || \
    defined(DEREFINE_Z_LARGER) || defined(DEREFINE_Z_SMALLER)
      return 1;
#endif

      if(fabs(Ngb_Nodes[no].hydro.Weights[1][W_RHO]) > All.DG_TargetSlope / All.DG_SlopeRangeFactor  // x slope
         || fabs(Ngb_Nodes[no].hydro.Weights[2][W_RHO]) >
                All.DG_TargetSlope / All.DG_SlopeRangeFactor  // y slope
                                         DG3D(|| fabs(Ngb_Nodes[no].hydro.Weights[3][W_RHO]) >
                                                     All.DG_TargetSlope / All.DG_SlopeRangeFactor))  // z slope
        {
          return 1;
        }
#else
      if(Ngb_Nodes[no].hydro.mass >
         0.5 * All.TargetGasMass)  // if the node is heavy enough: keep it refined, otherwise: merge subnodes.
        return 1;
#endif
#else
      return 1;
