  ...
  for(dir = 0; dir < NUMDIMS; dir++)
    {
      ...
      if(ref_boundary == 0)
        {
          if(ngb >= Ngb_FirstNonTopLevelNode ||
             (ngb >= Ngb_MaxPart && ngb < Ngb_FirstNonTopLevelNode &&
              (Ngb_DomainTask[ngb] == ThisTask || Ngb_DomainTask[ngb] == -1)))  // a local neighbor node, follow to is daughter
            {
      ...
      if(ref_boundary == 0)
        {
          if(ngb >= Ngb_FirstNonTopLevelNode ||
             (ngb >= Ngb_MaxPart &&
              (Ngb_DomainTask[ngb] == ThisTask ||
               Ngb_DomainTask[ngb] == -1)))  // a local neighbor node, follow to is daughter, or a leaf top node as neighbor
            {
              ngb = Ngb_Nodes[ngb].u.suns[subnode ^ (1 << dir)];
            }
        }
          ...
          if(ngb < Ngb_MaxPart && Mesh.DP[ngb].level == Mesh.DP[particle - Mesh.nodes_total].level)
            {
              Mesh.DP[ngb].neighbors[2 * dir + (r ^ 1)] = particle;
                  ...
                  (ngb >= Ngb_FirstNonTopLevelNode || (Ngb_DomainTask[ngb] == ThisTask || Ngb_DomainTask[ngb] == -1)))  // a local node
            {
