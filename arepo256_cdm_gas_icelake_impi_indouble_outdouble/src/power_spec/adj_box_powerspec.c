...
typedef long long ps_large_array_offset;
...
void adj_box_powerspec(void)
{
  ...
  fft_forward_plan = rfftw3d_mpi_create_plan(MPI_COMM_WORLD, FOURIER_GRID, FOURIER_GRID, FOURIER_GRID, FFTW_REAL_TO_COMPLEX,
  ...
  file_path_sprintf(fname, "%s/vel_powerspec_%03d.txt", All.OutputDir, RestartSnapNum);
  ...
  file_path_sprintf(fname, "%s/curl_powerspec_%03d.txt", All.OutputDir, RestartSnapNum);
  adj_box_powerspec_save(fname);
  ...
  file_path_sprintf(fname, "%s/vorticity_powerspec_%03d.txt", All.OutputDir, RestartSnapNum);
  adj_box_powerspec_save(fname);
...
double adj_box_powerspec_obtain_fields(void)
{
  ...
  ps_large_array_offset i, n,
      Ncount = ((ps_large_array_offset)nslab_x) * (FOURIER_GRID * FOURIER_GRID); /* number of grid points on the local slab */
  ...
  All.BunchSize  = (int)((All.BufferSize * 1024 * 1024) / (sizeof(data_index) + sizeof(struct data_nodelist) + sizeof(data_in) +
                                                          sizeof(data_out) + smax(sizeof(data_in), sizeof(data_out))));
              ...
              if(recvTask < NTask)
                {
                  if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
                    {
                      /* get the particles */
                      MPI_Sendrecv(&DataIn[Send_offset[recvTask]], Send_count[recvTask] * sizeof(data_in), MPI_BYTE, recvTask,
                                   TAG_DENS_A, &DataGet[Recv_offset[recvTask]], Recv_count[recvTask] * sizeof(data_in), MPI_BYTE,
              ...
              if(recvTask < NTask)
                {
                  if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
                    {
                      /* send the results */
                      MPI_Sendrecv(&DataResult[Recv_offset[recvTask]], Recv_count[recvTask] * sizeof(data_out), MPI_BYTE, recvTask,
                                   TAG_DENS_B, &DataOut[Send_offset[recvTask]], Send_count[recvTask] * sizeof(data_out), MPI_BYTE,
...
int adj_box_powerspec_treefind(MyDouble searchcenter[3], MyFloat hsml, int target, int *startnode, int mode, int *nexport,
                               int *nsend_local)
{
  ...
  while(no >= 0)
    {
      if(no < All.MaxPart) /* single particle */
      ...
      else
        {
                      ...
                      if(*nexport >= All.BunchSize)
                        {
                          *nexport = nexport_save;
                          if(nexport_save == 0)
                            terminate("buffer too small to process even a single particle"); /* in this case, the buffer is too small
                                                                                                to process even a single particle */
          ...
          if(mode == 1)
            {
              if(current->u.d.bitflags &
                 (1 << BITFLAG_TOPLEVEL)) /* we reached a top-level node again, which means that we are done with the branch */
                {
                  *startnode = -1;
                  return numngb;
                }
