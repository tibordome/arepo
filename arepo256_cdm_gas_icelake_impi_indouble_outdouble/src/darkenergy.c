...
void fwa_init(void)
{
  ...
  mpi_printf("initialize time dependent w ...\n");
  ...
  if((fd = fopen(All.DarkEnergyFile, "r")))
    {
...
#endif
                {
                  terminate("Wrong syntax in file `%s', line %d", All.DarkEnergyFile, count);
                }
              a = 1. / atof(buf1);
              if(a == 0.0 && count == 1)
      ...
      if(count >= ANZ_W_A_IN - 1)
        terminate("File `%s' contains too many datapoints, increase ANZ_W_A_IN!", All.DarkEnergyFile);
      if(count <= 2)
        terminate("File `%s' has too few Data Points (%d)!", All.DarkEnergyFile, count);
      ...
      /* Setp back if tables go beyond a=1 */
      ...
      if(atab[count - 1] < 1.)
        {
          atab[count] = 1.0;
          wtab[count] =
              wtab[count - 1] + (wtab[count - 1] - wtab[count - 2]) / (atab[count - 1] - atab[count - 2]) * (1. - atab[count - 1]);
#ifdef TIMEDEPGRAV
          dHtab[count] =
              dHtab[count - 1] + (dHtab[count - 1] - dHtab[count - 2]) / (atab[count - 1] - atab[count - 2]) * (1. - atab[count - 1]);
          dGtab[count] =
              dGtab[count - 1] + (dGtab[count - 1] - dGtab[count - 2]) / (atab[count - 1] - atab[count - 2]) * (1. - atab[count - 1]);
#endif
#ifdef EXTERNALHUBBLE
          Htab[count] =
              Htab[count - 1] + (Htab[count - 1] - Htab[count - 2]) / (atab[count - 1] - atab[count - 2]) * (1. - atab[count - 1]);
#endif
          /*            if(ThisTask ==0)
                        {
      ...
      for(i = ANZ_W_A - 1; i >= 1; i--)
        {
          ...
          if(count > 1) /* Still inside the table */
            {
              while(atab[count - 1] > a && count > 0)
                {
                  sum += 0.5 * ((1. + w_first) / a_first + (1. + wtab[count - 1]) / atab[count - 1]) * (a_first - atab[count - 1]);
                  count--;
                  a_first = atab[count];
                  w_first = wtab[count];
                }
          ...
          intwatab[i] = wtab[count - 1] + (wtab[count] - wtab[count - 1]) / (atab[count] - atab[count - 1]) * (a - atab[count - 1]);
#ifdef TIMEDEPGRAV
    ...
    {
      terminate("File `%s' not found!", All.DarkEnergyFile);
    }

  mpi_printf("Integrating w(a) finished.\n");
}
...
double DarkEnergy_t(double Time) /* only needed for physical integration */ { return All.DarkEnergyParam; }
