...
void circumstellar_swallow_gas(void)
{
  ...
  All.BunchSize =
      (int)((All.BufferSize * 1024 * 1024) /
            (sizeof(data_index) + sizeof(struct data_nodelist) + sizeof(struct circumstellardata_in) +
             sizeof(struct circumstellardata_out) + smax(sizeof(struct circumstellardata_in), sizeof(struct circumstellardata_out))));
  ...
  do
    {
      ...
      CircumstellarDataGet =
          (struct circumstellardata_in *)mymalloc("CircumstellarDataGet", nimport * sizeof(struct circumstellardata_in));
      CircumstellarDataIn =
          (struct circumstellardata_in *)mymalloc("CircumstellarDataIn", nexport * sizeof(struct circumstellardata_in));
      ...
      for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
        {
          ...
          if(recvTask < NTask)
            if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
              MPI_Sendrecv(&CircumstellarDataIn[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct circumstellardata_in),
                           MPI_BYTE, recvTask, TAG_DENS_A, &CircumstellarDataGet[Recv_offset[recvTask]],
                           Recv_count[recvTask] * sizeof(struct circumstellardata_in), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
        }

      myfree(CircumstellarDataIn);
      CircumstellarDataResult =
          (struct circumstellardata_out *)mymalloc("CircumstellarDataResult", nimport * sizeof(struct circumstellardata_out));
      CircumstellarDataOut =
          (struct circumstellardata_out *)mymalloc("CircumstellarDataOut", nexport * sizeof(struct circumstellardata_out));
          ...
          if(recvTask < NTask)
            if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
              MPI_Sendrecv(&CircumstellarDataResult[Recv_offset[recvTask]],
                           Recv_count[recvTask] * sizeof(struct circumstellardata_out), MPI_BYTE, recvTask, TAG_DENS_B,
                           &CircumstellarDataOut[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct circumstellardata_out),
      ...
      for(j = 0; j < nexport; j++)
        {
          ...
          for(k = 0; k < 3; k++)
            P[place].Vel[k] = (P[place].Vel[k] * P[place].Mass + CircumstellarDataOut[j].AccretedMomentum[k]) /
              ...
              if(P[j].Type == 0)
                {
                  ...
                  double sep = sqrt((pos[0] - P[j].Pos[0]) * (pos[0] - P[j].Pos[0]) + (pos[1] - P[j].Pos[1]) * (pos[1] - P[j].Pos[1]) +
                                    (pos[2] - P[j].Pos[2]) * (pos[2] - P[j].Pos[2]));
                  double E_bind =
                      0.5 * ((vel[0] - P[j].Vel[0]) * (vel[0] - P[j].Vel[0]) + (vel[1] - P[j].Vel[1]) * (vel[1] - P[j].Vel[1]) +
                             (vel[2] - P[j].Vel[2]) * (vel[2] - P[j].Vel[2])) +
                      -All.G * (mass + P[j].Mass) / sep / sep / sep;
...
#else /* #ifndef CIRCUMSTELLAR_SINKS_ALTERNATIVE */
...
#endif /* #ifndef CIRCUMSTELLAR_SINKS_ALTERNATIVE #else */
