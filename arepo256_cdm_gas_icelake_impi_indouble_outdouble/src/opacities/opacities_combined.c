...
void initialize_fa_opacity(opacitydata *data, char *dir)
{
  ...
  status = gsl_interp2d_init(data->inter, (double *)data->logR, (double *)data->logT, (double *)data->logkappa, data->nr, data->nt);
}
...
double get_fa_opacity(double rho, double t, opacitydata *data)
{
  ...
  if(logr < data->logR[0] || logr > data->logR[data->nr - 1] || logt < data->logT[0] || logt > data->logT[data->nt - 1])
    result = 0.0;
  else
    {
      int err;
