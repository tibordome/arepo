...
int modgrav_force_construct_full_tree(void)
{
                      ...
                      if(All.TreeAllocFactor > MAX_TREE_ALLOC_FACTOR)
                        {
                          ...
                          sprintf(buf,
                                  ...
                                  "Tree_NumNodes=%d Tree_MaxNodes=%d  Tree_NumPartImported=%d NumPart=%d\n",
  ...
  if(father < 0)
    {
      ...
      if(Nodes[no].mg_bitflag.num_daughters != 8)
        terminate("root node has less than 8 subnodes");
          ...
          (Nodes[no].mg_bitflag.num_daughters < 8 || Nodes[no].mg_bitflag.level == All.MaxAMRLevel))
    {
