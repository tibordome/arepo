 ...
 * \date        MM/YYYY
 * \author
 * \brief       this file processes the local groups in serial mode
 * \details
 *
 *
...
{
  int j, k, count_cand, len_non_gas, N, nsubs, part_index, subnr, totlen;
      ...
      if(Offs >= NumPart)
        {
          ...
          terminate("don't find a particle for groupnr=%d\n", Group[gr].GrNr);
        }
    }
  ...
  for(k = 0, nsubs = 0; k < count_cand; k++)
    {
      p       = candidates[k].head;
      int len = 0;
      for(int i = 0; i < candidates[k].len; i++, p = Next[p])
        if(Tail[p] < 0)
          ud[len++].index = p;
  ...
  if(nsubs != GroupCatLocal[gr].Nsubs)
    terminate("nsubs=%d != GroupCatLocal[gr=%d].Nsubs=%d, GrNr=%d", nsubs, gr, GroupCatLocal[gr].Nsubs, GrNr);
#endif
  ...
  for(k = 0, subnr = 0, totlen = 0; k < nsubs; k++)
    {
      int len = candidates[k].bound_length;
      ...
      p         = candidates[k].head;
      int count = 0;
      for(int i = 0; i < candidates[k].len; i++)
        {
          if(Tail[p] == candidates[k].nsub)
            ud[count++].index = p;
      ...
      subfind_determine_sub_halo_properties(ud, len, &SubGroup[Nsubgroups], GrNr, subnr, 0, nsubgroups_cat, gr);
      ...
      if(Offs >= NumPart)
        {
          ...
          terminate("don't find a particle for groupnr=%d", Group[gr].GrNr);
        }
    }
