 ...
 *
 * \file        src/voronoi_ghost_search.c
 * \date        MM/YYYY
 * \author
 * \brief
 * \details
...
#define OMIT_GENERIC_COMM_PATTERN
...
#pragma omp parallel private(i, j, q)
  {
        ...
        if((T->DTF[i] & 2) == 0) /* DT that is not flagged as tested ok */
          {
            ...
            if(j == (NUMDIMS + 1))
              /* this triangle does not have a local point. No need to test it */
              continue;
            ...
            if(j == (NUMDIMS + 1))
              /* this triangle does not have a point which is not at least neighbor to a primary point. No need to test it */
              continue;
 ...
 *
 *  \param[] TT Pointer to tessellation.
 ...
 *  \return 0
 */
int voronoi_ghost_search_evaluate(tessellation *TT, int target, int mode, int q, int thread_id)
{
  ...
  if(mode == MODE_LOCAL_PARTICLES)
    {
      pos[0]    = TT->DTC[target].cx;
      pos[1]    = TT->DTC[target].cy;
      pos[2]    = TT->DTC[target].cz;
      refpos[0] = TT->DP[q].x;
      refpos[1] = TT->DP[q].y;
      refpos[2] = TT->DP[q].z;
#ifndef DOUBLE_STENCIL
      maxdist = SphP[TT->DP[q].index].Hsml;
#else
      maxdist = TT->DP[q].Hsml;
#endif
      origin = ThisTask;
  ...
  numngb = ngb_treefind_ghost_search(TT, pos, refpos, h, maxdist, target, origin, mode, thread_id, numnodes, firstnode);
...
#ifdef EXTENDED_GHOST_SEARCH
/* this allowes for mirrored images in a full 3x3 grid in terms of the principal domain */

int ngb_treefind_ghost_search(tessellation *TT, MyDouble searchcenter[3], MyDouble refpos[3], MyFloat hsml, MyFloat maxdist,
                              int target, int origin, int *startnode, int bitflags, int mode, int *nexport, int *nsend_local)
{
                  ...
                  if(ListExports[listp].nextexport < 0)
                    {
                      if(Ninlist >= MaxNinlist)
                        {
                          TT->Indi.AllocFacNinlist *= ALLOC_INCREASE_FACTOR;
                          MaxNinlist = TT->Indi.AllocFacNinlist;
#ifdef VERBOSE
                          printf("Task=%d: increase memory allocation, MaxNinlist=%d Indi.AllocFacNinlist=%g\n", ThisTask, MaxNinlist,
                                 TT->Indi.AllocFacNinlist);
#endif
                          ListExports =
      ...
      if(origin == ThisTask)
        {
          ...
          if(TT->Ndp >= TT->MaxNdp)
            {
              TT->Indi.AllocFacNdp *= ALLOC_INCREASE_FACTOR;
              TT->MaxNdp = TT->Indi.AllocFacNdp;
#ifdef VERBOSE
              printf("Task=%d: increase memory allocation, MaxNdp=%d Indi.AllocFacNdp=%g\n", ThisTask, TT->MaxNdp,
                     TT->Indi.AllocFacNdp);
#endif
              TT->DP -= 5;
              TT->DP = (point *)myrealloc_movable(TT->DP, (TT->MaxNdp + 5) * sizeof(point));
              TT->DP += 5;
      ...
      else
        {
          ...
          if(N_DP_Buffer >= MaxN_DP_Buffer)
            {
              TT->Indi.AllocFacN_DP_Buffer *= ALLOC_INCREASE_FACTOR;
              MaxN_DP_Buffer = TT->Indi.AllocFacN_DP_Buffer;
#ifdef VERBOSE
              printf("Task=%d: increase memory allocation, MaxN_DP_Buffer=%d Indi.AllocFacN_DP_Buffer=%g\n", ThisTask, MaxN_DP_Buffer,
                     TT->Indi.AllocFacN_DP_Buffer);
#endif
 ...
 *
 *  \param[in] TT Pointer to tessellation.
 ...
 */
int ngb_treefind_ghost_search(tessellation *TT, MyDouble searchcenter[3], MyDouble refpos[3], MyFloat hsml, MyFloat maxdist,
                              int target, int origin, int mode, int thread_id, int numnodes, int *firstnode)
{
  int i, k, numngb, no, p;
  int image_flag = 0;
  struct NgbNODE *current;
          ...
          if(no < Ngb_MaxPart) /* single particle */
            {
              ...
              /* for each coordinates there are three possibilities. We
      ...
      if(origin == ThisTask)
        {
          ...
          dp->ID = P[p].ID;
          if(image_flag)
            /* this is a replicated/mirrored local point */
            dp->index = p + NumGas;
          else
            /* this is actually a local point that wasn't made part of the mesh yet */
            dp->index = p;
          dp->originalindex = p;
          dp->timebin = P[p].TimeBinHydro;
#if defined(REFLECTIVE_X) || defined(REFLECTIVE_Y) || defined(REFLECTIVE_Z) || defined(VORONOI_DYNAMIC_UPDATE)
 ...
 */
int count_undecided_tetras(tessellation *TT)
{
  int i, count;

  for(i = 0, count = 0; i < TT->Ndt; i++)
    if((TT->DTF[i] & 2) == 0)
      count++;
