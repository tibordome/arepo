      ...
      for(j = i + 1; j < T->Ndp; j++)
        {
          ...
          if(r2min == 0)
            terminate("i=%d j=%d equal.  DP[i].index=%d DP[j].index=%d\n", i, j, DP[i].index, DP[j].index);
        }
    }
  ...
  for(i = 0; i < T->Ndt; i++)
    {
      ...
      if(ivol <= 0)
        terminate("Tetra %d is NEGATIVE (%d %d %d %d) oriented or FLAT: ivol=%d vol=%g\n", i, (int)(t->p[0]), (int)(t->p[1]),
  ...
  for(i = 0; i < T->Ndt; i++)
    {
      ...
      if(test_tetra_orientation(p0, p1, p2, p3) > 0)
        ...
        terminate("Tetra %d is NEGATIVE oriented\n", i);
                    ...
                    if(res >= 0)
                      {
                        ...
                        if(res_exact > 0)
                          terminate("ERROR tetra=%d: point=%d  in tetra with edges=%d|%d|%d|%d   res=%d|%d\n", i, j, (int)(t->p[0]),
                                    (int)(t->p[1]), (int)(t->p[2]), (int)(t->p[3]), res, res_exact);
                      }
                  }
        }
    }
