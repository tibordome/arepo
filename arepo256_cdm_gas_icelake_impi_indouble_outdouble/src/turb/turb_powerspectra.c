...
#if defined(POWERSPEC_GRID) && (defined(VS_TURB) || defined(AB_TURB))
...
static fft_real *ksgsfield;
#ifdef SGS_TURBULENCE_MAKE_PRODUCTION_DISSIPATION_SPECTRUM
static fft_real *sgstadiab1field;
static fft_real *sgstadiab2field;
static fft_real *sgstturbprod1field;
static fft_real *sgstturbprod2field;
static fft_real *sgstviscdiss1field;
static fft_real *sgstviscdiss2field;
#endif
#endif
...
#ifdef SGS_TURBULENCE
static double PowerDc;
static double PowerDc_global;
...
#ifdef SGS_TURBULENCE
  MyDouble SgsTSpecificEnergy;
#ifdef SGS_TURBULENCE_MAKE_PRODUCTION_DISSIPATION_SPECTRUM
  MyDouble SGS_T_DuDt_Adiab;
  MyDouble SGS_T_DuDt_TurbProd;
  MyDouble SGS_T_DuDt_ViscDiss;
#endif
#endif
#ifdef TRACER_MC
  int NtracerMC;
...
#ifdef SGS_TURBULENCE
      ksgsfield[ip] = sqrt(out->SgsTSpecificEnergy);

#ifdef SGS_TURBULENCE_MAKE_PRODUCTION_DISSIPATION_SPECTRUM
      if(out->SGS_T_DuDt_Adiab >= 0)
        {
          sgstadiab1field[ip] = sqrt(out->SGS_T_DuDt_Adiab);
          sgstadiab2field[ip] = 0;
        }
      else
        {
          sgstadiab1field[ip] = 0;
          sgstadiab2field[ip] = sqrt(-out->SGS_T_DuDt_Adiab);
        }

      if(out->SGS_T_DuDt_TurbProd >= 0)
        {
          sgstturbprod1field[ip] = sqrt(out->SGS_T_DuDt_TurbProd);
          sgstturbprod2field[ip] = 0;
        }
      else
        {
          sgstturbprod1field[ip] = 0;
          sgstturbprod2field[ip] = sqrt(-out->SGS_T_DuDt_TurbProd);
        }

      if(out->SGS_T_DuDt_ViscDiss >= 0)
        {
          sgstviscdiss1field[ip] = sqrt(out->SGS_T_DuDt_ViscDiss);
          sgstviscdiss2field[ip] = 0;
        }
      else
        {
          sgstviscdiss1field[ip] = 0;
          sgstviscdiss2field[ip] = sqrt(-out->SGS_T_DuDt_ViscDiss);
        }
#endif
#endif
...
static turb_large_array_offset Ncount;
...
void powerspec_turb(int filenr, int type)
{
...
#ifdef SGS_TURBULENCE
  ksgsfield = (fft_real *)mymalloc("ksgsfield", maxfftsize * sizeof(fft_real));

#ifdef SGS_TURBULENCE_MAKE_PRODUCTION_DISSIPATION_SPECTRUM
  sgstadiab1field    = (fft_real *)mymalloc("sgstadiab1field", maxfftsize * sizeof(fft_real));
  sgstadiab2field    = (fft_real *)mymalloc("sgstadiab2field", maxfftsize * sizeof(fft_real));
  sgstturbprod1field = (fft_real *)mymalloc("sgstturbprod1field", maxfftsize * sizeof(fft_real));
  sgstturbprod2field = (fft_real *)mymalloc("sgstturbprod2field", maxfftsize * sizeof(fft_real));
  sgstviscdiss1field = (fft_real *)mymalloc("sgstviscdiss1field", maxfftsize * sizeof(fft_real));
  sgstviscdiss2field = (fft_real *)mymalloc("sgstviscdiss2field", maxfftsize * sizeof(fft_real));
#endif
#endif
...
#ifdef SGS_TURBULENCE
  memset(ksgsfield, 0, maxfftsize * sizeof(fft_real));
#ifdef SGS_TURBULENCE_MAKE_PRODUCTION_DISSIPATION_SPECTRUM
  memset(sgstadiab1field, 0, maxfftsize * sizeof(fft_real));
  memset(sgstadiab2field, 0, maxfftsize * sizeof(fft_real));
  memset(sgstturbprod1field, 0, maxfftsize * sizeof(fft_real));
  memset(sgstturbprod2field, 0, maxfftsize * sizeof(fft_real));
  memset(sgstviscdiss1field, 0, maxfftsize * sizeof(fft_real));
  memset(sgstviscdiss2field, 0, maxfftsize * sizeof(fft_real));
#endif
#endif
  ...
  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_B_%03d.txt", All.OutputDir, filenr);
  if(fname[0] != 0)
  ...
  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_vel_%03d.txt", All.OutputDir, filenr);
#ifdef TRACER_PARTICLE
  if(type == TRACER_PARTICLE)
    file_path_sprintf(fname, "%s/powerspec_TRPART_vel_%03d.txt", All.OutputDir, filenr);
#endif
  if(fname[0] != 0)
  ...
  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_velrho_%03d.txt", All.OutputDir, filenr);
#ifdef TRACER_PARTICLE
  if(type == TRACER_PARTICLE)
    file_path_sprintf(fname, "%s/powerspec_TRPART_velrho_%03d.txt", All.OutputDir, filenr);
#endif
#ifdef TRACER_MC
  if(type == TRACER_MC)
    file_path_sprintf(fname, "%s/powerspec_TRMC_velrho_%03d.txt", All.OutputDir, filenr);
#endif
  if(fname[0] != 0)
  ...
  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_vorticity_%03d.txt", All.OutputDir, filenr);
#ifdef TRACER_PARTICLE
  if(type == TRACER_PARTICLE)
    file_path_sprintf(fname, "%s/powerspec_TRPART_vorticity_%03d.txt", All.OutputDir, filenr);
#endif
  if(fname[0] != 0)
    powerspec_turb_save(fname, velrho_disp);
  ...
  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_dis1_%03d.txt", All.OutputDir, filenr);
  if(fname[0] != 0)
  ...
  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_dis2_%03d.txt", All.OutputDir, filenr);
  if(fname[0] != 0)
    powerspec_turb_save(fname, empty_disp);
  ...
  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_random_%03d.txt", All.OutputDir, filenr);
  if(fname[0] != 0)
    powerspec_turb_save(fname, empty_disp);
  ...
  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_density_%03d.txt", All.OutputDir, filenr);
#ifdef TRACER_PARTICLE
  if(type == TRACER_PARTICLE)
    file_path_sprintf(fname, "%s/powerspec_TRPART_density_%03d.txt", All.OutputDir, filenr);
#endif
#ifdef TRACER_MC
  if(type == TRACER_MC)
    file_path_sprintf(fname, "%s/powerspec_TRMC_density_%03d.txt", All.OutputDir, filenr);
#endif
  if(fname[0] != 0)
    powerspec_turb_save(fname, empty_disp);

#ifdef SGS_TURBULENCE
  /* Now compute the power spectrum of the subgrid-scale turbulence specific energy field */
  fname[0] = 0;
  PowerDc  = 0;
  for(i = 0; i < BINS_PS; i++)
    {
  ...
  powerspec_turb_calc_and_bin_spectrum(ksgsfield, 1);

  powerspec_turb_collect();

  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_ksgs_%03d.txt", All.OutputDir, filenr);

  if(fname[0] != 0)
    powerspec_turb_save(fname, empty_disp);

#ifdef SGS_TURBULENCE_MAKE_PRODUCTION_DISSIPATION_SPECTRUM
  /* Now compute the power spectrum of the subgrid-scale turbulence specific energy adiabatic production field 1 */
  fname[0] = 0;
  PowerDc  = 0;
  for(i = 0; i < BINS_PS; i++)
    {
  ...
  powerspec_turb_calc_and_bin_spectrum(sgstadiab1field, 1);

  powerspec_turb_collect();

  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_sgstadiab1_%03d.txt", All.OutputDir, filenr);

  if(fname[0] != 0)
    powerspec_turb_save(fname, empty_disp);

  /* Now compute the power spectrum of the subgrid-scale turbulence specific energy adiabatic production field 2 */
  fname[0] = 0;
  PowerDc  = 0;
  for(i = 0; i < BINS_PS; i++)
    {
  ...
  powerspec_turb_calc_and_bin_spectrum(sgstadiab2field, 1);

  powerspec_turb_collect();

  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_sgstadiab2_%03d.txt", All.OutputDir, filenr);

  if(fname[0] != 0)
    powerspec_turb_save(fname, empty_disp);

  /* Now compute the power spectrum of the subgrid-scale turbulence specific energy turbulent production field 1 */
  fname[0] = 0;
  PowerDc  = 0;
  for(i = 0; i < BINS_PS; i++)
    {
  ...
  powerspec_turb_calc_and_bin_spectrum(sgstturbprod1field, 1);

  powerspec_turb_collect();

  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_sgstturbprod1_%03d.txt", All.OutputDir, filenr);

  if(fname[0] != 0)
    powerspec_turb_save(fname, empty_disp);

  /* Now compute the power spectrum of the subgrid-scale turbulence specific energy turbulent production field 2 */
  fname[0] = 0;
  PowerDc  = 0;
  for(i = 0; i < BINS_PS; i++)
    {
  ...
  powerspec_turb_calc_and_bin_spectrum(sgstturbprod2field, 1);

  powerspec_turb_collect();

  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_sgstturbprod2_%03d.txt", All.OutputDir, filenr);

  if(fname[0] != 0)
    powerspec_turb_save(fname, empty_disp);

  /* Now compute the power spectrum of the subgrid-scale turbulence specific energy viscous dissipation field 1 */
  fname[0] = 0;
  PowerDc  = 0;
  for(i = 0; i < BINS_PS; i++)
    {
  ...
  powerspec_turb_calc_and_bin_spectrum(sgstviscdiss1field, 1);

  powerspec_turb_collect();

  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_sgstviscdiss1_%03d.txt", All.OutputDir, filenr);

  if(fname[0] != 0)
    powerspec_turb_save(fname, empty_disp);

  /* Now compute the power spectrum of the subgrid-scale turbulence specific energy viscous dissipation field 2 */
  fname[0] = 0;
  PowerDc  = 0;
  for(i = 0; i < BINS_PS; i++)
    {
  ...
  powerspec_turb_calc_and_bin_spectrum(sgstviscdiss2field, 1);

  powerspec_turb_collect();

  if(type == 0)
    file_path_sprintf(fname, "%s/powerspec_sgstviscdiss2_%03d.txt", All.OutputDir, filenr);
...
#ifdef SGS_TURBULENCE
#ifdef SGS_TURBULENCE_MAKE_PRODUCTION_DISSIPATION_SPECTRUM
  myfree(sgstviscdiss2field);
  myfree(sgstviscdiss1field);
  myfree(sgstturbprod2field);
  myfree(sgstturbprod1field);
  myfree(sgstadiab2field);
  myfree(sgstadiab1field);
#endif
  myfree(ksgsfield);
#endif /* #ifdef SGS_TURBULENCE */
  myfree(densityfield);
...
#ifdef SGS_TURBULENCE
  MPI_Allreduce(&PowerDc, &PowerDc_global, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
#endif /* #ifdef SGS_TURBULENCE */
}
...
double powerspec_turb_obtain_fields(int type)
{
  ...
  turb_large_array_offset n;
  Ncount = ((turb_large_array_offset)myplan.nslab_x) * (POWERSPEC_GRID * POWERSPEC_GRID); /* number of grid points on the local slab */
  ...
  if(index >= 0)
    {
...
#ifdef SGS_TURBULENCE
      out.SgsTSpecificEnergy = SphP[index].SgsTData.SpecificEnergy;
#ifdef SGS_TURBULENCE_MAKE_PRODUCTION_DISSIPATION_SPECTRUM
      out.SGS_T_DuDt_Adiab    = SphP[index].SgsTData.DuDt_Adiab;
      out.SGS_T_DuDt_TurbProd = SphP[index].SgsTData.DuDt_TurbProd;
      out.SGS_T_DuDt_ViscDiss = SphP[index].SgsTData.DuDt_ViscDiss;
#endif
#endif
#else
      ...
      unsigned long long xxb =
          ngb_double_to_int(((pos[0] - P[j].Pos[0] + amr_length[Mesh.DP[j].level + 1]) / amr_length[Mesh.DP[j].level]) + 1.0);
      unsigned long long yyb =
          ngb_double_to_int(((pos[1] - P[j].Pos[1] + amr_length[Mesh.DP[j].level + 1]) / amr_length[Mesh.DP[j].level]) + 1.0);
      unsigned long long zzb =
          ngb_double_to_int(((pos[2] - P[j].Pos[2] + amr_length[Mesh.DP[j].level + 1]) / amr_length[Mesh.DP[j].level]) + 1.0);
