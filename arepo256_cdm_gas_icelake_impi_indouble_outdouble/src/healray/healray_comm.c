...
void healray_split_comm(void)
{
  ...
  // mpi_distribute_items_from_search(searchdata, HRD.TmpHRRD, &HRD.TmpNumRays, &HRD.TmpMaxNumRays, sizeof(struct HRRD_struct),
  // TAG_DENS_A, offsetof(struct HRRD_struct, task), offsetof(struct HRRD_struct, idx));
  ...
  while(tot_nrays_left)
    {
      ...
      mpi_distribute_items_to_tasks(HRD.TmpHRRD, offsetof(struct HRRD_struct, task), &nrays_recv, &nrays_max, sizeof(struct
HRRD_struct), TAG_DENS_A);
