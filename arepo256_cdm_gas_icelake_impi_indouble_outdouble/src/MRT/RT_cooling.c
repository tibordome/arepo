 ...
 *
 * Perform gas cooling, option only activated if PS2009 or Ps2011 chemistry is used.
 * Can be unstable for large variations in the radiation field internsity/energy.
 */
...
#if defined(MRT) && defined(COOLING)
int do_cooling_mrt(int i)
{
  ...
  double ne, x, tsfr, factorEVP, egyeff = 0.;
...
{
  double n_gamma, c_light = CLIGHT / All.UnitVelocity_in_cm_per_s;

  double a3inv = 1. / All.cf_atime / All.cf_atime / All.cf_atime;

  // ppc->J_UV = 0;
#ifndef MRT_NO_UV
  ppc->J_UV = 1;
// #ifdef MRT_SOURCES
//   if(All.MRT_On == 0) {ppc->J_UV = 0; return ;}
// #endif
#if !defined(GFM_AGN_RADIATION) && !defined(GFM_UVB_CORRECTIONS) && !defined(UVB_SELF_SHIELDING) && !defined(RADCOOL)
  ppc->gJH0 = ppc->gJHep = ppc->gJHe0 = ppc->epsH0 = ppc->epsHep = ppc->epsHe0 = 0;
#endif
  for(int j = 0; j < UV_BINS; j++)
    {
      n_gamma = SphP[i].DensPhot[j] * 1e63 * a3inv;

      ppc->gJH0 += c_light * mrt_sigma_HI[j] * n_gamma;
      ppc->gJHe0 += c_light * mrt_sigma_HeI[j] * n_gamma;
      ppc->gJHep += c_light * mrt_sigma_HeII[j] * n_gamma;

      ppc->epsH0 += c_light * mrt_sigma_HI[j] * G_HI[j] * n_gamma;
      ppc->epsHe0 += c_light * mrt_sigma_HeI[j] * G_HeI[j] * n_gamma;
      ppc->epsHep += c_light * mrt_sigma_HeII[j] * G_HeII[j] * n_gamma;
    }

  // convert to cgs units
  ppc->gJH0 *= (All.HubbleParam / All.UnitTime_in_s);
  ppc->gJHep *= (All.HubbleParam / All.UnitTime_in_s);
  ppc->gJHe0 *= (All.HubbleParam / All.UnitTime_in_s);
  ppc->epsH0 *= All.UnitEnergy_in_cgs / All.UnitTime_in_s;
  ppc->epsHep *= All.UnitEnergy_in_cgs / All.UnitTime_in_s;
  ppc->epsHe0 *= All.UnitEnergy_in_cgs / All.UnitTime_in_s;
#endif
}

void update_chem_mrt(int i, double dtcool, GasState *pgs)
{
  double nH = HYDROGEN_MASSFRAC * SphP[i].Density * All.cf_a3inv / PROTONMASS * All.UnitMass_in_g / All.HubbleParam;
  double KK = dtcool * (CLIGHT / All.UnitVelocity_in_cm_per_s) * nH;

  double ratio;

  for(int j = 0; j < UV_BINS; j++)
    {
      double sum_KK = mrt_sigma_HI[j] * pgs->nH0
#ifdef MRT_INCLUDE_HE
                      + mrt_sigma_HeI[j] * pgs->nHe0 + mrt_sigma_HeII[j] * pgs->nHep
#endif
          ;
  ...
  SphP[i].HII  = pgs->nHp;
  SphP[i].Ne   = pgs->ne;
  SphP[i].nHI  = pgs->nH0 * P[i].Mass;
  SphP[i].nHII = pgs->nHp * P[i].Mass;
  SphP[i].ne   = pgs->ne * P[i].Mass;
#ifdef MRT_INCLUDE_HE
  SphP[i].HeI    = pgs->nHe0;
  SphP[i].HeII   = pgs->nHep;
  SphP[i].HeIII  = pgs->nHepp;
  SphP[i].nHeI   = pgs->nHe0 * P[i].Mass;
  SphP[i].nHeII  = pgs->nHep * P[i].Mass;
  SphP[i].nHeIII = pgs->nHepp * P[i].Mass;
#endif
}
...
double mrt_DoHeating(int i, double dt_internal)
{
  ...
  du = Lambda_Heat * dt_internal / (SphP[i].Density * a3inv); /*LVS: rt_DoHeating.c in arepo has no 1/hubble_a factor here?? */
...
double mrt_get_cooling_rate(int i, double utherm)
{
  ...
  temp = utherm * GAMMA_MINUS1 * (molecular_weight * PROTONMASS / All.UnitMass_in_g * All.HubbleParam) /
  ...
  //  deHe5 = (SphP[i].HeII + SphP[i].HeIII + SphP[i].HII) * nH * SphP[i].Ne * nH * rateHe5;
  ...
  //  if(P[i].Pos[0]<All.BoxSize/30.0)
  // printf("cool=%g\t%g\t%g\t%g\t%g\n", SphP[i].KappaIR[0], cspeed, radiation_constant, pow(temp,4),
  SphP[i].KappaIR[0]*cspeed*radiation_constant*pow(temp,4) ) ;
...
double mrt_get_heating_rate(int i)
{
  ...
  for(j = 0; j < UV_BINS; j++)
    {
      ...
      //      if (SphP[i].DensPhot[j] < 10.0 * MINDENSPHOT)
      // n_gamma = 0.0;
  ...
  //  if(P[i].Pos[0]<All.BoxSize/30.0)
  // printf("heat=%g\t%g\t%g\t%g\n", SphP[i].KappaIR[0], c_internal_units, SphP[i].DensPhot[0],
  SphP[i].KappaIR[0]*c_internal_units*SphP[i].DensPhot[0] ) ; #endif*/
  // return 0.0 ;
  return rate;
}
