 ...
 * than the original 2009. Can be unstable/extremely slow for large variations radiation field internsity.
 *
 */
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      nGamma = SphP[i].Cons_DensPhot[0] * 1e63 / nH / SphP[i].Volume / All.cf_atime;
      ...
      if(fabs(d_nGamma) <= 0.05 * nGamma) /* we are nearly in photoionization equilibrium, and/or the photon density is so large that
                                             it should change little */
        {
          ...
          if(isnan(nGamma_new))
            {
              printf("%g\t%g\t%g\t%g\n", SphP[i].Cons_DensPhot[0], nGamma, d_nGamma, nGamma_new);
      ...
      SphP[i].nHI            = nHI_new * nH_times_volume;
      SphP[i].nHII           = nHII_new * nH_times_volume;
      SphP[i].ne             = nHII_new * nH_times_volume;

      double ratio                      = (nGamma_new * All.cf_atime / 1e63 * nH * SphP[i].Volume) / SphP[i].Cons_DensPhot[0];
