...
static int is_in_shock_zone(int k, double dir_p[3], double dir_m[3])
{
  ...
  // double grad_S = SfVars.c_v * (log(p_p/pow(rho_p, GAMMA)) * rho_p * vol_p - log(p_m/pow(rho_m, GAMMA)) * rho_m * vol_m); //enropy
  // gradient
  ...
  VPRINTF("\tplus neighbour: (%f, %f, %f), density: %f, pressure: %f\n", P[particle_index_p].Pos[0], P[particle_index_p].Pos[1],
          P[particle_index_p].Pos[2], rho_p, p_p);
  VPRINTF("\tminus neighbour: (%f, %f, %f), density: %f, pressure: %f\n", P[particle_index_m].Pos[0], P[particle_index_m].Pos[1],
          P[particle_index_m].Pos[2], rho_m, p_m);
