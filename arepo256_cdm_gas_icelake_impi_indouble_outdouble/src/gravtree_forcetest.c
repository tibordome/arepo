...
/*! \brief This function does the gravitational force computation with direct summation for the
 *  specified particle.
 *
 *  This can be useful for debugging purposes, in particular for explicit checks of the force
 *  accuracy reached with the tree. Depending on whether or not a PMGRID is used, the code does a short-range
 *  tree-walk or a full one.
 *
...
#if !defined(FORCETEST_TESTFORCELAW) && !defined(GRAVITY_TALLBOX)
    ...
    for(int ny = -nmax; ny <= nmax; ny++)
      {
        if(nx != 0 || ny != 0)
          {
            double dx = x - nx * BOXX * All.BoxSize;
            double dy = y - ny * BOXY * All.BoxSize;
            double dz = z;
            double r2 = dx * dx + dy * dy + dz * dz;
            double r = sqrt(r2);
            double val = erfc(alpha * r) + 2.0 * alpha * r / sqrt(M_PI) * exp(-alpha2 * r2);
...
static void ewald_correction_force(double x, double y, double z, double force[4])
{
  ...
  int nxmax = (int)(4 * alpha * All.BoxSize * (BOXX / lmin) + 0.5);
  int nymax = (int)(4 * alpha * All.BoxSize * (BOXY / lmin) + 0.5);
    ...
    for(int ny = -nymax; ny <= nymax; ny++)
      {
        if(nx != 0 || ny != 0)
          {
            double kx = (2.0 * M_PI / (All.BoxSize * BOXX)) * nx;
            double ky = (2.0 * M_PI / (All.BoxSize * BOXY)) * ny;
            double k2 = kx * kx + ky * ky;
            double k = sqrt(k2);

            double val = M_PI / (All.BoxSize * BOXX * All.BoxSize * BOXY) * sin(kx * x + ky * y) *
                         (exp(k * z) * erfc(k / (2 * alpha) + alpha * z) + exp(-k * z) * erfc(k / (2 * alpha) - alpha * z)) / k;

            force[0] += -kx * val;
            force[1] += -ky * val;
            force[2] += M_PI / (All.BoxSize * BOXX * All.BoxSize * BOXY) * cos(kx * x + ky * y) *
                        (exp(k * z) * erfc(k / (2 * alpha) + alpha * z) - exp(-k * z) * erfc(k / (2 * alpha) - alpha * z));
          }
      }

  force[2] += -2.0 * M_PI / (All.BoxSize * BOXX * All.BoxSize * BOXY) * erf(alpha * z);
