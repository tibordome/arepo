...
void domain_prepare_voronoi_dynamic_update(void)
{
  ...
  trans_table = (struct trans_data *)mymalloc_movable(&trans_table, "trans_table", N_trans * sizeof(struct trans_data));
...
#endif
            {
              sprintf(buf, "%c%cbin=%2d     %10lld  %10lld  %10lld  %c %6.3f |%6.3f  %c   %6.3f |%6.3f\n",
