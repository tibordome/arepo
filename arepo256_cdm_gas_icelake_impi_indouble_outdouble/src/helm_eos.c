...
static inline double exp10(double x) { return exp(log(10) * x); }
#endif  // _GNU_SOURCE

// quintic hermite polynomials

// psi0 and its derivatives
static inline double psi0(double z) { return z * z * z * (z * (-6.0 * z + 15.0) - 10.0) + 1.0; }

static inline double dpsi0(double z) { return z * z * (z * (-30.0 * z + 60.0) - 30.0); }

static inline double ddpsi0(double z) { return z * (z * (-120.0 * z + 180.0) - 60.0); }

// psi1 and its derivatives
static inline double psi1(double z) { return z * (z * z * (z * (-3.0 * z + 8.0) - 6.0) + 1.0); }

static inline double dpsi1(double z) { return z * z * (z * (-15.0 * z + 32.0) - 18.0) + 1.0; }

static inline double ddpsi1(double z) { return z * (z * (-60.0 * z + 96.0) - 36.0); }

// psi2 and its derivatives
static inline double psi2(double z) { return 0.5 * z * z * (z * (z * (-z + 3.0) - 3.0) + 1.0); }

static inline double dpsi2(double z) { return 0.5 * z * (z * (z * (-5.0 * z + 12.0) - 9.0) + 2.0); }

static inline double ddpsi2(double z) { return 0.5 * (z * (z * (-20.0 * z + 36.0) - 18.0) + 2.0); }

// biquintic hermite polynomial
static inline double h5(double fi[36], double w0t, double w1t, double w2t, double w0mt, double w1mt, double w2mt, double w0d,
                        double w1d, double w2d, double w0md, double w1md, double w2md)
{
  return fi[0] * w0d * w0t + fi[1] * w0md * w0t + fi[2] * w0d * w0mt + fi[3] * w0md * w0mt + fi[4] * w0d * w1t + fi[5] * w0md * w1t +
         fi[6] * w0d * w1mt + fi[7] * w0md * w1mt + fi[8] * w0d * w2t + fi[9] * w0md * w2t + fi[10] * w0d * w2mt +
         fi[11] * w0md * w2mt + fi[12] * w1d * w0t + fi[13] * w1md * w0t + fi[14] * w1d * w0mt + fi[15] * w1md * w0mt +
         fi[16] * w2d * w0t + fi[17] * w2md * w0t + fi[18] * w2d * w0mt + fi[19] * w2md * w0mt + fi[20] * w1d * w1t +
         fi[21] * w1md * w1t + fi[22] * w1d * w1mt + fi[23] * w1md * w1mt + fi[24] * w2d * w1t + fi[25] * w2md * w1t +
         fi[26] * w2d * w1mt + fi[27] * w2md * w1mt + fi[28] * w1d * w2t + fi[29] * w1md * w2t + fi[30] * w1d * w2mt +
         fi[31] * w1md * w2mt + fi[32] * w2d * w2t + fi[33] * w2md * w2t + fi[34] * w2d * w2mt + fi[35] * w2md * w2mt;
}

// cubic hermite polynomial
// psi0 and its derivatives
static inline double xpsi0(double z) { return z * z * (2.0 * z - 3.0) + 1.0; }

static inline double xdpsi0(double z) { return z * (6.0 * z - 6.0); }

// psi1 and its derivatives
static inline double xpsi1(double z) { return z * (z * (z - 2.0) + 1.0); }

static inline double xdpsi1(double z) { return z * (3.0 * z - 4.0) + 1.0; }

// bicubic hermite polynomial
static inline double h3(double fi[16], double w0t, double w1t, double w0mt, double w1mt, double w0d, double w1d, double w0md,
                        double w1md)
{
  return fi[0] * w0d * w0t + fi[1] * w0md * w0t + fi[2] * w0d * w0mt + fi[3] * w0md * w0mt + fi[4] * w0d * w1t + fi[5] * w0md * w1t +
         fi[6] * w0d * w1mt + fi[7] * w0md * w1mt + fi[8] * w1d * w0t + fi[9] * w1md * w0t + fi[10] * w1d * w0mt +
         fi[11] * w1md * w0mt + fi[12] * w1d * w1t + fi[13] * w1md * w1t + fi[14] * w1d * w1mt + fi[15] * w1md * w1mt;
}
...
static inline void azbar(const double xnuc[], double *abar, double *zbar)
{
  ...
  *zbar = *zbar / xsum * *abar;
  ...
  if(!gsl_finite(*abar) || !gsl_finite(*zbar))
    {
      printf("HELMEOS: FAILURE Task=%d, abar=%g, zbar=%g\n", ThisTask, *abar, *zbar);
      for(int i = 0; i < nspecies; i++)
        printf("HELMEOS: FAILURE Task=%d, xnuc[%d]=%g\n", ThisTask, i, xnuc[i]);

      *(int *)(NULL) = 1;
      terminate("HELMEOS: FAILURE");
    }
}
...
helm_eos_rad(double rho, double temp, double abar, double zbar, double prad[5], double erad[5], double srad[5])
{
...
// ion section
static int __attribute__((unused)) helm_eos_ion(double rho, double temp, double ltemp, const struct helm_eos_cache *cache, double abar,
                                                double zbar, double pion[5], double eion[5], double sion[5])
{
  ...
  // const double s = (2.0 * M_PI * GSL_CONST_CGS_UNIFIED_ATOMIC_MASS * GSL_CONST_CGS_BOLTZMANN) / (GSL_CONST_CGS_PLANCKS_CONSTANT_H *
  // GSL_CONST_CGS_PLANCKS_CONSTANT_H) * temp; const double y = log(abar * abar * sqrt(abar) / (rho * GSL_CONST_NUM_AVOGADRO) * s *
  // sqrt(s));
  ...
  sion[2] = (pion[2] / rho + eion[2]) / temp - (pion[0] / rho + eion[0]) / (temp * temp) +
            1.5 * GSL_CONST_CGS_BOLTZMANN * GSL_CONST_NUM_AVOGADRO * ytot / temp;
  // dsion da
...
// electron-positron
static int __attribute__((unused))
helm_eos_ele(double rho, double temp, double l10temp, double pele[5], double eele[5], double sele[5], double etaele[5], double xne[5],
             const struct helm_eos_cache *cache, const int only_e)
{
  ...
  if(iat < 0 || iat >= helm_eos_table->nrho)
    {
      fprintf(stderr, "density (%g) off table [%d:%d], %g %g %g %g %d\n", rho, 0, helm_eos_table->nrho, cache->abar, cache->zbar,
              cache->ldin, cache->ye, iat);
      return -1;
    }
...
// Coulomb corrections
static int __attribute__((unused))
helm_eos_coul(double rho, double temp, double abar, double zbar, double pcoul[5], double ecoul[5], double scoul[5])
{
  // fitting parameters
...
int eos_init(const char *datafile, const char *speciesfile)
{
  ...
  helm_eos_table = (struct helm_eos_table *)malloc(sizeof(struct helm_eos_table));
        ...
        for(i = 0; i < helm_eos_table->nrho; i++)
          {
            ...
            if(fscanf(file, "%lf %lf %lf %lf %lf %lf %lf %lf %lf", &helm_eos_table->f[i][j], &helm_eos_table->fd[i][j],
                      &helm_eos_table->ft[i][j], &helm_eos_table->fdd[i][j], &helm_eos_table->ftt[i][j], &helm_eos_table->fdt[i][j],
                      &helm_eos_table->fddt[i][j], &helm_eos_table->fdtt[i][j], &helm_eos_table->fddtt[i][j]) != 9)
              {
    ...
    for(j = 0; j < helm_eos_table->ntemp; j++)
      {
        for(i = 0; i < helm_eos_table->nrho; i++)
          {
            if(fscanf(file, "%lf %lf %lf %lf", &helm_eos_table->dpdf[i][j], &helm_eos_table->dpdfd[i][j], &helm_eos_table->dpdft[i][j],
                      &helm_eos_table->dpdfdt[i][j]) != 4)
              {
    ...
    for(j = 0; j < helm_eos_table->ntemp; j++)
      {
        for(i = 0; i < helm_eos_table->nrho; i++)
          {
            if(fscanf(file, "%lf %lf %lf %lf", &helm_eos_table->ef[i][j], &helm_eos_table->efd[i][j], &helm_eos_table->eft[i][j],
                      &helm_eos_table->efdt[i][j]) != 4)
              {
    ...
    for(j = 0; j < helm_eos_table->ntemp; j++)
      {
        for(i = 0; i < helm_eos_table->nrho; i++)
          {
            if(fscanf(file, "%lf %lf %lf %lf", &helm_eos_table->xf[i][j], &helm_eos_table->xfd[i][j], &helm_eos_table->xft[i][j],
                      &helm_eos_table->xfdt[i][j]) != 4)
              {
  ...
  helm_eos_table->na  = (double *)malloc(helm_eos_table->nspecies * sizeof(double));
  helm_eos_table->nai = (double *)malloc(helm_eos_table->nspecies * sizeof(double));
  helm_eos_table->nz  = (double *)malloc(helm_eos_table->nspecies * sizeof(double));
  ...
  if(res != NULL && !only_e)
    {
      ...
      res->sound    = GSL_CONST_CGS_SPEED_OF_LIGHT *
                   sqrt(res->gamma_1 / (1.0 + (res->e.v + gsl_pow_2(GSL_CONST_CGS_SPEED_OF_LIGHT)) * rho / res->p.v));
...
int eos_calc_egiven(double rho, const double xnuc[], double e, double *tempguess, struct eos_result *res)
{
  ...
  if(res == NULL)
    {
      res          = (struct eos_result *)malloc(sizeof(struct eos_result));
      have_to_free = true;
    }
...
int helm_eos_update_cache(double rho, double abar, double zbar, struct helm_eos_cache *cache)
{
  ...
  cache->lswot15 = 1.5 * log((2.0 * M_PI * GSL_CONST_CGS_UNIFIED_ATOMIC_MASS * GSL_CONST_CGS_BOLTZMANN) /
                             (GSL_CONST_CGS_PLANCKS_CONSTANT_H * GSL_CONST_CGS_PLANCKS_CONSTANT_H));
