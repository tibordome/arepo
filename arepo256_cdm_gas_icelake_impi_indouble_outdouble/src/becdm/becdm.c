...
#if defined(PMGRID) && defined(DOUBLEPRECISION_FFTW) && defined(BECDM)
...
void do_becdm_kinetic_drift(void)
{
...
#else
  assert((PMGRID % NTask == 0) || (NTask % PMGRID == 0));  // for optimality
#endif
  ...
  /* Set up the FFTW */
  myplan.forward_plan_zdir = FFTW(plan_many_dft)(1, ndim, 1, densityfield, 0, 1, PMGRID, workspace, 0, 1, PMGRID, FFTW_FORWARD,
                                                 FFTW_ESTIMATE | FFTW_DESTROY_INPUT | alignflag);
  myplan.forward_plan_ydir = FFTW(plan_many_dft)(1, ndim, 1, densityfield, 0, stride, PMGRID * PMGRID, workspace, 0, stride,
                                                 PMGRID * PMGRID, FFTW_FORWARD, FFTW_ESTIMATE | FFTW_DESTROY_INPUT | alignflag);
  myplan.forward_plan_xdir = FFTW(plan_many_dft)(1, ndim, 1, densityfield, 0, stride, PMGRID * PMGRID, workspace, 0, stride,
                                                 PMGRID * PMGRID, FFTW_FORWARD, FFTW_ESTIMATE | FFTW_DESTROY_INPUT | alignflag);

  myplan.backward_plan_xdir = FFTW(plan_many_dft)(1, ndim, 1, densityfield, 0, stride, PMGRID * PMGRID, workspace, 0, stride,
                                                  PMGRID * PMGRID, FFTW_BACKWARD, FFTW_ESTIMATE | FFTW_DESTROY_INPUT | alignflag);
  myplan.backward_plan_ydir = FFTW(plan_many_dft)(1, ndim, 1, densityfield, 0, stride, PMGRID * PMGRID, workspace, 0, stride,
                                                  PMGRID * PMGRID, FFTW_BACKWARD, FFTW_ESTIMATE | FFTW_DESTROY_INPUT | alignflag);
  myplan.backward_plan_zdir = FFTW(plan_many_dft)(1, ndim, 1, densityfield, 0, 1, PMGRID, workspace, 0, 1, PMGRID, FFTW_BACKWARD,
                                                  FFTW_ESTIMATE | FFTW_DESTROY_INPUT | alignflag);
...
#else
  my_column_based_fft_init_c2c(&myplan, PMGRID, PMGRID, PMGRID);
  size_t maxfftsize = myplan.max_datasize;
#endif
  ...
  memset(workspace, 0, maxfftsize * sizeof(fft_complex));

  /* [2] make grid of psi for fft */
  particle2fftgrid(&myplan);  // fill up psifield
...
#else
  my_column_based_fft_c2c(&myplan, psifield, workspace, 1); /* result is in workspace, not in psifield as in slab-based case! */
  fft_complex *fft_of_psifield = (fft_complex *)&workspace[0];
#endif
...
#pragma omp parallel for private(x, y, z)
  for(large_array_offset ip = 0; ip < myplan.second_transposed_ncells; ip++)
    {
      large_array_offset ipcell = ip + ((large_array_offset)myplan.second_transposed_firstcol) * PMGRID;
      y                         = ipcell / (PMGRID * PMGRID);
      int yr                    = ipcell % (PMGRID * PMGRID);
      z                         = yr / PMGRID;
      x                         = yr % PMGRID;
#endif
          ...
          fft_of_psifield[ip][0] =
              (cosx * psihatRe + sinx * psihatIm) /
              pow(PMGRID, 3);  // evolves as  psi = exp(dt * (hbar/m) * (-1.i*((2*pi/Lbox)^2*double(kSq))/2)) * psi
  ...
  my_slab_based_fft_c2c(&myplan, &psifield[0], &workspace[0], -1);
#else
  my_column_based_fft_c2c(&myplan, workspace, psifield, -1);  // now psifield holds the evolved psi
#endif
...
#else
  myfree(myplan.count_recv_23back);
  myfree(myplan.count_send_23back);
  myfree(myplan.count_recv_13back);
  myfree(myplan.count_send_13back);
  myfree(myplan.count_recv_23);
  myfree(myplan.count_send_23);
  myfree(myplan.count_recv_13);
  myfree(myplan.count_send_13);
  myfree(myplan.count_recv_D);
  myfree(myplan.count_send_D);
  myfree(myplan.count_recv_C);
  myfree(myplan.count_send_C);
  myfree(myplan.count_recv_B);
  myfree(myplan.count_send_B);
  myfree(myplan.count_recv_A);
  myfree(myplan.count_send_A);
  myfree(myplan.offsets_recv_23back);
  myfree(myplan.offsets_send_23back);
  myfree(myplan.offsets_recv_13back);
  myfree(myplan.offsets_send_13back);
  myfree(myplan.offsets_recv_23);
  myfree(myplan.offsets_send_23);
  myfree(myplan.offsets_recv_13);
  myfree(myplan.offsets_send_13);
  myfree(myplan.offsets_recv_D);
  myfree(myplan.offsets_send_D);
  myfree(myplan.offsets_recv_C);
  myfree(myplan.offsets_send_C);
  myfree(myplan.offsets_recv_B);
  myfree(myplan.offsets_send_B);
  myfree(myplan.offsets_recv_A);
  myfree(myplan.offsets_send_A);
#endif
...
void particle2fftgrid(fft_plan *myplan)
{
  ...
  // [1] add locals to psifield[][], count non-locals on each task
  for(i = 0; i < NTask; i++)
    {
      psi_send_count[i] = 0;
      psi_recv_count[i] = 0;
...
#else
          correspTask           = y * NTask / PMGRID + z * NTask / PMGRID / PMGRID;
          correspSlabStartY     = correspTask * PMGRID / NTask;
          large_array_offset ip = ((large_array_offset)PMGRID) * (PMGRID * (y - correspSlabStartY) + z) + x;
#endif
      ...
      if(receive_task < NTask)
        {
          if(psi_send_count[receive_task] > 0 || psi_recv_count[receive_task] > 0)
            {
              MPI_Sendrecv(&psi_out[send_offset[receive_task]], psi_send_count[receive_task] * sizeof(psiType), MPI_BYTE, receive_task,
                           0, &psi_in[receive_offset[receive_task]], psi_recv_count[receive_task] * sizeof(psiType), MPI_BYTE,
                           receive_task, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
        }
    }
...
void fftgrid2particle(fft_plan *myplan)
{
  ...
  // [1] add psifield[][] local particles, count non-locals on each task
  for(i = 0; i < NTask; i++)
    {
      psi_send_count[i] = 0;
      psi_recv_count[i] = 0;
      ...
      if(receive_task < NTask)
        {
          if(psi_recv_count[receive_task] > 0 || psi_send_count[receive_task] > 0)
            {
              MPI_Sendrecv(&psi_in[receive_offset[receive_task]], psi_recv_count[receive_task] * sizeof(psiType), MPI_BYTE,
                           receive_task, 0, &psi_out[send_offset[receive_task]], psi_send_count[receive_task] * sizeof(psiType),
                           MPI_BYTE, receive_task, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
        }
    }
