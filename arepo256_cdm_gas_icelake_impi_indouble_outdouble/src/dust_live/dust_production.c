...
#if defined(DL_ONLY_HIGHRES_DUST) && defined(DL_REFINEMENT)
      /* For zoom-in runs, very large star particles could try to spawn large
       * dust particles, which would later cause issues with dust density
       * estimates and enclosed mass constraints.  Try to avoid this. */
      if(m_dust_each > 3.0 * All.DustMaxFrac * All.TargetGasMass)
        continue;
#endif

      if(prob < 0)
        terminate("prob < 0, P[i].Mass=%g m_dust=%g m_frac=%g STP(i).DeltaDustMassTot=%g", P[i].Mass, m_dust, m_frac,
                  STP(i).DeltaDustMassTot);

      if(prob > 1)
        {
          printf(
              "DUST_LIVE: Warning, need to make a dust particle heavier than desired. Task=%d prob=%g P[i].Mass=%g m_dust=%g "
              "m_dust_new=%g\n",
          ...
          for(int j = 0; j < All.NumDustPerSpawn; j++)
            {
              if(NumPart + dust_spawned >= All.MaxPart)
                terminate("For NumPart=%d and All.MaxPart=%d, no space left after spawning %d dust particles", NumPart, All.MaxPart,
                          dust_spawned);
...
#if defined(DL_SNE_DESTRUCTION) || defined(DL_SHATTERING) || defined(DL_COAGULATION)
  DustP[N_dust].MinNumNgbDeviationDust = All.MaxNumNgbDeviationDust;
  DustP[N_dust].MaxNumNgbDeviationDust = All.MaxNumNgbDeviationDust;
#endif

  DustP[N_dust].BinMassChgTau    = MAX_REAL_NUMBER;
  DustP[N_dust].OldBinMassChgTau = MAX_REAL_NUMBER;
  ...
  for(int idx = 0; idx < TimeBinsDust.NActiveParticles; idx++)
    {
      ...
      if(NumPart + dust_created >= All.MaxPart)
        terminate("For NumPart=%d and All.MaxPart=%d, no space left after creating %d dust particles", NumPart, All.MaxPart,
                  dust_created);
...
void subdivide_dust_particle(int iorig, int idust)
{
  ...
  DustP[N_dust].MinNumNgbDeviationDust = DTP(iorig).MinNumNgbDeviationDust;
  DustP[N_dust].MaxNumNgbDeviationDust = DTP(iorig).MaxNumNgbDeviationDust;
#endif

  DustP[N_dust].BinMassChgTau    = DTP(iorig).BinMassChgTau;
  DustP[N_dust].OldBinMassChgTau = DTP(iorig).OldBinMassChgTau;
  /* Needed for timestep criterion. */
...
{
  /* Use information about dust particles' closest high-mass dust neighbors to
   * actually transfer the contents of small dust particles. */
  exchange_derefine_results();
  ...
  for(int idx = 0; idx < TimeBinsDust.NActiveParticles; idx++)
    {
      ...
      /* We can only derefine dust particles below the derefinement mass limit,
       * with a valid neighbor, and with no particles derefining into us. */
      if((P[i].Mass >= All.DustMinFrac * All.TargetGasMass) || (DTP(i).HighMassNeighbor == 0) || (DTP(i).IsDerefinementTarget == 1))
        continue;

      P[i].Mass = 0.0;

      int active_grav_idx = -1;
      for(int idx_grav = 0; idx_grav < TimeBinsGravity.NActiveParticles; idx_grav++)
        {
          int i_grav = TimeBinsGravity.ActiveParticleList[idx_grav];
          if((i_grav >= 0) && (i_grav == i))
            {
              active_grav_idx = idx_grav;
              break;
            }
        }
      ...
      timebin_remove_particle(&TimeBinsDust, idx, P[i].TimeBinHydro);
      timebin_remove_particle(&TimeBinsGravity, active_grav_idx, P[i].TimeBinGrav);

      nderefine++;
    }

  int tot_nderefine;
  MPI_Allreduce(&nderefine, &tot_nderefine, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

  if(tot_nderefine > 0)
    mpi_printf("DUST_LIVE: derefinement removed %d dust particles.\n", tot_nderefine);
}
#endif
