...
/* Quick and dirty temporary fix for Llyod reg since the original module was broken with the time stepping change - only works for
 * single tstep, 1 processor*/
...
#ifdef TWODIMS
void process_edge_faces_and_volumes_predict_exact(tessellation *T, double *vol, double *cmxv, double *cmyv, double *cmzv, int tt,
                                                  int nr)
{
  int i, j, qq, p1, p2;
  tetra *q;
          ...
          if((dt > 0) && (SphP[i].Volume_predict > 0))
            {
              ...
              d_anchor = sqrt(pow(SphP[i].Center_predict[0] - SphP[i].Center_anchor[0], 2) +
                              pow(SphP[i].Center_predict[1] - SphP[i].Center_anchor[1], 2) +
              ...
              for(j = 0; j < 3; j++)
                {
                  SphP[i].Center_predict[j] =
                      SphP[i].Center_anchor[j] + (SphP[i].Center_predict[j] - SphP[i].Center_anchor[j]) *
                                                     fmin(1.0, 0.05 * dt * All.cf_atime * get_sound_speed(i) / d_anchor);
                }
