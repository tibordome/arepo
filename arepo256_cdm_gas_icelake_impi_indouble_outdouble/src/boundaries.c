 ...
 * \date        MM/YYYY
 * \author
 * \brief       Functions for special boundaries.
 * \details
 *
 *
...
/*! \brief Set vertex velocity of boundary cells to predefined value.
 *
 *  The boundary cells of a moving boundary should by construction not
 *  follow the moving-mesh criteria. This function sets the vertex
 *  velocity of the boundary cells to its predefined value.
 *
 *  \param[in, out] localP Pointer to local particle array.
 *  \param[out] localSphP Pointer to local cell array (SphP).
 *  \param[in] p Index of cell in localP and localSphP array.
 *
 *  \return void
 */
...
double usr_defined_get_damping_weight(int i)
{
  ...
  return weight;
#endif /* #ifdef CIRCUMSTELLAR_WBOUNDARIES */
  return 0;
}

/*! \brief Velocity of moving boundary.
 *
 *  Depending on the type of special boundary motion this funciton returns
 *  the appropriate motion given a specified position.
 *
 *  \param[in] x X-position of a cell.
 *  \param[in] y Y-position of a cell.
 *  \param[in] z Z-position of a cell.
 *  \param[out] vx pointer to intended vertex x-velocity of a boundary.
 *  \param[out] vy pointer to intended vertex y-velocity of a boundary.
 *  \param[out] vz pointer to intended vertex z-velocity of a boundary.
...
/*! \brief Motion of boundary moving in x direction.
 *
 *  \param[in] x X-position of a cell.
 *  \param[in] y Y-position of a cell.
 *  \param[in] z Z-position of a cell.
...
/*! \brief Motion of boundary moving in y direction.
 *
 *  \param[in] x X-position of a cell.
 *  \param[in] y Y-position of a cell.
 *  \param[in] z Z-position of a cell.
...
/*! \brief Motion of boundary moving in z direction.
 *
 *  \param[in] x X-position of a cell.
 *  \param[in] y Y-position of a cell.
 *  \param[in] z Z-position of a cell.
...
/*! \brief Motion of boundary moving in a circle
 *
 *  The circular motion is around the point x,y = 0.5,0.5 with angular velocity
 *  2/5 pi.
 *
 *  \param[in] x X-position of a cell.
 *  \param[in] y Y-position of a cell.
...
void boundary_double_concentric_motion(double x, double y, double z, double *vx, double *vy, double *vz)
{
  ...
  *vy = (r * sin(phi) - y) / dt;
  *vz = 0.0;
#endif /* #ifdef COAXIAL_BOUNDARIES */
}
...
/*! \brief Sets cell state of a cell of the special boundary.
 *
 *  \param[in] state_inside pointer to state on the inside of boundary (within
 *             domian).
 *  \param[out] state_outside pointer to state on the outside of boundary
 *              (outside domain).
 *  \param[in] orient Orientation; only needed for diode.
 *
 *  \return void
 */
...
/*! \brief Sets outside state of reflective boundary conditions.
 *
 *  \param[in] state_inside pointer to state on the inside of boundary (within
 *             domian).
 *  \param[out] state_outside pointer to state on the outside of boundary
...
/*! \brief Sets outside state of no-slip boundary conditions.
 *
 *  \param[in] state_inside pointer to state on the inside of boundary (within
 *             domian)
 *  \param[out] state_outside pointer to state on the outside of boundary
...
/*! \brief Sets outside state of subsonic outflow non-reflective boundary
 *         conditions.
 *
 *  \param[in] state_inside pointer to state on the inside of boundary
 *             (within domian).
 *  \param[out] state_outside pointer to state on the outside of boundary
 *              (outside domain).
 *
...
/*! \brief Sets outside state of subsonic ouflow open boundary conditions
 *
 *  \param[in] state_inside pointer to state on the inside of boundary
 *             (within domian).
 *  \param[out] state_outside pointer to state on the outside of boundary
...
/*! \brief Sets outside state of diode-type ouflow open boundary conditions
 *
 *  \param[in] state_inside pointer to state on the inside of boundary
 *             (within domian).
 *  \param[out] state_outside pointer to state on the outside of boundary
