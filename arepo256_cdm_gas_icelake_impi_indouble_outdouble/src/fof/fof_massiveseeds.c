...
void blackhole_massiveseeds(int nseed, int *seed_indices)
{
  ...
  All.BunchSize  = (int)((All.BufferSize * 1024 * 1024) /
                        (sizeof(data_index) + sizeof(struct data_nodelist) + sizeof(struct densdata_in) + sizeof(struct densdata_out) +
                         smax(sizeof(struct densdata_in), sizeof(struct densdata_out))));
          ...
          for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
            {
              ...
              if(recvTask < NTask)
                if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
                  /* get the particles */
                  MPI_Sendrecv(&DensDataIn[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct densdata_in), MPI_BYTE,
                               recvTask, TAG_DENS_A, &DensDataGet[Recv_offset[recvTask]],
                               Recv_count[recvTask] * sizeof(struct densdata_in), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                               MPI_STATUS_IGNORE);
            }
          ...
          for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
            {
              ...
              if(recvTask < NTask)
                if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
                  MPI_Sendrecv(&DensDataResult[Recv_offset[recvTask]], Recv_count[recvTask] * sizeof(struct densdata_out), MPI_BYTE,
                               recvTask, TAG_DENS_B, &DensDataOut[Send_offset[recvTask]],
                               Send_count[recvTask] * sizeof(struct densdata_out), MPI_BYTE, recvTask, TAG_DENS_B, MPI_COMM_WORLD,
                               MPI_STATUS_IGNORE);
            }
          ...
          if(blackhole_isactive(i))
            {
              ...
              if(BPP(i).BH_NumNgb < (All.DesNumNgbSeed - All.MaxNumNgbDeviation) ||
                 (BPP(i).BH_NumNgb > (All.DesNumNgbSeed + All.MaxNumNgbDeviation) && BPP(i).BH_Hsml > (1.01 * All.MinGasHsml)))
                {
                  ...
                  if(iter >= MAXITER - 10)
                    {
                      printf("i=%d task=%d ID=%d Hsml=%g Left=%g Right=%g Ngbs=%g Right-Left=%g\n   pos=(%g|%g|%g)\n", i, ThisTask,
                             (int)P[i].ID, BPP(i).BH_Hsml, Left[i], Right[i], (double)BPP(i).BH_NumNgb, Right[i] - Left[i],
...
void remove_swallowed_gas(int nseed, int *seed_indices)
{
  ...
  seed_indices_remove = (int *)mymalloc("seed_indices_remove", nseed_remove * sizeof(int));
  ...
  if(total_nseed_remove > 0)
    {
      ...
      seed_ID     = (MyIDType *)mymalloc("seed_ID", nseed_remove * sizeof(MyIDType));
      tot_seed_ID = (MyIDType *)mymalloc("tot_seed_ID", total_nseed_remove * sizeof(MyIDType));
      ...
      common_nseed = (int *)mymalloc("common_nseed", NTask * sizeof(int));
      disp         = (int *)mymalloc("disp", NTask * sizeof(int));
...
#else
      MPI_Allgatherv(seed_ID, nseed_remove, MPI_UNSIGNED_LONG_LONG, tot_seed_ID, common_nseed, disp, MPI_UNSIGNED_LONG_LONG,
                     MPI_COMM_WORLD);
#endif
