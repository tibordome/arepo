 ...
 *
 * \file        src/network/network_solver.c
 * \date        MM/YYYY
 * \author
 * \brief
 * \details
...
print_matrix(const int n, const double *matrix, const int *rowstart, const int *columns, const double *rhs, const int offset);

static void network_solver_integrate_internal(double temp, double rho, double *y, double maxtime, const struct network_data *nd,
                                              struct network_workspace *nw, struct network_solver_trajectory *traj);
static void network_solver_interpolate_traj_rhotemp(struct network_solver_trajectory *traj, double time, double *y, double *rho,
                                                    double *temp);
static void network_solver_calc_dy(double temp, double rho, double dt, int steps, double *y, double *yn, const double *first_rhs,
                                   const jacob_t *old_jacob, const struct network_data *nd, struct network_workspace *nw,
                                   struct network_solver_trajectory *traj);
static void network_solver_extrapolate(int iter, int matrixsize, double *x, double *y, double *dy, double *qcol);
...
struct network_solver_data *network_solver_init(double tolerance, int matrixsize, int nelements, const struct network_nucdata *nucdata)
{
  ...
  nsd = (struct network_solver_data *)malloc(sizeof(struct network_solver_data));
  ...
  nsd->aion      = (double *)malloc(nelements * sizeof(double));
  ...
  nsd->rhs_deriv = (network_var *)malloc(nsd->matrixsize * sizeof(network_var));
  nsd->first_rhs = (double *)malloc(nsd->matrixsize * sizeof(double));
  nsd->rhs       = (double *)malloc(nsd->matrixsize * sizeof(double));
...
#if NETWORK_SPARSE
  nsd->jacob.values       = (double *)malloc(nsd->matrixsize2 * sizeof(double));
  nsd->jacob.columns      = (int *)malloc(nsd->matrixsize2 * sizeof(int));
  nsd->jacob.rowstart     = (int *)malloc((nsd->matrixsize + 1) * sizeof(int));
  nsd->mod_jacob.values   = (double *)malloc(nsd->matrixsize2 * sizeof(double));
  nsd->mod_jacob.columns  = (int *)malloc(nsd->matrixsize2 * sizeof(int));
  nsd->mod_jacob.rowstart = (int *)malloc((nsd->matrixsize + 1) * sizeof(int));
#else
  nsd->jacob.matrix     = (double *)malloc(nsd->matrixsize2 * sizeof(double));
  nsd->mod_jacob.matrix = (double *)malloc(nsd->matrixsize2 * sizeof(double));
  ...
  for(i = 1; i < nsd->maxstep; i++)
    {
      for(k = 0; k < i; k++)
        {
          nsd->alf[i * nsd->maxstep + k] = pow(
              nsd->tolerance * 0.25, (nsd->a[k + 1] - nsd->a[i + 1]) / ((nsd->a[i + 1] - nsd->a[0] + 1.0) * (2. * (k + 1.0) + 1.0)));
        }
    }
...
void network_solver_integrate_traj(const struct network_data *nd, struct network_workspace *nw, struct network_solver_trajectory *traj)
{
  ...
  terminate("Integrate trajectory does not work with NETWORK_VARIABLE-switch.\n");
#endif
                                              ...
                                              struct network_workspace *nw, struct network_solver_trajectory *traj)
{
  ...
  if(fp == NULL)
    {
      terminate("error opening file `network_output.dat'");
    }
                                   ...
                                   struct network_solver_trajectory *traj)
{
  ...
  dCreate_CompRow_Matrix(&A, nMatrix, nMatrix, jacob->n_elements, jacob->values, jacob->columns, jacob->rowstart, SLU_NR, SLU_D,
                         SLU_GE);
                                   ...
                                   struct network_solver_trajectory *traj)
{
  ...
  if(!(ddumarray = (double *)malloc(matrixsize * sizeof(double))))
    {
      terminate("error allocating temporary space");
    }

  /* do decomposition only once */
  phase = 12;
  PARDISO(pt, &maxfct, &mnum, &mtype, &phase, &matrixsize, values, rowstart, columns, &idum, &nrhs, iparm, &msglvl, &ddum, &ddum,
          &error);
  if(error != 0)
    {
      terminate("error during LU decomposition\n");
    }
  ...
  iparm[7] = refinement; /* Max numbers of iterative refinement steps. */
  phase = 33;
  PARDISO(pt, &maxfct, &mnum, &mtype, &phase, &matrixsize, values, rowstart, columns, &idum, &nrhs, iparm, &msglvl, rhs, ddumarray,
          &error);
  if(error != 0)
    {
      terminate("error during first step\n");
    }
  ...
  for(j = 1; j < steps; j++)
    {
      ...
      if(error != 0)
        {
          terminate("error during middle step\n");
        }
      for(i = 0; i < matrixsize; i++)
        {
          dy[i] = dy[i] + 2.0 * rhs[i];
  ...
  if(error != 0)
    {
      terminate("error during last step\n");
    }
  for(i = 0; i < matrixsize; i++)
    {
      yn[i] = yn[i] + rhs[i];
...
static void print_matrix(const int n, const double *matrix, const int *rowstart, const int *columns, const double *rhs,
                         const int offset)
{
  int i, j;
  const int *col;
...
static void sanitize(double *y, const struct network_data *nd)
{
  ...
  for(yy = y; yy != yend; yy++)
    *yy = min(1.0, max(1e-30, *yy));
#ifdef NETWORK_SEP_YZ
  y[nd->iYz] = max(1e-30, min(y[nd->iYz], nd->nucdata[nd->nuc_count - 1].nz));
#endif
#if NETWORK_VARIABLE
  y[nd->iTemp] = max(1e7, min(y[nd->iTemp], NETWORK_T_MAX));
#endif
#if NETWORK_VARIABLE == NETWORK_VAR_RHO_TEMP
  y[nd->iRho] = max(1e-6, min(y[nd->iRho], 1e11));
#endif
}
