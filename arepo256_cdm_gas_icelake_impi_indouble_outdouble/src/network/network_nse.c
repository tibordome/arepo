...
const static double network_nse_parttemp[24] = {1.0e8, 1.5e8, 2.0e8, 3.0e8, 4.0e8, 5.0e8, 6.0e8, 7.0e8, 8.0e8, 9.0e8, 1.0e9, 1.5e9,
...
static void calcyi(double temp, double rho, double ye, double mu_n, double mu_p, double *yi, double *x, struct network_data *nd,
...
int network_nse_init(char *speciesfile, char *ratesfile, char *partfile, char *massesfile, char *weakratesfile,
                     struct network_data *nd, struct network_workspace *nw)
{
...
void network_nse_integrate_ye(double energy, double rho, double x[], double dt, double *dedt, double *temp, struct network_data *nd,
                              struct network_workspace *nw)
{
...
int network_nse_egiven(double energy, double rho, double ye, double *xnuc, double *tempguess, struct network_data *nd,
                       struct network_workspace *nw)
{
...
static int network_nse_efunc(const gsl_vector *x, void *params, gsl_vector *f)
{
  ...
  temp = fmin(fmax(temp, 1e9), 2e10);
  mu_n = fmin(fmax(mu_n, -1e-4), 1e-4);
  mu_p = fmin(fmax(mu_p, -1e-4), 1e-4);
  ...
  if(nd->nucdata[i].nz != 0.0)
    {
      ...
      return exp(-(A1 * (sqrt(Gamma_i * (A2 + Gamma_i)) - A2 * log(sqrt(Gamma_i / A2) + sqrt(1.0 + Gamma_i / A2))) +
...
static void screening_params(double rho, double ye, double T, double *Gamma_e, double *mu_p_coul)
{
  ...
  *Gamma_e = gsl_pow_2(ELECTRON_CHARGE_ESU) / GSL_CONST_CGS_BOLTZMANN / T *
             pow(4.0 / 3.0 * M_PI * rho * ye / GSL_CONST_CGS_UNIFIED_ATOMIC_MASS, 1.0 / 3.0);
  *mu_p_coul = (A1 * (sqrt(*Gamma_e * (A2 + *Gamma_e)) - A2 * log(sqrt(*Gamma_e / A2) + sqrt(1.0 + *Gamma_e / A2))) +
...
static void calcprefac(double temp, double rho, struct network_data *nd, struct network_workspace *nw)
{
  ...
  for(i = 0; i < nd->nuc_count; i++)
    {
      nw->prefac[i] = nd->nucdata[i].m / rho * (2.0 * nd->nucdata[i].spin + 1.0) * nw->gg[i].v *
  ...
  while(traj->time < maxtime)
    {
      ...
      dt     = fmin(dt, 0.1);  // we do not accept a timestep larger than 0.1s
...
void network_nse_integrate(double temp, double rho, double x[], double dt, double *dedt, struct network_data *nd,
                           struct network_workspace *nw, int verbose)
{
  double ye;
  double *new_x, dx;
  double t, sub_dt;
