...
void rk_save_conservative_variables(void)
{
  int idx, i, k;
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      for(k = 0; k < 3; k++)
        SphP[i].rk.Momentum[k] = SphP[i].Momentum[k];
      SphP[i].rk.Energy = SphP[i].Energy;
#ifdef MAXSCALARS
      for(k = 0; k < MAXSCALARS; k++)
        SphP[i].rk.Scalars[k] = *(MyFloat *)(((char *)(&SphP[i])) + scalar_elements[k].offset_mass);
#endif
#ifdef MHD
      for(k = 0; k < 3; k++)
        SphP[i].rk.BConserved[k] = SphP[i].BConserved[k];
#endif
    }
}

void rk_finish_step(void)
{
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      for(k = 0; k < 3; k++)
        SphP[i].Momentum[k] = 0.5 * (SphP[i].rk.Momentum[k] + SphP[i].Momentum[k]);
      SphP[i].Energy = 0.5 * (SphP[i].rk.Energy + SphP[i].Energy);
#ifdef MAXSCALARS
      for(k = 0; k < MAXSCALARS; k++)
        *(MyFloat *)(((char *)(&SphP[i])) + scalar_elements[k].offset_mass) =
            0.5 * (SphP[i].rk.Scalars[k] + *(MyFloat *)(((char *)(&SphP[i])) + scalar_elements[k].offset_mass));
#endif
#ifdef MHD
      for(k = 0; k < 3; k++)
        SphP[i].BConserved[k] = 0.5 * (SphP[i].rk.BConserved[k] + SphP[i].BConserved[k]);
#endif
    }
}
...
void rk_derefinement_add(struct conservative_variables *target, struct conservative_variables *source, double fac)
{
  ...
  for(k = 0; k < 3; k++)
    target->Momentum[k] += fac * source->Momentum[k];
  target->Energy += fac * source->Energy;
#ifdef MAXSCALARS
  for(k = 0; k < MAXSCALARS; k++)
    target->Scalars[k] += fac * source->Scalars[k];
#endif
#ifdef MHD
  for(k = 0; k < 3; k++)
    target->BConserved[k] += fac * source->BConserved[k];
#endif
}
...
void rk_multiply(struct conservative_variables *target, double fac)
{
  ...
  target->Mass *= fac;
  for(k = 0; k < 3; k++)
    target->Momentum[k] *= fac;
  target->Energy *= fac;
#ifdef MAXSCALARS
  for(k = 0; k < MAXSCALARS; k++)
    target->Scalars[k] *= fac;
#endif
#ifdef MHD
  for(k = 0; k < 3; k++)
    target->BConserved[k] *= fac;
#endif
}
