 ...
 * \date        MM/YYYY
 * \author
 * \brief       Routines for a Rusanov Riemann solver.
 * \details
 *
 *
...
#endif /* #if defined(RIEMANN_HLLC) || defined(RIEMANN_HLLD) || defined(RIEMANN_HLL) || defined(RIEMANN_GAMMA) */
...
/*! \brief Calculates state at face.
 *
 *  \param[in] st_L Left hand side state.
 *  \param[in] st_R Right hand side state.
 *  \param[out] st_face State at face.
 *  \param[in] flux_L Left hand side flux.
 *  \param[in] flux_R Right hand side flux.
 *  \param[in] S Position x / t.
 *
 *  \return void
 */
static void face_get_rosunov_state(const struct state *st_L, const struct state *st_R, struct state_face *st_face,
                                   const struct fluxes *flux_L, const struct fluxes *flux_R, double S)
{
  st_face->rho = 0.5 * (flux_L->mass - flux_R->mass + S * (st_L->rho + st_R->rho)) / S;

  st_face->velx =
      0.5 * (flux_L->momentum[0] - flux_R->momentum[0] + S * (st_L->rho * st_L->velx + st_R->rho * st_R->velx)) / (S * st_face->rho);
  st_face->vely =
      0.5 * (flux_L->momentum[1] - flux_R->momentum[1] + S * (st_L->rho * st_L->vely + st_R->rho * st_R->vely)) / (S * st_face->rho);
  st_face->velz =
      0.5 * (flux_L->momentum[2] - flux_R->momentum[2] + S * (st_L->rho * st_L->velz + st_R->rho * st_R->velz)) / (S * st_face->rho);

  double face_energy = 0.5 * (flux_L->energy - flux_R->energy + S * (st_L->Energy + st_R->Energy)) / S;
  st_face->press     = GAMMA_MINUS1 *
                   (face_energy - 0.5 * st_face->rho *
                                      (st_face->velx * st_face->velx + st_face->vely * st_face->vely + st_face->velz * st_face->velz));
}

/*! \brief Gets Rusanov fluxes.
 *
 *  \param[in] st_L Left state of the Riemann problem
 *  \param[in] st_R Right state of the Riemann problem
 *  \param[in] flux_L Left hand side flux.
 *  \param[in] flux_R Right hand side flux.
 *  \param[out] rosunov_flux Rusanov flux
 *  \param[in] S Position x / t that is evaluated
 *
 *  \return void
 */
static void get_rosunov_fluxes(const struct state *st_L, const struct state *st_R, const struct fluxes *flux_L,
                               const struct fluxes *flux_R, struct fluxes *rosunov_flux, double S)
{
  rosunov_flux->mass = 0.5 * (flux_L->mass + flux_R->mass - S * (st_R->rho - st_L->rho));

  rosunov_flux->momentum[0] =
      0.5 * (flux_L->momentum[0] + flux_R->momentum[0] - S * (st_R->rho * st_R->velx - st_L->rho * st_L->velx));

  rosunov_flux->momentum[1] =
      0.5 * (flux_L->momentum[1] + flux_R->momentum[1] - S * (st_R->rho * st_R->vely - st_L->rho * st_L->vely));

  rosunov_flux->momentum[2] =
      0.5 * (flux_L->momentum[2] + flux_R->momentum[2] - S * (st_R->rho * st_R->velz - st_L->rho * st_L->velz));

  rosunov_flux->energy = 0.5 * (flux_L->energy + flux_R->energy - S * (st_R->Energy - st_L->Energy));
}

/*! \brief Calculates Fluxes across interface from Riemann problem.
 *
 *  \param[in, out] st_L Left hand side state.
 *  \param[in, out] st_R Right hand side state.
 *  \param[out] st_face State at face.
 *  \param[out] flux Flux across interface.
 *
 *  \return Pressure in central region.
 */
  ...
  if(st_L->rho > 0 && st_R->rho > 0)
    {
      ...
      S_plus = fmax(fmax(fabs(st_L->velx - st_L->csnd), fabs(st_R->velx - st_R->csnd)),
                    fmax(fabs(st_L->velx + st_L->csnd), fabs(st_R->velx + st_R->csnd)));
...
#endif /* #if defined(RIEMANN_ROSUNOV) || (defined(RIEMANN_HLLC) && defined(DG)) */
