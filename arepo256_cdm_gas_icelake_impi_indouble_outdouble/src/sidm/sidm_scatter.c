      ...
      for(totScattersInStep = 0, reaction = 0; reaction < SIDM_REACTIONS; reaction++)
        {
          ...
          if(TSIDM[target].ScattersInStep[reaction] > 1)
            terminate(
                "SIDM: multiple scatter -> reaction=%d   out->ScattersInStep[reaction]=%d  target=%d  "
                "TSIDM[target].ScattersInStep[target] = %d\n",
                reaction, out->ScattersInStep[reaction], target, TSIDM[target].ScattersInStep[reaction]);
        }
      ...
      for(totScattersInStep = 0, reaction = 0; reaction < SIDM_REACTIONS; reaction++)
        {
          ...
          if(TSIDM[target].ScattersInStep[reaction] > 1)
            terminate(
                "SIDM: mutliple scatter -> reaction=%d   out->ScattersInStep[reaction]=%d  target=%d  "
                "TSIDM[target].ScattersInStep[reaction] = %d\n",
                reaction, out->ScattersInStep[reaction], target, TSIDM[target].ScattersInStep[reaction]);
        }
    ...
    for(reaction = 0; reaction < SIDM_REACTIONS; reaction++)
      if((TSIDM[i].ScattersInStep[reaction] > 1) || (TSIDM[i].ScattersInStep[reaction] < -1))
        terminate("SIDM: wrong scatter i=%d  reaction=%d  TSIDM[i].ScattersInStep[reaction]=%d  (NumPart + Tree_NumPartImported)=%d\n",
                  i, reaction, TSIDM[i].ScattersInStep[reaction], (NumPart + Tree_NumPartImported));
...
void sidm_Scatter_evaluate(int target, int mode, int threadid)
{
              ...
              if(P[no].ID == scatterID)
                {
                  ...
                  Ekin_before =
                      ...
                      (0.5 * mass * (vel[0] * vel[0] + vel[1] * vel[1] + vel[2] * vel[2]) +
                       0.5 * P[no].Mass * (P[no].Vel[0] * P[no].Vel[0] + P[no].Vel[1] * P[no].Vel[1] + P[no].Vel[2] * P[no].Vel[2]));
              ...
              if(Tree_Points[n].sidm_ID == scatterID)
                {
                  ...
                  Ekin_before = SIDM_avel * SIDM_avel *
                                (0.5 * mass * (vel[0] * vel[0] + vel[1] * vel[1] + vel[2] * vel[2]) +
                                 0.5 * Tree_Points[n].Mass *
                  ...
                  if(TSIDM[NumPart + n].ScattersInStep[reaction] > 1)
                    terminate(
                        "SIDM: multiple scatter -> reaction=%d  TSIDM[NumPart + n].ScattersInStep[reaction]=%d  n=%d  scatterID=%llu  "
                        "  NumPart=%d   (NumPart + Tree_NumPartImported)=%d\n",
                        reaction, TSIDM[NumPart + n].ScattersInStep[reaction], n, (unsigned long long)scatterID, NumPart,
  ...
  Ekin_after = SIDM_avel * SIDM_avel *
               (0.5 * sprdata_out.Mass1 *
                    (sprdata_out.Vel1[0] * sprdata_out.Vel1[0] + sprdata_out.Vel1[1] * sprdata_out.Vel1[1] +
                     sprdata_out.Vel1[2] * sprdata_out.Vel1[2]) +
                0.5 * sprdata_out.Mass2 *
                    (sprdata_out.Vel2[0] * sprdata_out.Vel2[0] + sprdata_out.Vel2[1] * sprdata_out.Vel2[1] +
                     sprdata_out.Vel2[2] * sprdata_out.Vel2[2]));
