...
void otvet_star_lum(void)
{
  ...
  All.BunchSize =
      (int)((All.BufferSize * 1024 * 1024) / (sizeof(data_index) + sizeof(struct data_nodelist) + 2 * sizeof(struct stardata_in)));
          ...
          if(recvTask < NTask)
            {
              if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
                {
                  /* get the particles */
                  MPI_Sendrecv(&StarDataIn[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct stardata_in), MPI_BYTE,
                               recvTask, TAG_DENS_A, &StarDataGet[Recv_offset[recvTask]],
                               Recv_count[recvTask] * sizeof(struct stardata_in), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                               MPI_STATUS_IGNORE);
                }
            }
        }
...
//  lum[0] = fac * All.IonizingLumPerSolarMass * All.UnitTime_in_s / All.HubbleParam;   //LVS: original in Gadget, but "fac" is used
//  again in Je??
#endif
