 ...
 *
 * \file        src/sfr_mcs/estimate_local_dust_column_mcs.c
 * \date        05/2019
...
static dust_column_search_data *searchdata;
      ...
      if(out->DustColumn >= 0)
        {
          ...
          searchdata[i].DustColumn  = out->DustColumn;
        }
    }
  else /* combine */
    {
...
static int dust_column_primary_cell_evaluate(int target, int mode, int threadid);
static int n;
...
#pragma omp atomic capture
        ...
        if(searchdata[i].Task == -1)
          dust_column_primary_cell_evaluate(i, MODE_LOCAL_PARTICLES, threadid);
      }
  }
}
...
#pragma omp atomic capture
        ...
        dust_column_primary_cell_evaluate(i, MODE_IMPORTED_PARTICLES, threadid);
      }
  }
}

/*! \brief Estimates local column density (Jeans)
 *
...
void estimate_dust_column(dust_column_search_data *searchdata_input, int nn, int verbose)
{
  ...
  /* Convert from DGR_MW UnitMass UnitLength^-2 to DGR_MW cm^-2 */
  double factor = sqrt(GAMMA * M_PI / All.G) * HYDROGEN_MASSFRAC * All.UnitMass_in_g * All.HubbleParam /
                  (All.UnitLength_in_cm * All.UnitLength_in_cm * PROTONMASS);
  for(i = 0; i < n; i++)
    searchdata[i].DustColumn *= factor;
}
  ...
  for(n = 0; n < numngb; n++)
    {
      ...
      if(r < distmax && r < h && P[j].ID != 0 && P[j].Mass > 0)
        {
          distmax = r;
          dgr     = get_DGR_relative_solar(SphP[j].Metallicity);
          dustcol = dgr * sqrt(SphP[j].Pressure * All.cf_a3inv);
        }
    }

  out.Distance   = distmax;
  out.Task       = ThisTask;
  out.DustColumn = dustcol;
