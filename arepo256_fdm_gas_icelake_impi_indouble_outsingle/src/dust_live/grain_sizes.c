...
double interval_overlap(double a_1, double b_1, double a_2, double b_2) { return fmax(0.0, fmin(b_1, b_2) - fmax(a_1, a_2)); }
...
double x_1_ij(int i, int j, double adot, double dt)
{
  ...
  if(j == -1)
    ...
    {
      return fmax(GSD.Edges[i], GSD.Edges[DL_GRAIN_BINS] - adot * dt);
    }
  else
    {
      return fmax(GSD.Edges[i], GSD.Edges[j] - adot * dt);
    }
}
...
double x_2_ij(int i, int j, double adot, double dt)
{
  ...
  if(j == -1)
    {
      return fmin(GSD.Edges[i + 1], GSD.Edges[0] - adot * dt);
    }
  else if(j == DL_GRAIN_BINS)
    {
    ...
    {
      return fmin(GSD.Edges[i + 1], GSD.Edges[j + 1] - adot * dt);
    }
}

double ind_overlap(double x_1, double x_2) { return (x_2 >= x_1) ? 1.0 : 0.0; }
...
double bin_slope_from_avg_a(int j, double avg_a, double num_grains)
{
  ...
  double denom = (pow(GSD.Edges[j + 1], 3.0) - pow(GSD.Edges[j], 3.0)) / 3.0 -
                 GSD.Midpoints[j] * (pow(GSD.Edges[j + 1], 2.0) - pow(GSD.Edges[j], 2.0)) / 2.0;
  return num / denom;
}
...
{
  double num =
      bin_mass - num_grains * M_PI * GSD.InternalDensity / 3.0 * (pow(GSD.Edges[j + 1], 4.0) - pow(GSD.Edges[j], 4.0)) / GSD.Widths[j];
  double denom = (pow(GSD.Edges[j + 1], 5.0) - pow(GSD.Edges[j], 5.0)) / 5.0 -
                 GSD.Midpoints[j] * (pow(GSD.Edges[j + 1], 4.0) - pow(GSD.Edges[j], 4.0)) / 4.0;
...
void slope_limit_bin(int k, int j, double bin_mass)
{
  ...
  d = (pow(GSD.Edges[j + 1], 5.0) - pow(GSD.Edges[j], 5.0)) / 5.0 -
      GSD.Midpoints[j] * (pow(GSD.Edges[j + 1], 4.0) - pow(GSD.Edges[j], 4.0)) / 4.0;
...
void rebin_boundary_mass(int k, int j, double boundary_mass, double bin_mass)
{
  ...
  b = (pow(GSD.Edges[j + 1], 5.0) - pow(GSD.Edges[j], 5.0)) / 5.0 -
      GSD.Midpoints[j] * (pow(GSD.Edges[j + 1], 4.0) - pow(GSD.Edges[j], 4.0)) / 4.0;
  b *= 4.0 * M_PI * GSD.InternalDensity / 3.0;

  d = (pow(GSD.Edges[j + 1], 3.0) - pow(GSD.Edges[j], 3.0)) / 3.0 -
      GSD.Midpoints[j] * (pow(GSD.Edges[j + 1], 2.0) - pow(GSD.Edges[j], 2.0)) / 2.0;
...
#else
void rebin_boundary_mass(int k, int j, double boundary_mass) { DustParticle[k].NewNumGrains[j] += boundary_mass / GSD.AvgMasses[j]; }
#endif

#if defined(DL_GROWTH) || defined(DL_SPUTTERING)
/* Evolve the grain size distribution for local dust particle with DustParticle
 * index k, rate of radius growth adot, timestep dt in Gyr and dt_code in code
 * units.  Return the most restrictive grain size evolution timescale in code
 * units. */
double update_grain_sizes(int k, double adot, double dt, double dt_code)
{
  ...
  double tau_gsd = MAX_REAL_NUMBER;
  for(int j = 0; j < DL_GRAIN_BINS; j++)
    {
#ifdef DL_GRAIN_BINS_PIECEWISE_LINEAR
      ...
      if((delta_M_j != 0.0) && (old_M > 0.0))
        {
          ...
          tau_gsd    = fmin(tau_gsd, tau);
        }
    }

  /* The updated quantities at the end of the timestep become the initial
   * quantities for a new timestep, which may be relevant for subcycling. */
  memcpy(DTP(p).NumGrains, DustParticle[k].NewNumGrains, DL_GRAIN_BINS * sizeof(MyFloat));
#ifdef DL_GRAIN_BINS_PIECEWISE_LINEAR
  memcpy(DTP(p).BinSlopes, DustParticle[k].NewBinSlopes, DL_GRAIN_BINS * sizeof(MyFloat));
#endif

  DTP(p).BinMassChgTau = fmin(DTP(p).BinMassChgTau, tau_gsd);
  return tau_gsd;
}
#endif

#ifdef DL_SNE_DESTRUCTION
/* Perform subgrid supernova destruction for the grain size distribution of
 * local DustParticle with index k over timestep dt in Gyr and dt_code in code
 * units.  Return the most restrictive grain size evolution timescale in code
 * units. */
  ...
  for(int j = 0; j < DL_GRAIN_BINS; j++)
    {
      ...
      for(int i = 0; i < DL_GRAIN_BINS; i++)
        {
          dMj += num_grains[i] * GSD.XiFrac[j][i] * 4.0 * M_PI / 3.0 * GSD.InternalDensity *
                 (pow(GSD.Edges[j + 1], 4) - pow(GSD.Edges[j], 4)) / 4.0;
        }
      dMj -= bin_masses[j];
...
double I_1_ik(int p, int i, int k)
{
  ...
  double res =
      -((ak - akp1) *
        (-(pow(ai, 4) *
           (360 * pow(ai, 3) * si * (2 * Nk + (ak - 2 * akc + akp1) * sk) +
            105 * (Ni - aic * si) *
                (3 * pow(ak, 3) * sk + pow(ak, 2) * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk) +
                 ak * akp1 * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk) + pow(akp1, 2) * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk)) +
            140 * pow(ai, 2) *
                (3 * Ni * (2 * Nk + (ak - 2 * akc + akp1) * sk) +
                 si * (6 * ak * Nk + 6 * akp1 * Nk + 4 * pow(ak, 2) * sk - 6 * ak * akc * sk + 4 * ak * akp1 * sk -
                       6 * akc * akp1 * sk + 4 * pow(akp1, 2) * sk - 3 * aic * (2 * Nk + (ak - 2 * akc + akp1) * sk))) +
            84 * ai *
                (3 * pow(ak, 3) * si * sk + pow(ak, 2) * (4 * Nk * si + (8 * Ni - 8 * aic * si - 4 * akc * si + 3 * akp1 * si) * sk) +
                 ak * (4 * Ni * (3 * Nk - 3 * akc * sk + 2 * akp1 * sk) +
                       si * (-4 * aic * (3 * Nk - 3 * akc * sk + 2 * akp1 * sk) + akp1 * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk))) +
                 akp1 * (4 * Ni * (3 * Nk - 3 * akc * sk + 2 * akp1 * sk) + si * (-4 * aic * (3 * Nk - 3 * akc * sk + 2 * akp1 * sk) +
                                                                                  akp1 * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk)))))) +
         pow(aip1, 4) *
             (360 * pow(aip1, 3) * si * (2 * Nk + (ak - 2 * akc + akp1) * sk) +
              105 * (Ni - aic * si) *
                  (3 * pow(ak, 3) * sk + pow(ak, 2) * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk) +
                   ak * akp1 * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk) + pow(akp1, 2) * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk)) +
              140 * pow(aip1, 2) *
              ...
              84 * aip1 *
                  (3 * pow(ak, 3) * si * sk +
                   pow(ak, 2) * (4 * Nk * si + (8 * Ni - 8 * aic * si - 4 * akc * si + 3 * akp1 * si) * sk) +
                   ak * (4 * Ni * (3 * Nk - 3 * akc * sk + 2 * akp1 * sk) +
                         si * (-4 * aic * (3 * Nk - 3 * akc * sk + 2 * akp1 * sk) + akp1 * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk))) +
                   akp1 *
                       (4 * Ni * (3 * Nk - 3 * akc * sk + 2 * akp1 * sk) + si * (-4 * aic * (3 * Nk - 3 * akc * sk + 2 * akp1 * sk) +
                                                                                 akp1 * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk))))))) /
      5040.;
...
double I_2_kj(int p, int k, int j)
{
  ...
  double res =
      (9 * pow(aj, 4) * (ak - akp1) * sj * (2 * Nk + (ak - 2 * akc + akp1) * sk) +
       6 * aj * (ak - akp1) * (Nj - ajc * sj) *
           (3 * pow(ak, 3) * sk + pow(ak, 2) * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk) +
            ak * akp1 * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk) + pow(akp1, 2) * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk)) +
       4 * pow(aj, 3) * (ak - akp1) *
           (3 * Nj * (2 * Nk + (ak - 2 * akc + akp1) * sk) +
            sj * (6 * ak * Nk + 6 * akp1 * Nk + 4 * pow(ak, 2) * sk - 6 * ak * akc * sk + 4 * ak * akp1 * sk - 6 * akc * akp1 * sk +
                  4 * pow(akp1, 2) * sk - 3 * ajc * (2 * Nk + (ak - 2 * akc + akp1) * sk))) +
       3 * pow(aj, 2) *
           (3 * pow(ak, 4) * sj * sk + 12 * pow(ak, 2) * (Nj - ajc * sj) * (Nk - akc * sk) +
            4 * pow(ak, 3) * (Nk * sj + (2 * Nj - (2 * ajc + akc) * sj) * sk) +
            pow(akp1, 2) *
                (-4 * Nj * (3 * Nk - 3 * akc * sk + 2 * akp1 * sk) +
                 sj * (akp1 * (-4 * Nk + 4 * akc * sk - 3 * akp1 * sk) + 4 * ajc * (3 * Nk - 3 * akc * sk + 2 * akp1 * sk)))) -
       ajp1 * (9 * pow(ajp1, 3) * (ak - akp1) * sj * (2 * Nk + (ak - 2 * akc + akp1) * sk) +
               6 * (ak - akp1) * (Nj - ajc * sj) *
                   (3 * pow(ak, 3) * sk + pow(ak, 2) * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk) +
                    ak * akp1 * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk) + pow(akp1, 2) * (4 * Nk - 4 * akc * sk + 3 * akp1 * sk)) +
               4 * pow(ajp1, 2) * (ak - akp1) *
                   (3 * Nj * (2 * Nk + (ak - 2 * akc + akp1) * sk) +
                    sj * (6 * ak * Nk + 6 * akp1 * Nk + 4 * pow(ak, 2) * sk - 6 * ak * akc * sk + 4 * ak * akp1 * sk -
                          6 * akc * akp1 * sk + 4 * pow(akp1, 2) * sk - 3 * ajc * (2 * Nk + (ak - 2 * akc + akp1) * sk))) +
               3 * ajp1 *
                   (3 * pow(ak, 4) * sj * sk + 12 * pow(ak, 2) * (Nj - ajc * sj) * (Nk - akc * sk) +
                    4 * pow(ak, 3) * (Nk * sj + (2 * Nj - (2 * ajc + akc) * sj) * sk) +
                    pow(akp1, 2) * (-4 * Nj * (3 * Nk - 3 * akc * sk + 2 * akp1 * sk) +
                                    sj * (akp1 * (-4 * Nk + 4 * akc * sk - 3 * akp1 * sk) +
                                          4 * ajc * (3 * Nk - 3 * akc * sk + 2 * akp1 * sk)))))) /
      72.;
  return res; /* um^2 */
#else
...
double sigma_helper(double M, double s_fac) { return 0.30 * pow(s_fac + 1.0 / M - 0.11, 0.13) / (s_fac + 1.0 / M - 1.0); }
...
void correct_fragment_range(double *af_min, double *af_max)
{
  ...
  double new_max = fmin(*af_max, GSD.Edges[DL_GRAIN_BINS]);
  double new_min = fmax(*af_min, GSD.Edges[0]);
...
void m_shat_ikj(int p, int k, int j, double v_kj, double *delta_bins, double norm_fac)
{
  ...
  double M_shock = GSD.AvgMasses[j] * (1.0 + 2.0 * R_script) / pow(1.0 + R_script, 9.0 / 16.0) *
                   pow(M_r * M_r / (sigma_1 * M_1 * M_1), 8.0 / 9.0) / pow(sigma_1i, 1.0 / 9.0); /* internal mass */
  ...
  v_cat  = c_0_int * pow(GSD.AvgMasses[k] / ((1.0 + 2.0 * R_script) * GSD.AvgMasses[j]), 9.0 / 16.0) * sqrt(sigma_1) *
          pow(sigma_1i, 1.0 / 16.0) * (1.0 + R_script) * M_1; /* um/s */
  ...
  for(int i = 0; i < DL_GRAIN_BINS; i++)
    {
      ...
      double lim2 = fmin(af_max, GSD.Edges[i + 1]); /* um */
      double lim1 = fmax(af_min, GSD.Edges[i]);     /* um */
  ...
  if(M_shock > 0.5 * GSD.AvgMasses[k])
    {
      ...
      v_cat = c_0_int * pow(GSD.AvgMasses[j] / ((1.0 + 2.0 * R_script) * GSD.AvgMasses[k]), 9.0 / 16.0) * sqrt(sigma_1) *
              pow(sigma_1i, 1.0 / 16.0) * (1.0 + R_script) * M_1; /* um/s */
  ...
  else
    {
      ...
      double z_fac = 3.4;
      double af_max3 =
          (M_ej / GSD.InternalDensity) * 3.0 / (16.0 * M_PI) * (z_fac + 1.0) / (z_fac * z_fac * z_fac * (z_fac - 2.0)); /* um^3 */
...
void bin_state_post_shattering(int l, int i, double old_mass, double delta_mass)
{
  ...
  b = (pow(GSD.Edges[i + 1], 5.0) - pow(GSD.Edges[i], 5.0)) / 5.0 -
      GSD.Midpoints[i] * (pow(GSD.Edges[i + 1], 4.0) - pow(GSD.Edges[i], 4.0)) / 4.0;
  b *= 4.0 * M_PI * GSD.InternalDensity / 3.0;

  d = (pow(GSD.Edges[i + 1], 3.0) - pow(GSD.Edges[i], 3.0)) / 3.0 -
      GSD.Midpoints[i] * (pow(GSD.Edges[i + 1], 2.0) - pow(GSD.Edges[i], 2.0)) / 2.0;

  double old_atot = DTP(p).NumGrains[i] * c_fac + DTP(p).BinSlopes[i] * d;
  double new_a;
  if(delta_mass > 0.0)
    {
      double a_shat_i = 2.3 / 1.3 * (pow(GSD.Edges[i + 1], -1.3) - pow(GSD.Edges[i], -1.3)) /
                        (pow(GSD.Edges[i + 1], -2.3) - pow(GSD.Edges[i], -2.3));
      double m_shat_i = 4.0 * M_PI * GSD.InternalDensity / 3.0 * -2.3 / 0.7 * (pow(GSD.Edges[i + 1], 0.7) - pow(GSD.Edges[i], 0.7)) /
                        (pow(GSD.Edges[i + 1], -2.3) - pow(GSD.Edges[i], -2.3));
 ...
 * local index k over time-step dt in Gyr (dt_code in code units).  Here, type
 * must be one of GSD_SHATTERING or GSD_COAGULATION.  Return the most
 * restrictive bin timescale in code units. */
...
#else
          double dMi_dt = -M_PI * v_ik * ind * pow(GSD.Midpoints[i] + GSD.Midpoints[k], 2.0) * DTP(p).NumGrains[i] *
                          DTP(p).NumGrains[k] * GSD.AvgMasses[i] / V_d_um3; /* internal mass / s */
#endif
...
#else
          double norm_fac = M_PI * v_kj * ind * pow(GSD.Midpoints[k] + GSD.Midpoints[j], 2.0) * DTP(p).NumGrains[k] *
                            DTP(p).NumGrains[j] / V_d_um3 * dt_s / 2.0; /* dimensionless */
#endif
#ifdef DL_SHATTERING
          if(type == GSD_SHATTERING)
            {
