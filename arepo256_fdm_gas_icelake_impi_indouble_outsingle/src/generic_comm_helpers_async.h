...
static void generic_set_info(const char *func, const char *file, int line)
{
  ...
  mpi_printf("GENERIC-ASYNC: function %s(), file %s, line %d: MinSpace = %g MB  NTopleaves = %d  ExportSpace = %g MB\n", func, file,
             line, MinSpace * TO_MBYTE_FAC, NTopleaves, ExportSpace * TO_MBYTE_FAC);
#endif
...
static void generic_polling_secondary(void) { generic_polling(); }
  ...
  if(flag)
    {
      ...
      if(generic_calc_import_storage(Nimport, NimportNodes) > FreeBytes())
        terminate("Sorry, we have a serious problem: %g MB are free but we need %g for this import", FreeBytes() * TO_MBYTE_FAC,
                  generic_calc_import_storage(Nimport, NimportNodes) * TO_MBYTE_FAC);
      ...
      MPI_Issend(DataResult, Recv[source].Count * sizeof(data_out), MPI_BYTE, source, TAG_RESULTS + MyTagOffset, MYCOMMUNICATOR,
                 &result_send_requests[n_result_send_requests++]);
    }
  return flag;
}
  ...
  while(1)
    {
      ...
      if(Thread[0].PartListOld)
        {
          ...
          /* explicitly mark the data block buffers as unallocated (just for safety so that we are guaranteed not to use them by
           * accident) */
      ...
      for(int ngrp = 0; ngrp < (1 << PTask); ngrp++)
        {
          ...
          if(j < MyNTask)
            if(Send[j].Count > 0)
              {
                MPI_Issend(&Send[j], sizeof(struct send_recv_counts), MPI_BYTE, j, TAG_N + MyTagOffset, MYCOMMUNICATOR,
                           &requests[n_requests++]);
                MPI_Issend(&DataIn[Send_offset[j]], Send[j].Count * sizeof(data_in), MPI_BYTE, j, TAG_PART_DATA + MyTagOffset,
                           MYCOMMUNICATOR, &requests[n_requests++]);
                MPI_Issend(&NodeDataIn[Send_offset_nodes[j]], Send[j].CountNodes, MPI_INT, j, TAG_NODE_DATA + MyTagOffset,
                           MYCOMMUNICATOR, &requests[n_requests++]);
                MPI_Irecv(&DataOut[Send_offset[j]], Send[j].Count * sizeof(data_out), MPI_BYTE, j, TAG_RESULTS + MyTagOffset,
                          MYCOMMUNICATOR, &result_recv_requests[n_result_recv_requests++]);
              }
        }
                            ...
                            if((mask & received_levels) == mask)
                              {
                                ...
                                MPI_Issend(&stagelist[stage], 1, MPI_INT, target, TAG_BARRIER + MyTagOffset, MYCOMMUNICATOR,
                                           &level_requests[stage]);
                              }
                          }
                    }
                  break;
      ...
      if(received_levels == ((1 << nLevels) - 1) && sent_levels == ((1 << nLevels) - 1))
        {
          /* make sure that all our barrier sends have really been dealt with and have been picked up (note that we are going to free
           * stagelist) */
          int flag;
