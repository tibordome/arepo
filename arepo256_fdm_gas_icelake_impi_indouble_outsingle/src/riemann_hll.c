 ...
 * \date        MM/YYYY
 * \author
 * \brief       Routines for a HLL Riemann solver.
 * \details
 *
 *
...
#endif /* #if (defined(RIEMANN_HLLC) && !defined(DG)) || defined(RIEMANN_HLLD) || defined(RIEMANN_ROSUNOV) || defined(RIEMANN_GAMMA) \
        */
...
/*! \brief Calculates the central state in HLL approximation.
 *
 *  This means only one central state and no contact discontinuity present.
 *
 *  \param[in] st_L Left state of the Riemann problem.
 *  \param[in] st_R Right state of the Riemann problem.
 *  \param[out] st_face State at the face (determined by this routine).
 *  \param[in] flux_L Flux through face from left.
 *  \param[in] flux_R Flux through face from right.
 *  \param[in] S_L speed of characteristics from left state.
 *  \param[in] S_R speed of characteristics from right state.
 *
 *  \return void
 */
static void face_get_hll_state(const struct state *st_L, const struct state *st_R, struct state_face *st_face,
                               const struct fluxes *flux_L, const struct fluxes *flux_R, double S_L, double S_R)
{
  if(S_L >= 0.0)
    {
  ...
  else
    {
      ...
      st_face->velx = fac * (flux_L->momentum[0] - flux_R->momentum[0] - S_L * st_L->rho * st_L->velx + S_R * st_R->rho * st_R->velx) /
                      st_face->rho;
      st_face->vely = fac * (flux_L->momentum[1] - flux_R->momentum[1] - S_L * st_L->rho * st_L->vely + S_R * st_R->rho * st_R->vely) /
                      st_face->rho;
      st_face->velz = fac * (flux_L->momentum[2] - flux_R->momentum[2] - S_L * st_L->rho * st_L->velz + S_R * st_R->rho * st_R->velz) /
                      st_face->rho;
...
/*! \brief Calculates hll flux from fluxes and states at both sides of a face.
 *
 *  \param[in] st_L Left state of the Riemann problem.
 *  \param[in] st_R Right state of the Riemann problem.
 *  \param[in] flux_L Flux through face from left.
 *  \param[in] flux_R Flux through face from right.
 *  \param[out] hll_flux Net flux at faces (determined by this routine).
 *  \param[in] S_L speed of characteristics from left state.
 *  \param[in] S_R speed of characteristics from right state.
 *
 *  \return void
 */
static void get_hll_fluxes(const struct state *st_L, const struct state *st_R, const struct fluxes *flux_L,
                           const struct fluxes *flux_R, struct fluxes *hll_flux, double S_L, double S_R)
{
  /* upwinding */
  S_L = fmin(S_L, 0.0);
  S_R = fmax(S_R, 0.0);
...
/*! \brief Main routine for the hll Riemann solver.
 *
  ...
  if(st_L->rho > 0 && st_R->rho > 0)
    {
      ...
      S_L = fmin(st_L->velx - st_L->csnd, st_R->velx - st_R->csnd);
      S_R = fmax(st_L->velx + st_L->csnd, st_R->velx + st_R->csnd);
...
#endif /* #if defined(RIEMANN_HLL) || (defined(RIEMANN_HLLC) && defined(DG)) */
