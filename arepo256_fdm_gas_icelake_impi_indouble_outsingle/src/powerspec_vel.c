...
void powerspec_vel(int RestartSnapNum)
{
  ...
  mpi_printf("Center: %g, %g, %g\n", All.PSCenterX, All.PSCenterY, All.PSCenterZ);
#endif
  ...
  myplan.forward_plan_zdir = FFTW(plan_many_dft_r2c)(1, ndim, 1, densityfield, 0, 1, PMGRID2, (fft_complex *)workspace, 0, 1, PMGRIDz,
                                                     FFTW_ESTIMATE | FFTW_DESTROY_INPUT | alignflag);

  myplan.forward_plan_ydir =
      FFTW(plan_many_dft)(1, ndim, 1, (fft_complex *)densityfield, 0, stride, PMGRIDz * PMGRID, (fft_complex *)workspace, 0, stride,
                          PMGRIDz * PMGRID, FFTW_FORWARD, FFTW_ESTIMATE | FFTW_DESTROY_INPUT | alignflag);
...
void powerspec_vel_collect_and_save(struct powerspectrum *ps, char *name, int RestartSnapNum)
{
  ...
  MPI_Allgather(ps->CountModes, BINS_PS * sizeof(long long), MPI_BYTE, countbuf, BINS_PS * sizeof(long long), MPI_BYTE,
                MPI_COMM_WORLD);
  ...
  if(ThisTask == 0)
    {
      ...
      file_path_sprintf(fname, "%s/powerspec_%s_%03d.txt", All.OutputDir, name, RestartSnapNum);
      if(!(fd = fopen(fname, "w")))
...
double powerspec_vel_obtain_fields(fft_plan *myplan)
{
...
#ifdef VEL_POWERSPEC_BOX
  mpi_printf("Limiting to %g<radius<%g (code units), %g<radius<%g (physical units).\n", All.PSMinRadius, All.PSRadius,
             All.PSMinRadius * All.cf_atime / All.HubbleParam, All.PSRadius * All.cf_atime / All.HubbleParam);
#endif
  ...
  for(int n = 0; n < NumGas; n++)
    {
      ...
      if(r2 > All.PSMinRadius * All.PSMinRadius && r2 < All.PSRadius * All.PSRadius)
        {
          gasCount++;
  ...
  mpi_printf("Total kinetic energy in radius: %g (code units), %g (physical units).\n", eKinAll,
             eKinAll * All.cf_a2inv / All.HubbleParam);
  ...
  mpi_printf("Total magnetic energy in radius: %g (code units), %g (physical units).\n", eMagAll,
             eMagAll / All.cf_atime / All.HubbleParam);
#endif
  ...
  do
    {
      ...
      for(large_array_offset i = 0; i < Ncount; i++)
        {
          if(powerspec_vel_nearest_distance[i] > 1.0e29)
            {
            ...
            {
              powerspec_vel_nearest_distance[i] = 0; /* we do not continue to search for this particle */
            }
        }
  ...
  mpi_printf("Total kinetic energy on grid: %g (code units), %g (physical units).\n", ekin_all,
             ekin_all * All.cf_a2inv / All.HubbleParam);
  ...
  mpi_printf("Total magnetic energy on grid: %g (code units), %g (physical units).\n", emag_all,
             emag_all / All.cf_atime / All.HubbleParam);
#endif
