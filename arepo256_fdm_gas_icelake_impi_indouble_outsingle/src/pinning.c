...
void detect_topology(void)
{
  ...
  if(depth == (unsigned)HWLOC_TYPE_DEPTH_UNKNOWN)
    sockets = -1;
  else
...
void pin_to_core_set(void)
{
  ...
  if(num_threads != NUM_THREADS)
    {
      terminate(
          "You have activated NUM_THREADS, but the value of the environment variable OMP_NUM_THREADS=%d is not equal to "
          "NUM_THREADS=%d!",
  ...
  if(available_pus == pus)
  ...
  else
    {
      if(available_pus >= 1)
        {
          mpi_printf("PINNING: Looks like a tight binding was already imposed before start of the code.\n");
...
#else
          mpi_printf(
              "PINNING: We refrain from any pinning attempt ourselves. (This can be changed by setting "
