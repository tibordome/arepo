...
in the neighboring cells*/
{
  ...
  for(k = 0; k < N_Hess; k++)
    {
      hessian_elements[k].min_value = (double *)mymalloc("gradmin", 3 * NumGas * sizeof(double));
      hessian_elements[k].max_value = (double *)mymalloc("gradmax", 3 * NumGas * sizeof(double));
    }
#endif
...
#else
                        hessian_elements[k].gradient0[l] = hessian_elements[k].gradient1[l] + hessian_elements[k].gradient1[0] * n[0] +
                                                           hessian_elements[k].gradient1[1] * n[1] +
                                                           hessian_elements[k].gradient1[2] * n[2];
#endif
                      }
                }
            }
                    ...
                    for(l = 0; l < 3; l++) /*no-slip condition. Extrapolate all velocity gradients */
                      {
                        ...
                        hessian_elements[k].gradient1[l] = hessian_elements[k].gradient0[l] + hessian_elements[k].gradient0[0] * n[0] +
                                                           hessian_elements[k].gradient0[1] * n[1] +
                                                           hessian_elements[k].gradient0[2] * n[2];
#endif
                      }
                }
            }
                  ...
                  for(l = 0; l < 3; l++)
                    {
                      for(j = l; j < 3; j++)
                        {
                          direct = fac0 * n[j] * (hessian_elements[k].gradient0[l] + hessian_elements[k].gradient1[l]) +
                                   facA * c[j] * (hessian_elements[k].gradient0[l] - hessian_elements[k].gradient1[l]);

                          transpose = fac0 * n[l] * (hessian_elements[k].gradient0[j] + hessian_elements[k].gradient1[j]) +
                                      facA * c[l] * (hessian_elements[k].gradient0[j] - hessian_elements[k].gradient1[j]);
                  ...
                  for(l = 0; l < 3; l++)
                    {
                      for(j = l; j < 3; j++)
                        {
                          direct = fac1 * n[j] * (hessian_elements[k].gradient1[l] + hessian_elements[k].gradient0[l]) +
                                   facB * c[j] * (hessian_elements[k].gradient1[l] - hessian_elements[k].gradient0[l]);

                          transpose = fac1 * n[l] * (hessian_elements[k].gradient1[j] + hessian_elements[k].gradient0[j]) +
                                      facB * c[l] * (hessian_elements[k].gradient1[j] - hessian_elements[k].gradient0[j]);
                          ...
                          for(l = 0; l < 3; l++)
                            {
                              ...
                              limit_gradient(d, value, hessian_elements[k].min_value[3 * q + l],
                                             hessian_elements[k].max_value[3 * q + l], data);
                            }
                        }
                    }
                }
