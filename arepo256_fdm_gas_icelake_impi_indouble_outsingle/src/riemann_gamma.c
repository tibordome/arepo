...
#endif /* #if defined(RIEMANN_HLLC) || defined(RIEMANN_HLLD) || defined(RIEMANN_ROSUNOV) || defined(RIEMANN_HLL) */

#define TOL 1.0e-8
#define SMALLP 1.0e-20

/*! \brief Calculates the state at the face from Riemann problem.
 *
 *  \param[in] st_L Left hand side state.
 *  \param[in] st_R Right hand side state.
 *  \param[out] st_face State at the face
 *
 *  \return void
 */
...
/*! \brief Calculates Mach numbers of shocks of Riemann problem.
 *
 ...
 *  \param[in] Mach_L (unused)
 *  \param[in] Mach_R (unused)
 *
 *  \return void
 */
...
#endif /* #if (defined ENTROPY_MACH_THRESHOLD) || (defined GODUNOV_STATS) */
}
 ...
 *  \param[in] st_L Left hand side state.
 *  \param[in] st_R Right hand side state.
 *  \param[in] Press_star Pressure in central region.
 *  \paramm[in] Vel_star Velocity in central region.
                              ...
                              double W_R, struct state_face *st_face)
{
  ...
  if(We < 0.)
    ...
    {
      fac1 = 0.5 * (1. + (We + We_star) / fmax(fmax(We - We_star, We + We_star), 1e-5));
      fac2 = 1. - fac1;
  ...
  if(!gsl_finite(st_face->gammaE))
    printf("GammaE=%g, GammaE_star=%g, fac1=%g, fac2=%g, We=%g, We_star=%g, W_L=%g, W_R=%g, Rho=%g\n", GammaE, GammaE_star, fac1, fac2,
           We, We_star, W_L, W_R, Rho);
}

/*! \brief Riemann solver, i.e. gives approximate solution for central
 *         pressure.
 *
 *  Iterative solver with Newton-Raphson root-finding.
 *
 *  \param[in] st_L Left hand side state.
 *  \param[in] st_R Right hand side state.
 *  \param[out] Press Central pressure.
 *  \param[out] Vel Central velocity.
 *  \param[out] W_L ToDo: what is this?
 *  \param[out] W_R ToDo: what is this?
 *
 *  \return 1
 */
int riemann_gamma(struct state *st_L, struct state *st_R, double *Press, double *Vel, double *W_L, double *W_R)
{
  ...
  double GammaEmin = fmin(st_L->gammaE, st_R->gammaE);
  double GammaEmax = fmax(st_L->gammaE, st_R->gammaE);
  ...
  pold = fmax(
      st_L->press + (st_R->press - st_L->press - st_R->csnd * (st_R->velx - st_L->velx)) * (st_L->csnd / (st_L->csnd + st_R->csnd)),
      SMALLP);
  ...
  p     = fmax(st_L->press + (st_R->press - st_L->press - *W_R * (st_R->velx - st_L->velx)) * (*W_L / (*W_L + *W_R)), SMALLP);
  pnew  = p;
  ...
  do /* newton-raphson scheme */
    {
      ...
      pnew = fmax(p - udiff * (p - pold) / (udiff - udiffold), SMALLP);
      pold = p;
      p    = pnew;
  ...
  *Vel   = 0.5 * ((st_L->velx - (p - st_L->press) / (*W_L)) + (st_R->velx + (p - st_R->press) / (*W_R)));
  ...
  if(!gsl_finite(*W_L) || !gsl_finite(*W_R))
    printf(
        "W_L=%g, W_R=%g, Press=%g, pold=%g, Vel=%g, critVel=%g, v_L=%g, v_R=%g, P_L=%g, P_R=%g, csnd=%g|%g, GammaC=%g|%g, "
        "Gamma=%g|%g|%g\n",
        *W_L, *W_R, *Press, pold, *Vel, critVel, st_L->velx, st_R->velx, st_L->press, st_R->press, st_L->csnd, st_R->csnd,
        st_L->gammaC, st_R->gammaC, GammaFac, GammaEmin, GammaEmax);
...
/*! \brief Calculates W factors for riemann_gamma.
 *
 *  \param[in] Pstar Central pressure
 *  \param[in] GammaFac ToDo: what is this?
 *  \param[in] st State
 *  \param[in] GammaEmin ToDo: what is this?
 *  \param[out] W W factor.
 *
 *  \return void
 */
void calcW(double Pstar, double GammaFac, struct state *st, double GammaEmin, double GammaEmax, double *W)
{
  ...
  gammaE_star = fmax(GammaEmin, fmin(GammaEmax, gammaE_star));
  ...
  if(fabs(pdiff / psum) < 1e-20)
    ...
    {
      *W = sqrt(pdiff * (Pstar + 0.5 * (gammaE_star - 1.0) * psum) * st->rho /
                (Pstar - (gammaE_star - 1.0) / (st->gammaE - 1.0) * st->press));
    }
  *W = fmax(*W, sqgame * st->csnd);
}

#endif /* #ifdef RIEMANN_GAMMA */
