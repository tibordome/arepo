 ...
 *
 * \file        src/mhd_nonideal/implicit_ohmic_diffusion_ct_cn.c
 * \date        10/2017
...
/*
 * ! \file implicit_ohmic_diffusion_ct_cn.c
  ...
  for(i = 0; i < NumGas; i++)
    {
      ...
      dEmag[i] = -0.5 * (SphP[i].BConserved[0] * SphP[i].BConserved[0] + SphP[i].BConserved[1] * SphP[i].BConserved[1] +
...
void set_coeff(HYPRE_IJMatrix *A, HYPRE_IJVector *x, HYPRE_IJVector *b, int *fld_offsets, double dt, int direction)
{
  ...
  int i;
  double *aval = (double *)mymalloc("a", sizeof(double) * NumGas);
  memset(aval, 0, NumGas * sizeof(double));
      ...
      while(q >= 0)
        {
          ...
          sum += dval;
          aval[i] -= 0.5 * dval * (apot - SphP[i].A[direction]);
          ;
  ...
  HYPRE_IJVectorSetValues(*b, NumGas, rows, aval);
