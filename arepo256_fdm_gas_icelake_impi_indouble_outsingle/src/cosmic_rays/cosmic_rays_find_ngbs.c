          ...
          if(Todo[i] > 0)
            {
              ...
              if(NgbCount[i] == 0)
                {
                  if(Todo[i] == 2)
                    terminate("In principle this should not happen. Iter=%d, particle=%d, Right=%g, Left=%g, Hsml=%g", iter, i,
                              Right[i], Left[i], P[i].Hsml);
              ...
              if(NumNgb[i] < (All.DesNumNgbCRInjection - All.MaxNumNgbDeviationCRInjection) ||
                 (NumNgb[i] > (All.DesNumNgbCRInjection + All.MaxNumNgbDeviationCRInjection)))
                {
                  /* need to redo this particle */
                  npleft++;

                  /* This means that the minimum Hsml has already been computed but it is still not converged (from now on the particle
                   * has at least 1 neighbour) */
                  Todo[i] = 2;
                  ...
                  if(iter >= MAXITER - 10)
                    {
                      printf("i=%d task=%d ID=%d Hsml=%g Left=%g Right=%g Ngbs=%g, NgbsCount=%d Right-Left=%g\n   pos=(%g|%g|%g)\n", i,
                             ThisTask, (int)P[i].ID, P[i].Hsml, Left[i], Right[i], (float)NumNgb[i], NgbCount[i], Right[i] - Left[i],
          ...
          if(r2 < h2)
            {
              ...
              mass_j = P[j].Mass;
              weighted_numngb +=
                  NORM_COEFF * mass_j * wk /
                  hinv3; /* 4.0/3 * PI = 4.188790204786, swallowed cells are not counted here, because they have mass_j=0 */
