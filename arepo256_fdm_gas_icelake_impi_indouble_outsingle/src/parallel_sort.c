...
static void check_local_rank(char *element, size_t tie_braking_rank, char *base, size_t nmemb, size_t size, size_t noffs_thistask,
                             long long left, long long right, size_t loc, int (*compar)(const void *, const void *));
#endif
  ...
  if(Local_NTask > 1 && Color == 1)
    {
      ...
      char *element_guess              = (char *)mymalloc("element_guess", Local_NTask * size);
      size_t *element_tie_braking_rank = (size_t *)mymalloc("element_tie_braking_rank", Local_NTask * sizeof(size_t));
      size_t *desired_glob_rank        = (size_t *)mymalloc("desired_glob_rank", Local_NTask * sizeof(size_t));
      size_t *current_glob_rank        = (size_t *)mymalloc("current_glob_rank", Local_NTask * sizeof(size_t));
      size_t *current_loc_rank         = (size_t *)mymalloc("current_loc_rank", Local_NTask * sizeof(size_t));
      long long *range_left            = (long long *)mymalloc("range_left", Local_NTask * sizeof(long long));
      long long *range_right           = (long long *)mymalloc("range_right", Local_NTask * sizeof(long long));
      int *max_loc                     = (int *)mymalloc("max_loc", Local_NTask * sizeof(int));

      size_t *list                         = (size_t *)mymalloc("list", Local_NTask * sizeof(size_t));
      size_t *range_len_list               = (size_t *)mynew("range_len_list", size_t, Local_NTask);
      char *median_element                 = (char *)mymalloc("median_element", size);
      median_element_list                  = (char *)mymalloc("median_element_list", Local_NTask * size);
      size_t *tie_braking_rank_list        = (size_t *)mymalloc("tie_braking_rank_list", Local_NTask * sizeof(size_t));
      int *index_list                      = (int *)mymalloc("index_list", Local_NTask * sizeof(int));
      int *max_loc_list                    = (int *)mymalloc("max_loc_list", Local_NTask * sizeof(int));
      size_t *source_range_len_list        = (size_t *)mynew("source_range_len_list", size_t, Local_NTask);
      size_t *source_tie_braking_rank_list = (size_t *)mynew("source_tie_braking_rank_list", size_t, Local_NTask);
      char *source_median_element_list     = (char *)mymalloc("source_median_element_list", Local_NTask * size);
      char *new_element_guess              = (char *)mymalloc("new_element_guess", size);
      ...
      do
        {
...
#ifdef CHECK_LOCAL_RANK
                  check_local_rank(element_guess + i * size, element_tie_braking_rank[i], (char *)base, nmemb, size,
                                   noffs[Local_ThisTask], range_left[i], range_right[i], current_loc_rank[i], compar);
#endif
                }
            }
          ...
          if(Local_ThisTask < Local_NTask - 1)
            {
              if(current_glob_rank[Local_ThisTask] != desired_glob_rank[Local_ThisTask])
                {
                  /* in this case we're not yet done for this split point */
                  for(j = 0; j < Local_NTask; j++)
                    max_loc_list[j] = j;
                  ...
                  if(iter & 1)
                    ...
                    {
                      /* do a serial sort of the remaining elements
                       * (indirectly, so that we have the order of tie braking list as well) */
                      comparfunc   = compar;
                      element_size = size;
                      for(j = 0; j < nleft; j++)
                        index_list[j] = j;
       ...
       * and the MPI data exchange though MPI_Alltoall has to be modified such that
       * buffers > 2 GB become possible */

      int *send_count  = (int *)mymalloc("send_count", Local_NTask * sizeof(int));
      int *recv_count  = (int *)mymalloc("recv_count", Local_NTask * sizeof(int));
      int *send_offset = (int *)mymalloc("send_offset", Local_NTask * sizeof(int));
      int *recv_offset = (int *)mymalloc("recv_offset", Local_NTask * sizeof(int));
                           ...
                           int (*compar)(const void *, const void *)) /* user-specified  comparison function */
{
  ...
  if(left == 0 && (size_t)right == nmemb + 1)
    {
...
void parallel_sort_test_order(char *base, size_t nmemb, size_t size, int (*compar)(const void *, const void *))
{
  ...
  recv = -1;
  for(int i = 0; i < ThisTask && nmemb > 0; i++)
    if(nlist[i] > 0)
      recv = i;

  send = -1;
  for(int i = ThisTask + 1; nmemb > 0 && i < NTask; i++)
    if(nlist[i] > 0)
      {
        send = i;
        break;
  ...
  char *element = (char *)mymalloc("element", size);
