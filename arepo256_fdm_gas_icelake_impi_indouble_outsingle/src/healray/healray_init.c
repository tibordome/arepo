...
void healray_alloc(void);
void healray_init_mesh(void);
void healray_init_variables(void);
void healray_init_emitters(void);
void healray_init_emitters_alt(void);
  ...
  if(HRD.RayNumTrace)
    {
      ...
      for(i = 0; i < HRD.RayNumThreads; i++)
        {
          HRD.TraceEnergyFrac[i] =
              mymalloc_movable(&HRD.TraceEnergyFrac[i], "HRD.TraceEnergyFrac[i]", HRD.TraceNumIter * HRD.RayNumTrace * sizeof(double));
        }
#ifdef TGCHEM
      if(!TGCD.ChemMode && HRD.RayMultiFreqMode)
        {
          HRD.TraceEnergyFracFreq =
              mymalloc_movable(&HRD.TraceEnergyFracFreq, "HRD.TraceEnergyFracFreq", HRD.RayNumThreads * sizeof(void *));

          for(i = 0; i < HRD.RayNumThreads; i++)
            HRD.TraceEnergyFracFreq[i] = mymalloc_movable(&HRD.TraceEnergyFracFreq[i], "HRD.TraceEnergyFracFreq[i]",
                                                          HRD.TraceNumIter * HRD.RayNumTrace * HRD.RayNumBins * sizeof(double));
        }
#endif
    }
...
void healray_init_emitters(void)
{
  ...
  HRD.RayEnergyLineInit =
      mymalloc_movable(&HRD.RayEnergyLineInit, "HRD.RayEnergyLineInit", HRD.MaxNumRays * HRD.RayNumLines * sizeof(double));
...
void healray_init_emitters_alt(void)
{
  ...
  HRD.RayEnergyLineInit =
      mymalloc_movable(&HRD.RayEnergyLineInit, "HRD.RayEnergyLineInit", HRD.LimitNumRays * HRD.RayNumLines * sizeof(double));
            ...
            if(!TGCD.ChemMode)
              {
                      ...
                      for(k = m = 0; k < HRD.RayNumLines; k++)
                        {
                          ...
                          for(l = 0; l < HRD.RayNumFreq; l++)
                            HRD.RayEnergyLineInit[HRD.NumRays * HRD.RayNumLines + k] +=
                                HRD.RayEnergy[HRD.NumRays * HRD.RayNumBins + m++];

                          printf("line emiss: %d %d %d %g %g\n", k, temp_idx, TGCD.H2LineIdxTable[k * TGCHEM_NUM_TEMP + temp_idx],
                                 TGCD.H2EmissTable[TGCD.H2LineIdxTable[k * TGCHEM_NUM_TEMP + temp_idx] * TGCHEM_NUM_TEMP + temp_idx],
                                 HRD.RayEnergyLineInit[HRD.NumRays * HRD.RayNumLines + k] / fac);
                        }
                  }
                ...
                if(HRD.RayDebugMode)
                  printf("cell: %g %g %g %g %g %g %g %g %d %g %g %g %g %g\n", nh, abh2, abhii, SphP[i].Volume, HRD.VolToCgs, fac, mu,
                         temp, temp_idx, dtemp, H2TotEmiss, tot_emiss, tot_emiss_test, HRD.HRRD[HRD.NumRays].energy);
              }
            else
...
void healray_find_intersection(double *l0, double *dir, double *sec)
{
  ...
  for(j = 0; j < 3; j++)
    sec[j] = fmin(fmax(sec[j], 0.), (1. - 1e-10) * All.BoxSize);
}
