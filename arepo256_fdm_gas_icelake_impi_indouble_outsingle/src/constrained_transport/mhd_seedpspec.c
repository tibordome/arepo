  ...
  for(i = 0; i < StNModes; i++)
    {
      ...
      kz   = StMode[3 * i + 2];
      k    = sqrt(kx * kx + ky * ky + kz * kz);
      ampl = 1.0 / sqrt(2.0 / M_PI) *
             sqrt(All.B_pspec_ampl / 3.0 * pow(k, All.B_pspec_slope - 2.0) * exp(-pow(k / All.B_pspec_kcut, 4))) * pow(Nk, 3) /
             (2.0 * M_PI);
  ...
  for(m = 0; m < StNModes; m++)
    {
      ...
      //}
      B_value[0] += (ky * StAhatIm[3 * m + 2] - kz * StAhatIm[3 * m + 1]) * realt +
                    (ky * StAhatRe[3 * m + 2] - kz * StAhatRe[3 * m + 1]) * imagt;
      B_value[1] += (kz * StAhatIm[3 * m + 0] - kx * StAhatIm[3 * m + 2]) * realt +
                    (kz * StAhatRe[3 * m + 0] - kx * StAhatRe[3 * m + 2]) * imagt;
      B_value[2] += (kx * StAhatIm[3 * m + 1] - ky * StAhatIm[3 * m + 0]) * realt +
                    (kx * StAhatRe[3 * m + 1] - ky * StAhatRe[3 * m + 0]) * imagt;
    }

  double bfac = 1. / (sqrt(All.UnitMass_in_g / All.UnitLength_in_cm) / (All.UnitTime_in_s / All.HubbleParam));
  for(j = 0; j < 3; j++)
    {
      SphP[i].BConserved[j] = (-2.0 * (2.0 * M_PI) / pow(Nk, 3)) * B_value[j] * SphP[i].Volume *
                              bfac;  // / sqrt(4. * M_PI);   /* convert Gauss-cgs to heavyside - lorentz done later!*/
      SphP[i].B[j] = SphP[i].BConserved[j] / SphP[i].Volume;
    }
}

void set_pscpec_ICs(void)
{
  init_pspec_bfld();
