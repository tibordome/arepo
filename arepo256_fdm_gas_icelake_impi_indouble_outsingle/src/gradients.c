...
void init_gradients(void)
{
...
#if defined(SPECIAL_RELATIVITY) || defined(GENERAL_RELATIVITY) || defined(DEREFINE_GENTLY) || defined(CONDUCTION_SATURATION) || \
...
#ifdef MHD_DEDNER
  gradient_init(&SphP[0].Psi, &PrimExch[0].Psi, SphP[0].Grad.dPsi, GRADIENT_TYPE_NORMAL);
#endif
  ...
  gradient_init(&SphP[0].n_elec, &PrimExch[0].n_elec, SphP[0].Grad.dn_elec, GRADIENT_TYPE_NORMAL);
#endif
#ifdef BIERMANN_BATTERY
  gradient_init(&SphP[0].p_elec, &PrimExch[0].p_elec, SphP[0].Grad.dp_elec, GRADIENT_TYPE_NORMAL);
#endif
#ifdef DURRIVE_BATTERY
  gradient_init(&SphP[0].pdot_elec[0], &PrimExch[0].pdot_elec[0], SphP[0].Grad.dpdot_elec[0], GRADIENT_TYPE_NORMAL);
  gradient_init(&SphP[0].pdot_elec[1], &PrimExch[0].pdot_elec[1], SphP[0].Grad.dpdot_elec[1], GRADIENT_TYPE_NORMAL);
  gradient_init(&SphP[0].pdot_elec[2], &PrimExch[0].pdot_elec[2], SphP[0].Grad.dpdot_elec[2], GRADIENT_TYPE_NORMAL);
#endif

#ifdef SGS_TURBULENCE
  gradient_init(&SphP[0].SgsTData.Pressure, &PrimExch[0].SGS_T_Pressure, SphP[0].Grad.dsgstPressure, GRADIENT_TYPE_NORMAL);
#endif
...
void gradient_init(MyFloat *addr, MyFloat *addr_exch, MySingle *addr_grad, int type)
{
  ...
  grad_elements[N_Grad].idx = N_Grad;
#endif
