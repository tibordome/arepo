 ...
 *
 * Model the coupling between the gas-dust and the IR radiation field.
 */
...
{
  int idx, i;
  mpi_printf_rt(0, "RT: Calculating IR chemistry.....\n");
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      for(int num1 = UV_BINS; num1 < MRT_BINS; num1++)
        {
          double A     = dt * SphP[i].KappaIR_R[num1] * c_internal_units;
...
double mrt_update_IR_cooling(int i, double dt)
{
  if(dt <= 0.0)
  ...
  else
    {
      ...
      double inv_cv = GAMMA_MINUS1 * (molecular_weight * PROTONMASS / All.UnitMass_in_g * All.HubbleParam) /
                      (BOLTZMANN / All.UnitEnergy_in_cgs * All.HubbleParam) / SphP[i].Density;
          ...
          do
            {
              for(int kk = 0; kk < times; kk++)
                {
                  double de = (cspeed * radiation_constant * pow(temp, 4) - c_internal_units * Enew) /
                              (1.0 / (dt * SphP[i].KappaIR_P[num1 - UV_BINS]) + c_internal_units +
                               4.0 * cspeed * radiation_constant * pow(temp, 3) * inv_cv);
...
double mrt_update_IR_cooling(int i, double dt)
{
  ...
  if(dt <= 0.0
  ...
  else
    {
      ...
      double unew = SphP[i].RT_utherm * SphP[i].Density;
#else
      double unew = SphP[i].Utherm * SphP[i].Density;
#endif
      double uold = unew;
      ...
      if(P[i].ID == 50000701)
        printf("uold =  %g \t unew = %g \t kappa_p = %g \n", uold, unew, SphP[i].KappaIR_P[num1]);
      ...
      double trap_frac = SphP[i].Trapped_Cons_DensPhot[num1 - UV_BINS] /
                         (SphP[i].Trapped_Cons_DensPhot[num1 - UV_BINS] + SphP[i].Cons_DensPhot[num1]);
#else
      double Enew = SphP[i].DensPhot[num1];
#endif
      double Eold = Enew;
      ...
      // if(P[i].ID>=40000000)
      // mpi_printf_rt(0,"%g \t %g \n", uold*inv_cv, pow(Eold/radiation_constant, (1./4.))) ;

      double temp = uold * inv_cv;

      if(P[i].ID == 50000701)
        printf("uold =  %g \t unew = %g \t density = %g \n", uold * inv_cv, unew * inv_cv, SphP[i].Density);

      double de = (cspeed * radiation_constant * pow(temp, 4) - c_internal_units * Enew) /
                  (1.0 / (dt * SphP[i].KappaIR_P[num1]) + c_internal_units +
                   4.0 * cspeed * radiation_constant * pow(temp, 3) * inv_cv); /*Semi-implicit estimate*/
      ...
      if(fabs(de / unew) < 0.1 && fabs(de / Enew) < 0.1)
      ...
      else
        {
          ...
          double coeff[2]               = {SphP[i].KappaIR_P[num1] * c_internal_units,
                             SphP[i].KappaIR_P[num1] * cspeed * radiation_constant * pow(inv_cv, 4.0)};
          ...
          while(t < t1)
            {
              ...
              if(status != GSL_SUCCESS)
                terminate(
                    "ID = %d || we seem to have failed in the integration ----  Tgas = %g \t Trad = %g \t\t Position of particle = %g "
                    "%g %g\t\t Coeffss = %g %g \t\t ratio = %g\n",
                    P[i].ID, uold * inv_cv, pow(Eold / radiation_constant, (1. / 4.)), P[i].Pos[0], P[i].Pos[1], P[i].Pos[2], coeff[0],
                    coeff[1], coeff[0] * y[0] / (coeff[1] * y[1] * y[1] * y[1] * y[1]));
            }
          ...
          */
          ...
          for(int ll=0;ll<UV_BINS;ll++)
            SphP[i].Cons_DensPhot[num1] += dt*KappaIR_P[ll]*SphP[i].DensPhot[ll]*SphP[i].Volume ;
            #endif*/
        }
