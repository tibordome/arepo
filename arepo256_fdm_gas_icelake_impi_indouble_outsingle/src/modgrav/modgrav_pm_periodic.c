      ...
      for(ix = minx; ix <= maxx; ++ix)
        {
          ...
          if(actual_x >= myplan.first_slab_x_of_task[ThisTask] &&
             actual_x < myplan.first_slab_x_of_task[ThisTask] + myplan.slabs_x_per_task[ThisTask])
            {
                  ...
                  for(iz = minz; iz <= maxz; ++iz)
                    {
                      ...
                      large_array_offset offset =
                          ((large_array_offset)GRID2) * (GRIDZ * (actual_x - myplan.first_slab_x_of_task[ThisTask]) + actual_y) +
                          actual_z;
