...
void sidm_set_velocities(scatter_process_data_in *sprdata_in, scatter_process_data_out *sprdata_out, double dvabs_in, double epsilon,
                         double *vcm_in, double *xunit)
{
  int k;
  ...
  for(k = 0; k < 3; k++)
    {
      ...
      vcm_in[k] = SIDM_avel * (sprdata_in->Mass1 * sprdata_in->Vel1[k] + sprdata_in->Mass2 * sprdata_in->Vel2[k]) /
                  (sprdata_in->Mass1 + sprdata_in->Mass2);
