...
static void set_shock_zone(void);
static int is_in_shock_zone(int k);
static int calculate_shock_direction(int k, double *dir);
static void ray_action(void);
static void rays_ini(void);
static inline void insert_ray(double x, double y, double z, double dir_x, double dir_y, double dir_z, double surface_value,
                              int current_cell, char preshock_dir);
static inline shock_ray *insert_ray_empty(void);
...
static void rays_finish(void);
static void set_shock_surface(void);
static void set_machnumber(void);  // and pre-shock quantities
static void set_additional_info(void);
...
#ifdef SHOCK_FINDER_POST_PROCESSING
  mpi_printf_each("\tTask %d: Particles in shock zone: %lld\n", ThisTask, particles_in_shock_zone);
#endif
}
...
static int is_in_shock_zone(int k)
{
...
#ifdef COSMIC_RAYS
  if(SDATA(k).CRpseudoTgrad[0] * SphP[k].Grad.drho[0] + SDATA(k).CRpseudoTgrad[1] * SphP[k].Grad.drho[1] +
         SDATA(k).CRpseudoTgrad[2] * SphP[k].Grad.drho[2] <=
     0)
    {
      return 0;
    }
#else
  if(SDATA(k).Tgrad[0] * SphP[k].Grad.drho[0] + SDATA(k).Tgrad[1] * SphP[k].Grad.drho[1] + SDATA(k).Tgrad[2] * SphP[k].Grad.drho[2] <=
     0)
    {
      return 0;
    }
...
#endif
#ifdef ZONE_JUMP_V
      v_preshock = PrimExch[preshockcell].VelGas[0] * asdir[0] + PrimExch[preshockcell].VelGas[1] * asdir[1] +
                   PrimExch[preshockcell].VelGas[2] * asdir[2];
...
#endif
#ifdef ZONE_JUMP_V
      v_postshock = PrimExch[postshockcell].VelGas[0] * asdir[0] + PrimExch[postshockcell].VelGas[1] * asdir[1] +
                    PrimExch[postshockcell].VelGas[2] * asdir[2];
#endif
  ...
  if(delta_log_s < log(((2 * GAMMA * All.MachMin * All.MachMin - GAMMA + 1) / (GAMMA + 1)) *
                       pow((GAMMA - 1 + 2 / (All.MachMin * All.MachMin)) / (GAMMA + 1), GAMMA)))  // mach number too low
    {
      return 0;
    }
  ...
  if(delta_log_T < log(((2 * GAMMA * All.MachMin * All.MachMin) - (GAMMA - 1)) * ((GAMMA - 1) * All.MachMin * All.MachMin + 2) /
                       ((GAMMA + 1) * (GAMMA + 1) * All.MachMin * All.MachMin)))  // mach number too low
    {
      return 0;
    }
...
static int calculate_shock_direction(int k, double *dir)
{
  ...
  dir[0] = -(SphP[k].Grad.dpress[0] / pow(SphP[k].Density, GAMMA) -
             GAMMA * SphP[k].Pressure * pow(SphP[k].Density, -GAMMA - 1) * SphP[k].Grad.drho[0]);
  dir[1] = -(SphP[k].Grad.dpress[1] / pow(SphP[k].Density, GAMMA) -
             GAMMA * SphP[k].Pressure * pow(SphP[k].Density, -GAMMA - 1) * SphP[k].Grad.drho[1]);
  dir[2] = -(SphP[k].Grad.dpress[2] / pow(SphP[k].Density, GAMMA) -
             GAMMA * SphP[k].Pressure * pow(SphP[k].Density, -GAMMA - 1) * SphP[k].Grad.drho[2]);
#endif
...
static int inconsistent_jump(shock_ray *ray)
{
  ...
  if(All.ComovingIntegrationOn && (All.OmegaBaryon != 0))
    {
      ...
      double overdensity        = ray->rho_pre_shock / (All.OmegaBaryon * 3 * All.Hubble * All.Hubble / (8 * M_PI * All.G)) -
                           1;  // a factors and h factors cancel
...
#ifdef COSMIC_RAYS
  double M_e = calculate_machnumber_cosmic_rays_from_energy_equation(ray->pth_pre_shock, ray->pth_post_shock, ray->pcr_pre_shock,
                                                                     ray->pcr_post_shock, ray->rho_pre_shock, ray->rho_post_shock);
  ...
  if(!((M_rho >= 1 && M_rho <= 3) ||
       (ray->machnumber >= 1 && ray->machnumber <= 3)))  // perform the check only if density or temperature Mach number is in range
                                                         // [1,3] (density jump not sensitive for high Mach numbers)
    {
      return 0;  // consistent
    }
...
#ifdef SHOCK_JUMP_V
      ray->post_shock_quant = P[ray->current_cell].Vel[0] * ray->dir[0] + P[ray->current_cell].Vel[1] * ray->dir[1] +
                              P[ray->current_cell].Vel[2] * ray->dir[2];
#endif
...
#ifdef SHOCK_JUMP_V
      ray->post_shock_quant = PrimExch[ray->current_cell].VelGas[0] * ray->dir[0] +
                              PrimExch[ray->current_cell].VelGas[1] * ray->dir[1] +
                              PrimExch[ray->current_cell].VelGas[2] * ray->dir[2];
#endif
...
#endif
          ray->c_pre_shock =
              sqrt(GAMMA * BOLTZMANN * SHOCK_JUMP_T_FLOOR * All.UnitMass_in_g / (All.UnitEnergy_in_cgs * meanweight * PROTONMASS));
        }
#endif

#ifdef SHOCK_JUMP_V

      double v_preshock = P[ray->current_cell].Vel[0] * (-ray->dir[0]) + P[ray->current_cell].Vel[1] * (-ray->dir[1]) +
                          P[ray->current_cell].Vel[2] * (-ray->dir[2]);
...
#ifdef SHOCK_JUMP_V
      double v_preshock = PrimExch[ray->current_cell].VelGas[0] * (-ray->dir[0]) +
                          PrimExch[ray->current_cell].VelGas[1] * (-ray->dir[1]) +
                          PrimExch[ray->current_cell].VelGas[2] * (-ray->dir[2]);
      ...
      if(2 * fabs(gamma1 - gamma2) / fabs(gamma1 + gamma2) < 0.01)  // fallback to thermal criterion
        {
          ...
          double gamma_mean = 0.5 * (gamma1 + gamma2);
          ray->machnumber =
              sqrt((gamma_mean + 1) * (pth2 + pcr2) / (2 * gamma_mean * (pth1 + pcr1)) + (gamma_mean - 1) / (2 * gamma_mean));
        }
      else
        {
...
static void move_ray(shock_ray *ray)
{
  ...
  ray->current_cell =
      find_neighbouring_cell_pos_para(ray->current_cell, ray->pos, ray->dir, ray->previous_cell, ray->previous_cell_task, ray->pos,
                                      &task, &original_index, &edge, &boundary_reached);
  ...
  MOVEPRINTF("task: %d: moved ray to %f %f \tray now in cell: %f %f\n", ThisTask, ray->pos[0], ray->pos[1],
             P[ray->current_cell].Pos[0], P[ray->current_cell].Pos[1])
  ...
  if(this_task)
    {
      if((ray_wrong_direction(SDATA(ray->current_cell).ShockDir, ray, 1) || !SDATA(ray->current_cell).ShockZone) &&
         !ray->preshock_dir)  // ray reached the post shock region
        {
          ...
          ray->preshock_dir = 1;
          ray->previous_cell =
              ray->current_cell;  // note: dir changed, therefore the current cell has to be ignored in find_next_voronoi_cell
      ...
      else if((ray_wrong_direction(SDATA(ray->current_cell).ShockDir, ray, 1) || !SDATA(ray->current_cell).ShockZone) &&
              ray->preshock_dir)  // ray reached the pre shock region
        {
...
#error Use USE_MAXIMUM_FOR_POST_SHOCK in combination with SHOCK_JUMP_T.
  ...
  else  // this_task == 0, ray crossed the local domain
    {
      ...
      if((ray_wrong_direction(SDATAEXCH(ray->current_cell).ShockDir, ray, 0) || !SDATAEXCH(ray->current_cell).ShockZone) &&
         !ray->preshock_dir)  // ray reached the post shock region
        {
      ...
      else if((ray_wrong_direction(SDATAEXCH(ray->current_cell).ShockDir, ray, 0) || !SDATAEXCH(ray->current_cell).ShockZone) &&
              ray->preshock_dir)  // ray reached the pre shock region
        {
      ...
      if(SDATA(k).ShockZone)
        {
          ...
          insert_ray(SphP[k].Center[0], SphP[k].Center[1], SphP[k].Center[2], (-1 * SDATA(k).ShockDir[0]), (-1 * SDATA(k).ShockDir[1]),
                     (-1 * SDATA(k).ShockDir[2]), divergence(k), k, 0);
        }
    }
...
#ifdef SHOCK_FINDER_POST_PROCESSING
  mpi_printf_each("\tTask %d: Particles in shock surface: %lld\n", ThisTask, particles_in_shock_surface);
#endif
}
 ...
 */
static void set_machnumber(void)
{
  shock_ray *current_ray = Rays;
      ...
      if(current_ray->current_cell == -1 && SDATA(current_ray->original_cell).ShockSurface)  // it's a potential shock
        {
          ...
          if(!wrong_jump)
            {
              wrong_jump =
                  in_effective_eos_range(SphP[current_ray->original_cell].Density, SDATA(current_ray->original_cell).Temperature);
            }
      ...
      if(receive_task < NTask)
        {
          if(rays_send_count[receive_task] > 0 || rays_receive_count[receive_task] > 0)
            {
              MPI_Sendrecv(&rays_out[send_offset[receive_task]], rays_send_count[receive_task] * sizeof(shock_ray), MPI_BYTE,
                           receive_task, 0, &rays_in[receive_offset[receive_task]],
                           rays_receive_count[receive_task] * sizeof(shock_ray), MPI_BYTE, receive_task, 0, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
            }
        }
    }
...
static void copy_ray(shock_ray *origin, shock_ray *destination)
{
  destination->startpos[0] = origin->startpos[0];
  destination->startpos[1] = origin->startpos[1];
  destination->startpos[2] = origin->startpos[2];
