 ...
 *
 * \file        src/pm/pm_mpi_fft.c
 * \date        MM/YYYY
 * \author
 * \brief       Here we implement our parallel FFT transforms as needed by the code
 * \details     We only use the one-dimensional FFTW3 routines, because the MPI versions of FFTW3
 *              allocate memory for themselves during the transforms (which we want to strictly avoid),
...
void my_column_based_fft_init(fft_plan *plan, int NgridX, int NgridY, int NgridZ)
{
  ...
  plan->offsets_send_A      = (size_t *)mymalloc_clear("offsets_send_A", NTask * sizeof(size_t));
  plan->offsets_recv_A      = (size_t *)mymalloc_clear("offsets_recv_A", NTask * sizeof(size_t));
  plan->offsets_send_B      = (size_t *)mymalloc_clear("offsets_send_B", NTask * sizeof(size_t));
  plan->offsets_recv_B      = (size_t *)mymalloc_clear("offsets_recv_B", NTask * sizeof(size_t));
  plan->offsets_send_C      = (size_t *)mymalloc_clear("offsets_send_C", NTask * sizeof(size_t));
  plan->offsets_recv_C      = (size_t *)mymalloc_clear("offsets_recv_C", NTask * sizeof(size_t));
  plan->offsets_send_D      = (size_t *)mymalloc_clear("offsets_send_D", NTask * sizeof(size_t));
  plan->offsets_recv_D      = (size_t *)mymalloc_clear("offsets_recv_D", NTask * sizeof(size_t));
  plan->offsets_send_13     = (size_t *)mymalloc_clear("offsets_send_13", NTask * sizeof(size_t));
  plan->offsets_recv_13     = (size_t *)mymalloc_clear("offsets_recv_13", NTask * sizeof(size_t));
  plan->offsets_send_23     = (size_t *)mymalloc_clear("offsets_send_23", NTask * sizeof(size_t));
  plan->offsets_recv_23     = (size_t *)mymalloc_clear("offsets_recv_23", NTask * sizeof(size_t));
  plan->offsets_send_13back = (size_t *)mymalloc_clear("offsets_send_13back", NTask * sizeof(size_t));
  plan->offsets_recv_13back = (size_t *)mymalloc_clear("offsets_recv_13back", NTask * sizeof(size_t));
  plan->offsets_send_23back = (size_t *)mymalloc_clear("offsets_send_23back", NTask * sizeof(size_t));
  plan->offsets_recv_23back = (size_t *)mymalloc_clear("offsets_recv_23back", NTask * sizeof(size_t));

  plan->count_send_A      = (size_t *)mymalloc_clear("count_send_A", NTask * sizeof(size_t));
  plan->count_recv_A      = (size_t *)mymalloc_clear("count_recv_A", NTask * sizeof(size_t));
  plan->count_send_B      = (size_t *)mymalloc_clear("count_send_B", NTask * sizeof(size_t));
  plan->count_recv_B      = (size_t *)mymalloc_clear("count_recv_B", NTask * sizeof(size_t));
  plan->count_send_C      = (size_t *)mymalloc_clear("count_send_C", NTask * sizeof(size_t));
  plan->count_recv_C      = (size_t *)mymalloc_clear("count_recv_C", NTask * sizeof(size_t));
  plan->count_send_D      = (size_t *)mymalloc_clear("count_send_D", NTask * sizeof(size_t));
  plan->count_recv_D      = (size_t *)mymalloc_clear("count_recv_D", NTask * sizeof(size_t));
  plan->count_send_13     = (size_t *)mymalloc_clear("count_send_13", NTask * sizeof(size_t));
  plan->count_recv_13     = (size_t *)mymalloc_clear("count_recv_13", NTask * sizeof(size_t));
  plan->count_send_23     = (size_t *)mymalloc_clear("count_send_23", NTask * sizeof(size_t));
  plan->count_recv_23     = (size_t *)mymalloc_clear("count_recv_23", NTask * sizeof(size_t));
  plan->count_send_13back = (size_t *)mymalloc_clear("count_send_13back", NTask * sizeof(size_t));
  plan->count_recv_13back = (size_t *)mymalloc_clear("count_recv_13back", NTask * sizeof(size_t));
  plan->count_send_23back = (size_t *)mymalloc_clear("count_send_23back", NTask * sizeof(size_t));
  plan->count_recv_23back = (size_t *)mymalloc_clear("count_recv_23back", NTask * sizeof(size_t));
  ...
  if(forward == 1)
    {
      /* do the z-direction FFT, real to complex */
#pragma omp parallel for
      for(size_t n = 0; n < (size_t)plan->base_ncol; n++)
...
#pragma omp parallel for
      for(size_t n = 0; n < (size_t)plan->transposed_ncol; n++)
...
#pragma omp parallel for
      for(size_t n = 0; n < (size_t)plan->second_transposed_ncol; n++)
 ...
 *  \return void
 */
static void my_fft_column_remap(fft_complex *data, int Ndims[3], /* global dimensions of data cube */
                                int in_firstcol, int in_ncol,    /* first column and number of columns */
                                fft_complex *out, int perm[3], int out_firstcol, int out_ncol, size_t *offset_send,
                                size_t *offset_recv, size_t *count_send, size_t *count_recv, size_t just_count_flag)
{
  ...
  if(just_count_flag)
    {
      ...
      if(nexport != ncells)
        terminate("nexport=%zu != ncells=%zu", nexport, ncells);
    }
  else
    {
      nimport = 0;
      ...
      if(count != nimport)
        {
          ...
          terminate("count=%zu nimport=%zu   ncol=%d fi=%d la=%d first=%d last=%d", count, nimport, out_ncol, fi, la, first[1],
                    last[1]);
        }
    }
}
