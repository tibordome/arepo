 ...
 *  \return void
 */
void pressure_function(double PP, struct state *st, double *F, double *FD)
{
  if(PP <= st->press) /* rarefaction wave */
    {
      double prat = PP / st->press;
  ...
  else /* shock wave */
    {
      ...
      double qrt = sqrt(ak / (bk + PP));

      *F  = (PP - st->press) * qrt;
      *FD = (1.0 - 0.5 * (PP - st->press) / (bk + PP)) * qrt;
    }
}
