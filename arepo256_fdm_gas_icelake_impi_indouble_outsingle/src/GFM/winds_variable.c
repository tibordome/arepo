...
static double gfm_calc_variable_wind_parameters_specific_mass_loading(double velocity, double v_vir, double vel_disp, double utherm,
                                                                      double metallicity)
{
...
#error "GFM_WIND_ENERGY_METAL_DEPENDENCE_TANH requires GFM_WIND_ENERGY_METAL_DEPENDENCE"
  ...
  if(metallicity > 0)
    wind_energy *= 1.0 - (1.0 - All.WindEnergyReductionFactor) * 0.5 *
                             (tanh(log10(metallicity / All.WindEnergyReductionMetallicity) / All.WindEnergyReductionExponent) + 1.0);
#else
  wind_energy *=
      (All.WindEnergyReductionFactor + (1 - All.WindEnergyReductionFactor) / (1 + pow(metallicity / All.WindEnergyReductionMetallicity,
                                                                                      All.WindEnergyReductionExponent)));
#endif
#endif
...
#endif
  return (wind_energy + sqrt(wind_energy * wind_energy + velocity * velocity * wind_momentum * wind_momentum)) /
         (velocity * velocity + 2 * utherm);
}
...
void gfm_calc_variable_wind_parameters(double stellar_mass, double halo_val, double vel_disp, double metallicity, wind_parameter *wp)
{
  ...
  /* we take here the FOF mass */ v_vir = pow(10.0 * All.G * All.cf_H * halo_val, 1.0 / 3);

  v_vir *= 0.95; /* empirical correction factor to better line up with the local velocity dispersion estimates obtained with
                    GFM_WINDS_VARIABLE for well resolved halos */
...
void init_variable_winds(void)
{
  ...
  file_path_sprintf(buf, "%s/variable_wind_scaling.txt", All.OutputDir);
  FILE *fd = fopen(buf, "w");
