  ...
  if(st_L->rho > 0 && st_R->rho > 0)
    {
      ...
      double DednerSpeed = fmax(cf_L, cf_R);
      st_face->Bx        = 0.5 * (st_R->Bx + st_L->Bx) - 0.5 / DednerSpeed * (st_R->Psi - st_L->Psi);
      st_face->Psi       = 0.5 * (st_R->Psi + st_L->Psi) - 0.5 * DednerSpeed * (st_R->Bx - st_L->Bx);

      flux->B[0] = 0;
      flux->Psi  = 0;
#else
      /* Dedner et. al 2002, eq. 42 */
      st_face->Bx  = 0.5 * (st_R->Bx + st_L->Bx) - 0.5 / All.DednerSpeed * (st_R->Psi - st_L->Psi);
      st_face->Psi = 0.5 * (st_R->Psi + st_L->Psi) - 0.5 * All.DednerSpeed * (st_R->Bx - st_L->Bx);

      flux->B[0] = st_face->Psi;
      flux->Psi  = All.DednerSpeed * All.DednerSpeed * st_face->Bx;
#endif /* MHD_DEDNER_VARIABLE_SPEED */
#else
      st_face->Bx = 0.5 * (st_L->Bx + st_R->Bx);
      flux->B[0]  = 0.;
#endif
      ...
      S = fmax(fmax(fabs(st_L->velx - cf_L), fabs(st_R->velx - cf_R)), fmax(fabs(st_L->velx + cf_L), fabs(st_R->velx + cf_R)));

      S_L = fmin(st_L->velx - cf_L, st_R->velx - cf_R);
      S_R = fmax(st_L->velx + cf_L, st_R->velx + cf_R);
      ...
      if(S_L >= 0)
      ...
      else
        {
          ...
          if(S_L_star >= 0 || (st_face->Bx == 0 && S_M >= 0))  // we already know: S_L <= 0
            {
              st_middle = &st_Lstar;
              hlld_get_fluxes_star(st_L, &st_Lstar, &flux_L, S_L, flux);
            }
          else if(S_R_star <= 0 || (st_face->Bx == 0))  // we already know: S_R >= 0
            {
              st_middle = &st_Rstar;
...
#ifdef MHD_DEDNER_VARIABLE_SPEED
  if(flux->mass > 0.)
    flux->Psi += st_L->Psi * flux->mass;
  else
    flux->Psi += st_R->Psi * flux->mass;
#endif
#endif
...
static double hlld_get_fast_wave(struct state *st)
{
  ...
  gPress += All.SgsTConst.GammaSgsT * st->sgstPressure;
#endif
...
static void hlld_get_fluxes_from_state(struct state *st, struct fluxes *flux, double *st_ptot)
{
...
#ifdef SGS_TURBULENCE_RIEMANN_PRESSURE
  double sgst_press = st->sgstPressure;
#else
  double sgst_press = 0;
#endif
  ...
  double Bsqr       = st->Bx * st->Bx + st->By * st->By + st->Bz * st->Bz;
  flux->momentum[0] = st->rho * st->velx * st->velx + st->press + 0.5 * Bsqr - st->Bx * st->Bx + cr_press + sgst_press;
  ...
  double ptot = st->press + 0.5 * Bsqr + cr_press + sgst_press;
