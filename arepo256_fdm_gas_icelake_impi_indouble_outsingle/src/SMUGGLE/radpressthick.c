 ...
 * 
 * \file        src/SMUGGLE/radpressthick.c
 * \date        03/2020
...
#if defined(SMUGGLE_RADPRESS_OPT_THICK) && defined(OTVET)
...
void radpressthick(void)
{
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      /*
                dt = (All.otvet_Radiation_Ti_endstep -
         All.otvet_Radiation_Ti_begstep) * All.Timebase_interval;
      */
      ...
      for(k = 0; k < 3; k++)
        {
          ...
          if(!gsl_finite(dvel))
            terminate(
                "SMUGGLE_RADPRESS_OPT_THICK: bad radiation pressure dvel=%g "
  ...
  mpi_printf("SMUGGLE_RADPRESS_OPT_THICK: kin. energy ratio = (%g - %g) \n", global_ekin_min, global_ekin_max);
}
