          ...
          if(All.Time < 0.2 * All.TimeMax)
            {
              ...
              {
                Temp = All.TempShell - abs(P[i].Pos[0] - All.ShellBaseRadius) / abs(All.ShellBaseRadius - All.RTempMin) *
                                           abs(All.TempShell - All.TempMin);  // for a linear interpolation of the temperature
              }
              d /
                  ...
                  if((P[i].Pos[0] < wdR[k]) && (P[i].Pos[0] > wdR[k - 1]))
                    {
                      //                    Temp = wdTemp[k];
                      Temp = wdTemp[k - 1] +
                             (wdTemp[k - 1] - wdTemp[k]) / (wdR[k - 1] - wdR[k]) *
                                 (P[i].Pos[0] - wdR[k - 1]);  // linear interpolation between known positions and temperatures
...
static void compute_orbital_period(void);
static void apply_orbital_acceleration(void);

void do_binary_source_terms_first_half(void) { apply_orbital_acceleration(); }
...
void compute_orbital_period(void)
{
...
void apply_orbital_acceleration(void)
{
  int idx;
