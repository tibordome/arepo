      ...
      if(recvTask < NTask)
        {
          if(Mesh_Send_count[recvTask] > 0 || Mesh_Recv_count[recvTask] > 0)
            {
              /* get the particles */
              MPI_Sendrecv(&rt_tmpPrimExch[Mesh_Send_offset[recvTask]], Mesh_Send_count[recvTask] * sizeof(struct rt_primexch),
                           MPI_BYTE, recvTask, TAG_DENS_A, &RTPrimExch[Mesh_Recv_offset[recvTask]],
