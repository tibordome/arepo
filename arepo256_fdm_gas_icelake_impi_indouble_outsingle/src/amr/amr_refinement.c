...
static void amr_refine_cells(void);
...
static void amr_derefine_cells(void);
...
{
  TIMER_START(CPU_AMR_REFINEMENT);
  if(All.TotNumGas == 0)
    return;
...
#ifndef FORCE_EQUAL_TIMESTEPS
  AbortRefinement =
      (struct abort_refinement *)mymalloc("AbortRefinement", Mesh.Nghost_nodes + Mesh.Nghost_dp * sizeof(struct abort_refinement));
#endif
  Nexport = 0;
              ...
              if(recvTask < NTask)
                {
                  if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
                    {
                      /* get the particles */
                      MPI_Sendrecv(&RefDataIn[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct refdata_in), MPI_BYTE,
                                   recvTask, TAG_GRAV_A, &RefDataGet[Recv_offset[recvTask]],
                                   Recv_count[recvTask] * sizeof(struct refdata_in), MPI_BYTE, recvTask, TAG_GRAV_A, MPI_COMM_WORLD,
                                   &status);
                    }
                }
            }
  ...
  if(countall)
    {
      ...
      if(Mesh.Ndp + count >= Mesh.MaxNdp)
        {
          Mesh.DP = (point *)myrealloc_movable(Mesh.DP, (Mesh.MaxNdp + count) * sizeof(point));
          Mesh.MaxNdp += count;
        }
  ...
  for(k = 0; k < 3; k++)
    {
      ...
      SphP[j].B[k] = SphP[i].B[k] + SphP[i].Grad.dB[k][0] * (P[j].Pos[0] - P[i].Pos[0]) +
...
void amr_sum_hydro(int new_cell, int i)
{
  ...
  for(s = 0; s < N_Scalar; s++)
    *(MyFloat *)(((char *)(&SphP[new_cell])) + scalar_elements[s].offset_mass) +=
        (*(MyFloat *)(((char *)(&SphP[i])) + scalar_elements[s].offset_mass));
#endif
#ifdef TRACER_FIELD
