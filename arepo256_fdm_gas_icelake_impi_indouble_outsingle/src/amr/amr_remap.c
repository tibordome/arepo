...
int amr_remap(void)
{
  ...
  int *topNodeLevel = (int *)mymalloc_movable(&topNodeLevel, "topNodeLevel", sizeof(int) * NTopleaves);
...
static void distribute_sph_particles(void)
{
  ...
  All.BunchSize  = (int)((All.BufferSize * 1024 * 1024) / (sizeof(data_index) + sizeof(struct data_nodelist) +
                                                          sizeof(struct distributedata_in) + sizeof(struct distributedata_out) +
                                                          smax(sizeof(struct distributedata_in), sizeof(struct distributedata_out))));
  ...
  do
    {
          ...
          if(recvTask < NTask)
            {
              if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
                {
                  /* get the particles */
                  MPI_Sendrecv(&DistributeDataIn[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct distributedata_in),
                               MPI_BYTE, recvTask, TAG_DENS_A, &DistributeDataGet[Recv_offset[recvTask]],
                               Recv_count[recvTask] * sizeof(struct distributedata_in), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                               MPI_STATUS_IGNORE);
                }
            }
        }
      ...
      DistributeDataResult =
          (struct distributedata_out *)mymalloc("DistributeDataResult", nimport * sizeof(struct distributedata_out));
          ...
          if(recvTask < NTask)
            {
              if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
                {
                  /* send the results */
                  MPI_Sendrecv(&DistributeDataResult[Recv_offset[recvTask]], Recv_count[recvTask] * sizeof(struct distributedata_out),
                               MPI_BYTE, recvTask, TAG_DENS_B, &DistributeDataOut[Send_offset[recvTask]],
                               Send_count[recvTask] * sizeof(struct distributedata_out), MPI_BYTE, recvTask, TAG_DENS_B,
                  ...
                  if(r2 < h2)
                    {
                      ...
                      dhsmlrho += -vol * (NUMDIMS * hinv * wk + u * dwk);
                    }
                }
            }
        }
...
static void spread_sph_particles(void)
{
  ...
  All.BunchSize  = (int)((All.BufferSize * 1024 * 1024) / (sizeof(data_index) + sizeof(struct data_nodelist) +
                                                          sizeof(struct spreaddata_in) + sizeof(struct spreaddata_in)));
  DataIndexTable = (data_index *)mymalloc("DataIndexTable", All.BunchSize * sizeof(data_index));
  DataNodeList   = (struct data_nodelist *)mymalloc("DataNodeList", All.BunchSize * sizeof(struct data_nodelist));
          ...
          if(recvTask < NTask)
            {
              if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
                {
                  /* get the particles */
                  MPI_Sendrecv(&SpreadDataIn[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct spreaddata_in), MPI_BYTE,
                               recvTask, TAG_DENS_A, &SpreadDataGet[Recv_offset[recvTask]],
                               Recv_count[recvTask] * sizeof(struct spreaddata_in), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                               MPI_STATUS_IGNORE);
                }
            }
        }
