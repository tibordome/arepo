...
static double init_tracers_from_snap(void);
...
#endif
  double TracerMass;
  if(All.NumberOfTracersToGenerate == 0)
    {
      TracerMass = init_tracers_from_snap();
    }
  else
    {
      TracerMass = generate_tracers();
    }
#else
...
#ifdef TRACER_TRAJECTORY_GENERATE
double init_tracers_from_snap(void)
{
  int i;
  double mTask = 0;
  for(i = 0; i < NumGas; i++)
      ...
      if(P[i].Type == TRACER_PARTICLE)
        {
          ...
          P[i].TracerHsml = All.MinimumTracerHsml;
          // P[i].TimeBinHydro = 0;
          // P[i].TimeBinGrav  = 0;
        }
    }
  ...
  if(ThisTask == 0)
    {
      ...
      ntTasks = (int *)mymalloc("ntTasks", NTask * sizeof(int));
    }

  MPI_Gather(&mTask, 1, MPI_DOUBLE, mTasks, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Gather(&ntTask, 1, MPI_INT, ntTasks, 1, MPI_INT, 0, MPI_COMM_WORLD);
  ...
  if(ThisTask == 0)
    {
      ...
      printf("mtot=%g, nttot=%d\n", mtot, nttot);
      nTracertot = nttot;
      tracerMass = mtot / nttot;
  ...
  MPI_Bcast(&nTracertot, 1, MPI_INT, 0, MPI_COMM_WORLD);

  All.NTracerTot = nTracertot;

  mpi_printf("All.TotNumPart=%d, All.NTracerTot=%d\n", All.TotNumPart, All.NTracerTot);
  mpi_printf("TracerMass=%g\n", tracerMass);

  mpi_printf("All tracers initiated from snap.\n");
...
double generate_tracers(void)
{
  ...
  domain_resize_storage(nTracersLocal, 0, 0);
...
void generate_single_tracer_in_cell(int idx)
{
  ...
  while(!found)
    {
      ...
      found = 1;
      dist2 = (px - P[idx].Pos[0]) * (px - P[idx].Pos[0]) + (py - P[idx].Pos[1]) * (py - P[idx].Pos[1]) +
              (pz - P[idx].Pos[2]) * (pz - P[idx].Pos[2]);
      ...
      while(q >= 0)
        {
          ...
          double d2 = (px - Mesh.DP[dp].x) * (px - Mesh.DP[dp].x) + (py - Mesh.DP[dp].y) * (py - Mesh.DP[dp].y) +
                      (pz - Mesh.DP[dp].z) * (pz - Mesh.DP[dp].z);
      ...
      if(iter > 100)
        {
          printf("There is a problem with cell %d (ID=%d): pos=%g,%g,%g min=%g,%g,%g max=%g,%g,%g, cellvol=%g, searchvol=%g\n", idx,
                 P[idx].ID, P[idx].Pos[0], P[idx].Pos[1], P[idx].Pos[2], min[0], min[1], min[2], max[0], max[1], max[2],
                 SphP[idx].Volume, dx * dy * dz);
          terminate("fail");
        }
    }
  ...
  memset(&P[p], 0, sizeof(struct particle_data));
  ...
  if(ThisTask == 0)
    {
      ...
      file_path_sprintf(buf, "%s/%s", All.OutputDir, All.TracerOutputFile);
  ...
  if(ThisTask == 0)
    {
      ...
      char buf[MAXLEN_PATH];
      file_path_sprintf(buf, "%s/tracer.dat", All.OutputDir);
      ...
      float *data = (float *)mymalloc("tdata_single", NTracer * sizeof(float));
