...
int add_backgroundgrid(void)
{
  ...
  // count_leaves. what goes at the top?
...
#ifdef METALS
                  SphP[count_leaves].MassMetallicity = 0.0;
#endif

#ifdef REFINEMENT_RPS
                  SphP[count_leaves].RPSGalaxyMass = 0.0;
#endif
  ...
  int ncells       = 0, ncellstot;
  double locvolume = 0.0, totvolume;
  ...
  MPI_Allreduce(&ncells, &ncellstot, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&locvolume, &totvolume, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  ...
  mpi_printf("ADD BACKGROUND GRID: new particle number=%lld\n", All.TotNumPart);
  mpi_printf("ADD BACKGROUND GRID: new gas particle number=%lld\n\n", All.TotNumGas);
  ...
  mpi_printf(
      ...
      totvolume / ncellstot, mtot / ngas_count_all_old);
...
void modify_boxsize(const double new_val)
{
  All.BoxSize       = new_val;
  double box_long[] = {1, 1, 1};
#ifdef LONG_X
  box_long[0] = LONG_X;
#endif
#ifdef LONG_Y
  ...
  for(size_t i = 0; i < sizeof(All.BoxSizes) / sizeof(All.BoxSizes[0]); i++)
    All.BoxSizes[i] = All.BoxSize * box_long[i];
}
