 ...
 * \notes       Please contact Christine Simpson before using this routine
 *              christine.simpson@gmail.com
 *
...
void create_dummy_particles(void)
{
...
#if defined(EXTERNALSHEARBOX_MIXED_INJECTION) & !defined(EXTERNALSHEARBOX_KSRATE)

  prob_factor = (1.0 / EXTERNALSHEARBOX_MIXED_INJECTION - 1.0);
  ...
  mpi_printf("CMS_FEEDBACK: create particles: time = %f prob = %f prob_factor = %f sfr = %g dt = %g %d %d\n", All.Time, prob,
             prob_factor, totpeaksfrrate, dt, All.HighestActiveTimeBin, All.HighestOccupiedTimeBin);

#else /* not mixed injection */

#ifdef EXTERNALSHEARBOX_MIXED_INJECTION
  if(EXTERNALSHEARBOX_MIXED_INJECTION > 0.0)
    prob_factor = (1.0 / EXTERNALSHEARBOX_MIXED_INJECTION - 1.0);
#endif

  double sigma_star = 2.5e-4 * pow(Sigma, 1.4) * (All.UnitTime_in_s / 3.15569e7) * (1.9891e33 / All.UnitMass_in_g) *
                      (All.UnitLength_in_cm / 3.0857e21) * (All.UnitLength_in_cm / 3.0857e21); /*KS relation in code units */
  double totsfrrate = sigma_star * boxSize_X * boxSize_Y;
  double total_sm   = totsfrrate * dt;
  double prob       = prob_factor * sigma_star * boxSize_X * boxSize_Y * dt * SNRate / 100.0;
...
{
  double Mass_Normalization = All.UnitMass_in_g / (100.0 * 1.9891e+33);
  CPU_Step[CPU_MISC] += measure_time();
              ...
              if(P[i].Mass == MinimumStarMass)
                {
                  ...
                  prob = P[i].Mass * All.LocalFeedbackSNRate * Mass_Normalization;
                  rand = get_random_number();
                  if(rand < prob)
                    {
                      P[i].TagSNe = 1;
                  ...
                  P[i].MeanDensity = 0.0;
                  // printf("Create stars, Tag SNe = %d \t Mass of star  = %g, SN rate = %g \n", P[i].TagSNe, P[i].Mass,
                  // All.LocalFeedbackSNRate) ;
#endif
                }
              else
                {
...
#ifdef MRT_LOCAL_FEEDBACK
                  prob = MinimumStarMass * All.LocalFeedbackSNRate * Mass_Normalization;
                  rand = get_random_number();
                  if(rand < prob)
                    {
                      P[NumPart + count_spawned].TagSNe = 1;
                      sum_sne_mass += 1.0;
                      P[NumPart + count_spawned].FeedbackDone = 0;
                    }
                  else
                    {
                      P[NumPart + count_spawned].TagSNe       = 0;
                      P[NumPart + count_spawned].FeedbackDone = 1;
                    }
                  P[NumPart + count_spawned].MeanDensity = 0.0;
                  // printf("Create stars, Tag SNe = %d \t Mass of star  = %g, SN rate = %g \n", P[NumPart+count_spawned].TagSNe,
                  // MinimumStarMass, All.LocalFeedbackSNRate);
#endif
                }
            }
#endif
      ...
      for(i = 0; i < count_spawned; i++)
        {
          ...
          //          P[NumPart + i].ID, P[NumPart + i].Mass, abs(P[NumPart + i].Pos[2] - 5000.0));
        }
  ...
  if(All.HighestActiveTimeBin == All.HighestOccupiedTimeBin)
    { /* only do this for full steps */
      double totsfrrate, total_sm, total_sum_mass_stars, total_sum_sne_mass;
...
#ifdef MRT_LOCAL_FEEDBACK
      MPI_Reduce(&sum_sne_mass, &total_sum_sne_mass, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
#endif
#else
      ...
      if(ThisTask == 0)
        {
          fprintf(FdSfr, "%14e %14e %14e %14e %14e %14e %14e\n", All.Time, total_sm, totsfrrate, rate_in_msunperyear,
                  total_sum_mass_stars, cum_mass_stars, total_sum_sne_mass / (total_sum_mass_stars * Mass_Normalization));
          myflush(FdSfr);
        }
    }
}
