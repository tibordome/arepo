 ...
 */
static double tripple_min(double a, double b, double c) { return fmin(fmin(a, b), c); }

/*!
 * maximum of three numbers
 */
static double tripple_max(double a, double b, double c) { return fmax(fmax(a, b), c); }
  ...
  for(k = 0; k < 5; k++)
    {
      ...
      cos_phi /= sqrt(3 * wx_ngb_pos[k] * wx_ngb_pos[k] + dx_ngb_pos * dx_ngb_pos / 4.) *
                 sqrt(3 * wx_center[k] * wx_center[k] + dx_center * dx_center / 4.);  // minus sign irrelevant
      ...
      cos_phi /= sqrt(3 * wx_ngb_neg[k] * wx_ngb_neg[k] + dx_ngb_neg * dx_ngb_neg / 4.) *
                 sqrt(3 * wx_center[k] * wx_center[k] + dx_center * dx_center / 4.);  // minus sign irrelevant
...
void minmod_limiter(CBV(a))
{
  ...
  double(*a_temp)[NOF_BASE_FUNCTIONS][5] =
      (double(*)[NOF_BASE_FUNCTIONS][5])mymalloc("a_temp", NumGas * Nof_base_functions * 5 * sizeof(double));
...
#ifdef CHARACTERISTIC_LIMITER
  s_char_slopes =
      (double(*)[NUMDIMS][5])mymalloc("s_char_slopes", NumGas * NUMDIMS * 5 * sizeof(double));  // char slopes[cell][dim][char_var]
      ...
      for(k = 0; k < 5; k++)
        {
          ...
          if(min_angles_x[k] > All.DG_min_angle)  // no limiting
            ...
            {
              w1new = minmod(a[i][1][k], All.DG_beta / sqrt(3.) * (a[i][0][k] - w0_left[k]),
                             All.DG_beta / sqrt(3.) * (w0_right[k] - a[i][0][k]));
            }
          ...
          if(min_angles_y[k] > All.DG_min_angle)  // no limiting
            ...
            {
              w2new = minmod(a[i][2][k], All.DG_beta / sqrt(3.) * (a[i][0][k] - w0_back[k]),
                             All.DG_beta / sqrt(3.) * (w0_front[k] - a[i][0][k]));
            }
#ifndef TWODIMS
          if(min_angles_z[k] > All.DG_min_angle)  // no limiting
            {
            ...
            {
              w3new = minmod(a[i][3][k], All.DG_beta / sqrt(3.) * (a[i][0][k] - w0_bottom[k]),
                             All.DG_beta / sqrt(3.) * (w0_top[k] - a[i][0][k]));
            }
#endif
#elif defined(MINMOD_B)
          w1new = minmodB(a[i][1][k], All.DG_beta / sqrt(3.) * (a[i][0][k] - w0_left[k]),
                          All.DG_beta / sqrt(3.) * (w0_right[k] - a[i][0][k]), dl);
          w2new = minmodB(a[i][2][k], All.DG_beta / sqrt(3.) * (a[i][0][k] - w0_back[k]),
                          All.DG_beta / sqrt(3.) * (w0_front[k] - a[i][0][k]), dl);
          DG3D(w3new = minmodB(a[i][3][k], All.DG_beta / sqrt(3.) * (a[i][0][k] - w0_bottom[k]),
                               All.DG_beta / sqrt(3.) * (w0_top[k] - a[i][0][k]), dl);)
#else
          w1new = minmod(a[i][1][k], All.DG_beta / sqrt(3.) * (a[i][0][k] - w0_left[k]),
                         All.DG_beta / sqrt(3.) * (w0_right[k] - a[i][0][k]));
          w2new = minmod(a[i][2][k], All.DG_beta / sqrt(3.) * (a[i][0][k] - w0_back[k]),
                         All.DG_beta / sqrt(3.) * (w0_front[k] - a[i][0][k]));
          DG3D(w3new = minmod(a[i][3][k], All.DG_beta / sqrt(3.) * (a[i][0][k] - w0_bottom[k]),
                              All.DG_beta / sqrt(3.) * (w0_top[k] - a[i][0][k]));)
#endif
          if(fabs(w1new - a[i][1][k]) > eps ||
             fabs(w2new - a[i][2][k]) > eps DG3D(|| fabs(w3new - a[i][3][k]) > eps))  // limiter triggered
            {
              a_temp[i][1][k] = w1new;
              a_temp[i][2][k] = w2new;
...
static double calc_tau(double w_mean[5], double pv_mean[5], double w[5])
{
  ...
  if(pv[PRES] >= Epsilon_p || same_w(w, w_mean))  // no limiting necessary
  ...
  else  // solve quadratic equation
    {
      ...
      c  = (w_mean[4] - Epsilon_p / (GAMMA - 1.)) * w_mean[0] -
          0.5 * (w_mean[1] * w_mean[1] + w_mean[2] * w_mean[2] + w_mean[3] * w_mean[3]);
...
void positivity_limiter(double a_cell[NOF_BASE_FUNCTIONS][5], int cell)
{
  ...
  if(pv_mean[PRES] < Epsilon_p || pv_mean[RHO] < Epsilon_rho)
    {
      printf("cell (%f, %f, %f), mean rho:%g, mean p:%g\n", P[cell].Pos[0], P[cell].Pos[1], P[cell].Pos[2], pv_mean[RHO],
             pv_mean[PRES]);
