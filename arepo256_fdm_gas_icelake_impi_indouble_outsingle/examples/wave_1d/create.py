...
def create_ics(path, filename='ics.hdf5'):
    ...
    ## perturbations
    Density *= FloatType(1.0) + delta * np.sin(
    ...
    with h5py.File(FilePath, 'w') as IC:  # open/create file
        ## create hdf5 groups
        header = IC.create_group('Header')  # create header group
        part0 = IC.create_group(
            'PartType0')  # create particle group for gas cells
        ...
        ## write cell data
        part0.create_dataset('ParticleIDs',
                             data=np.arange(1, NumberOfCells + 1))
...
if __name__ == '__main__':
    ...
    print('examples/wave_1d/create.py: creating ICs in directory ' +
