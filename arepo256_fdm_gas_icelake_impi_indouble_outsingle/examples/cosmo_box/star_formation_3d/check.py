#!/usr/bin/env python3
""" @package ./examples/cosmo_box/star_formation_3d/check.py
...
print('cosmo_box/star_formation_3d: checking simulation output in directory ' +
...
if createReferenceSolution:
    np.savetxt(os.path.join(simulation_directory, 'sfr_ref_L8n32_reduced.txt'),
               data[::50, :])
...
data_ref = np.loadtxt(
    os.path.join(simulation_directory, 'sfr_ref_L8n32_reduced.txt'))
...
if makeplots:
    ...
    for i, a in enumerate(scalefactor_to_probe):
        ...
        delta_mass_stars[i] -= np.interp(a, scalefactor_ref,
                                         cum_mass_stars_ref)
        avg_mass_stars[i] = 0.5 * np.interp(a, scalefactor, cum_mass_stars)
        avg_mass_stars[i] += 0.5 * np.interp(a, scalefactor_ref,
                                             cum_mass_stars_ref)
        ...
        posstars = np.array(data['PartType4']['Coordinates'],
                            dtype=FloatType) / HubbleParam / 1000.
        ...
        ax[ax_row][ax_col].scatter(pos[i_select, 0],
                                   pos[i_select, 1],
                                   marker='.',
                                   s=0.01,
                                   alpha=0.5,
                                   rasterized=True)
        ax[ax_row][ax_col].scatter(posstars[:, 0],
                                   posstars[:, 1],
                                   marker='*',
                                   c='r',
                                   s=0.01,
    ...
    fig.savefig(os.path.join(simulation_directory, 'plots',
                             'dark_matter_and_stars_evolution.pdf'),
                dpi=300)
    ...
    fig.savefig(
        os.path.join(simulation_directory, 'plots', 'Mstar_redshift.pdf'))
...
if CompareToReferenceRun:
    ...
    for i, a in enumerate(scalefactor_to_probe):
        ...
        if np.max(np.abs(delta_mass_stars)) > abs_tolerance:
            print(
                'Error: stellar mass deviaties from reference more than %g (Msun/Mpc^3): '
                % abs_tolerance)
            print('redshifts:')
