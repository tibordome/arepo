...
def f_K_prime(pres, W_K, gamma_K):
    ...
    if pres > W_K[2]:  ##shock
        return np.sqrt(A / (B + pres)) * (1.0 - (pres - W_K[2]) / (2.0 *
                                                                   (B + pres)))
    else:  ## rarefaction
...
def NewtonRaphson(pres, W_L, W_R, gamma):
    ...
    while np.abs(change) > 1e-6:
        ...
        function = f_K(pres, W_L, gamma) + f_K(pres, W_R,
                                               gamma) + W_R[1] - W_L[1]
        function_prime = f_K_prime(pres, W_L, gamma) + f_K_prime(
            pres, W_R, gamma)
        ...
        if (iter > 100):
            print(
                "ERROR: NewtonRaphson: maximum number of iterations reached! Could not converge!"
            )
            break
    return pres
...
def rarefaction_fan_left(W_K, gamma, xx, time):
    ...
    rho = W_K[0] * (2.0 / gp1 + gamma_ratio / a *
                    (W_K[1] - xx / time))**(2.0 / gm1)  ## wrong?
    vel = 2.0 / gp1 * (a + gm1 / 2.0 * W_K[1] + xx / time)
    pres = W_K[2] * (2.0 / gp1 + gamma_ratio / a *
                     (W_K[1] - xx / time))**(2.0 * gamma / gm1)
...
def RiemannProblem(xx, x0, W_L, W_R, gamma, time):
    ...
    gp1 = gamma + 1.0
    gamma_ratio = gm1 / gp1
    PosOfCharacteristics = np.zeros(
        5
    )  ## 0: left most to 4: right most; 2 is rarefaction; if 0 and 1 (3 and 4) have the same value, there is a shock, otherwise rarefaction wave
    ...
    ustar = 0.5 * (W_L[1] + W_R[1]) + 0.5 * (f_K(pstar, W_R, gamma) -
                                             f_K(pstar, W_L, gamma))
    ...
    if pstar > W_L[2]:  ## shock on left
        PosOfCharacteristics[0] = W_L[1] - a_L * np.sqrt(
            gp1 / 2.0 / gamma * pstar / W_L[2] + gm1 / 2.0 / gamma)
    ...
    if pstar > W_R[2]:  ## shock on right
        PosOfCharacteristics[4] = W_R[1] + a_R * np.sqrt(
            gp1 / 2.0 / gamma * pstar / W_R[2] + gm1 / 2.0 / gamma)
    ...
    i_star_L, = np.where((xx - x0 >= PosOfCharacteristics[1])
                         & (xx - x0 < PosOfCharacteristics[2]))
    i_star_R, = np.where((xx - x0 >= PosOfCharacteristics[2])
                         & (xx - x0 < PosOfCharacteristics[3]))
    ...
    if PosOfCharacteristics[0] != PosOfCharacteristics[
            1]:  ## rarefaction wave on left
        i_fan_L, = np.where((xx - x0 >= PosOfCharacteristics[0])
                            & (xx - x0 < PosOfCharacteristics[1]))
        W[i_fan_L, :] = rarefaction_fan_left(W_L, gamma, xx[i_fan_L] - x0,
                                             time)
        W[i_star_L, 0] = (pstar / W_L[2])**(1. / gamma) * W_L[0]
    else:  ## shock on left
        W[i_star_L, 0] = W_L[0] * ((pstar / W_L[2] + gamma_ratio) /
                                   (gamma_ratio * pstar / W_L[2] + 1.0))

    W[i_star_L, 1] = ustar
    W[i_star_L, 2] = pstar

    if PosOfCharacteristics[3] != PosOfCharacteristics[
            4]:  ## rarefaction wave on right
        i_fan_R, = np.where((xx - x0 >= PosOfCharacteristics[3])
                            & (xx - x0 < PosOfCharacteristics[4]))
        W[i_fan_R, :] = rarefaction_fan_right(W_R, gamma, xx[i_fan_R] - x0,
                                              time)
        W[i_star_R, 0] = (pstar / W_R[2])**(1. / gamma) * W_R[0]
    else:  ## shock on right
        W[i_star_R, 0] = W_R[0] * ((pstar / W_R[2] + gamma_ratio) /
                                   (gamma_ratio * pstar / W_R[2] + 1.0))
