...
void dump_sink_particle_snapshot(int snapNum)
{
  ...
  sprintf(buf, "%ssink_snap_%03d", All.OutputDir, snapNum);
...
int load_sink_particle_snapshot(int snapNum)
{
  ...
  for(int i = 0; i < ScanNSinks; i++)
    mpi_printf("SINK PARTICLES: sink %d ID %d Mass %g Formation mass %g Task %d \n", i, SinkP[i].ID, SinkP[i].Mass, SinkP[i].FormationMass, SinkP[i].HomeTask);
#endif
