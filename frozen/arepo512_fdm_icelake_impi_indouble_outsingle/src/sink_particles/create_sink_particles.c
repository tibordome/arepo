...
int create_sink_particles(void)
{
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      if(P[i].Type > 0 || P[i].ID == 0 || P[i].Mass == 0)
        continue;
      if(SphP[i].Density > SinkCreationDensityCodeUnits && SphP[i].PotentialPeak == 1 && SphP[i].Density > search_in.density)
        {
  ...
  */
  if ((candidate.Density > SinkTestsGiveupDensityCodeUnits) && (environment_out.divv <= 0) &&  (environment_out.diva <= 0)) 
    {
       mpi_printf("SINK_PARTICLES: Candidate density of %g is now very dense, but converging... Forcing sink formation regardless of energy checks! \n", candidate.Density);
    }
  else
    {
