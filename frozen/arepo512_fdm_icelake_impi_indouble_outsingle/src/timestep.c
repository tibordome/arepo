          ...
          for(int isink = 0; isink < NSinksAllTasks; isink++)
            {
              ...
              vrad = csnd + fabs((SinkP[isink].Vel[0] - P[p].Vel[0])*dx + (SinkP[isink].Vel[1] 
                        - P[p].Vel[1])*dy + (SinkP[isink].Vel[2] - P[p].Vel[2])*dz) / dist;
              dist += 0.5*rad; /* add half the cell size to keep the distance reasonable! */
              dt_sink_interaction = dist / vrad;
