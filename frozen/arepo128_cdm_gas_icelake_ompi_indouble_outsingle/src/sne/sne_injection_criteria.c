...
enum SNE_type sne_sink_feedback_TIMING()
{
  ...
  for (int i=0; i<NSinksAllTasks; i++)
    {
      ...
      for (int j=0; j<SinkP[i].N_sne; j++)
        {
          /*if the time has come for the star it goes supernova */
          if (All.Time >= SinkP[i].explosion_time[j])
              return SNE_SINK; /*for now for testing we just exit as soon as we encounter a sn*/
        }
    }
#else
  terminate("If you want SN feedback from coupled with sink particles recompile the code with 
             SINK_PARTICLES and SINK_PARTICLES_FEEDBACK, otherwise change injection criterion.\n");
#endif
      ...
      for (int j=0; j<SinkP[i].N_sne; j++)
        {
          ...
          if (All.Time >= SinkP[i].explosion_time[j])
            {
              /*We distribute the supernovae around the position of the sink particle*/
              sne_pos[0] = SinkP[i].Pos[0] + gsl_ran_gaussian(sne_rng, All.SNEScatterAroundSink);
              sne_pos[1] = SinkP[i].Pos[1] + gsl_ran_gaussian(sne_rng, All.SNEScatterAroundSink);
              sne_pos[2] = SinkP[i].Pos[2] + gsl_ran_gaussian(sne_rng, All.SNEScatterAroundSink);

              /*remove the dead star from the star list associated to the sink*/
              for (int k=j; k<SinkP[i].N_sne-1; k++)
                SinkP[i].explosion_time[k] = SinkP[i].explosion_time[k+1];

              SinkP[i].N_sne -= 1;
          
              
              mpi_printf("SINK_PARTICLES_FEEDBACK EXPLOSION: we just exploded sink %d which now has still %d sne to go \n", SinkP[i].ID, SinkP[i].N_sne);
