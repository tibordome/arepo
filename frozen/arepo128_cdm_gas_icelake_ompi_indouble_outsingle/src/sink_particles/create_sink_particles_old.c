...
int create_sink_particles(void)
{
  ...
  */
          ...
          if (P[i].Potential < candidate.Potential) 
            {
              ...
              printf("SINK_PARTICLES: Candidate failed potential check Task %d Candidate task %d \n", ThisTask, candidate_task);
              printf("SINK_PARTICLES: Candidate failed potential check Task %d This pot %g and rho %g and TimeBin %d. Candidate pot %g and density %g \n", ThisTask, P[i].Potential,SphP[i].Density, P[i].TimeBinHydro, candidate.Potential, candidate.Density);
              printf("SINK_PARTICLES: Candidate failed potential check Task %d. dx %g dy %g dz %g \n", ThisTask, dx, dy, dz);
#endif
              potential_peak = 0;
              break; 
            }
  ...
  if(ThisTask == candidate_task)
    printf("SINK_PARTICLES: Considering particle %d for sink. Potential %g Density %g accels %g %g %g dist to nearest sink %g \n", P[candidate_index].ID, P[candidate_index].Potential, SphP[candidate_index].Density, P[candidate_index].GravAccel[0], P[candidate_index].GravAccel[1], P[candidate_index].GravAccel[2], sqrt(rad_sink_closest));
  ...
  */
  if ((candidate.Density > SinkTestsGiveupDensityCodeUnits) && (environment_out.divv <= 0) &&  (environment_out.diva <= 0)) 
    {
       mpi_printf("SINK_PARTICLES: Candidate density of %g is now very dense, but converging... Forcing sink formation! \n", candidate.Density);
    }
  else
    {
  ...
  */
  ...
  assign_sne(NSinksAllTasks, candidate.Mass, candidate_task);
#endif
