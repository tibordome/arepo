...
void open_image_files(char *id1, int num, FILE ** dens, FILE ** temp, FILE ** met, FILE ** vel, FILE ** mag, FILE ** vort, FILE ** phot, FILE ** chem, FILE ** denstr, FILE ** dust, FILE ** h2, FILE ** hp, FILE ** co, FILE ** chx, FILE ** ohx, FILE ** hcop, FILE ** cp, FILE ** mp, FILE ** hep)
{
  if(ThisTask != 0)
    return;
  ...
  if(chx)
    {
      sprintf(buf, "%s/xCHX_%s_%03d", All.OutputDir, id1, num);
      *chx = open_file(buf);
    }

  if(ohx)
    {
      sprintf(buf, "%s/xOHX_%s_%03d", All.OutputDir, id1, num);
      *ohx = open_file(buf);
    }

  if(hcop)
    {
      sprintf(buf, "%s/xHCOP_%s_%03d", All.OutputDir, id1, num);
      *hcop = open_file(buf);
    }

  if(cp)
    {
      sprintf(buf, "%s/xCP_%s_%03d", All.OutputDir, id1, num);
      *cp = open_file(buf);
    }

  if(mp)
    {
      sprintf(buf, "%s/xMP_%s_%03d", All.OutputDir, id1, num);
      *mp = open_file(buf);
    }

  if(hep)
    {
      sprintf(buf, "%s/xHeP_%s_%03d", All.OutputDir, id1, num);
      *hep = open_file(buf);
    }
