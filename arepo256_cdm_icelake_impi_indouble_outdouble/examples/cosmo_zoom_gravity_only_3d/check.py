#!/usr/bin/env python3
""" @package ./examples/cosmo_zoom_gravity_only_3d/check.py
...
print('cosmo_zoom_gravity_only_3d: checking simulation output in directory ' +
...
for i_file, z in enumerate(Redshifts):
    ...
    zpos = 62.
    if i_file != 0:
        SubhaloMass = np.array(data['Subhalo']['SubhaloMass'],
                               dtype=FloatType) * UnitMass / HubbleParam
        SubhaloPos = np.array(data['Subhalo']['SubhaloPos'],
                              dtype=FloatType) / HubbleParam / 1000.
        SubhaloRad = np.array(data['Subhalo']['SubhaloHalfmassRad'],
                              dtype=FloatType) / HubbleParam / 1000.
        ...
        if createReferenceSolution:
            ...
            np.savetxt(
                os.path.join(simulation_directory, 'Masses_z%.1g.txt' % z),
                SubhaloMass)
    # optional figure
    if makeplots:
        ...
        pos = np.array(data['PartType1']['Coordinates'],
                       dtype=FloatType) / HubbleParam / 1000.
        pos2 = np.array(data['PartType2']['Coordinates'],
                        dtype=FloatType) / HubbleParam / 1000.
        vel2 = np.array(data['PartType2']['Velocities'], dtype=FloatType)
        mass2 = np.array(data['PartType2']['Masses'],
                         dtype=FloatType) / HubbleParam * UnitMass
        ...
        if i_file != 0:
            ...
            print('minimum distance of contaminating (low-res) particle: %g' %
                  np.min(cont_dist))
            ...
            if len(i_issue) > 0:
                ...
                data = h5py.File(os.path.join(directory, filename), 'r')
                id2_ic = np.array(data['PartType2']['ParticleIDs'],
                                  dtype=IntType)
                pos2_ic = np.array(data['PartType2']['Coordinates'],
                                   dtype=FloatType)

                ids, i_select1, dummy = np.intersect1d(id2_ic,
                                                       id2[i_issue],
                                                       return_indices=True)
        ...
        particle_select = np.where((pos[:, 2] > zpos - 2.5)
                                   & (pos[:, 2] < zpos + 2.5))[0]
        particle2_select = np.where((pos2[:, 2] > zpos - 2.5)
                                    & (pos2[:, 2] < zpos + 2.5))[0]
        ax.scatter(pos[particle_select, 0],
                   pos[particle_select, 1],
                   marker='.',
                   s=0.05,
                   alpha=0.5)
        ax.scatter(pos2[particle2_select, 0],
                   pos2[particle2_select, 1],
                   marker='.',
                   s=0.05,
                   c='r',
                   alpha=0.5)
        ...
        if i_file != 0:
            ax.add_artist(
                plt.Circle((GrpPos[0, 0], GrpPos[0, 1]),
                           GrpR200c[0],
                           linestyle='--',
                           color='k',
                           fill=False))
            ...
            for i in np.arange(SubhaloRad.shape[0]):
                if (dist[i] < 2.0 * GrpR200c[0]):
                    ax.add_artist(
                        plt.Circle((SubhaloPos[i, 0], SubhaloPos[i, 1]),
                                   SubhaloRad[i],
                                   color='k',
                                   fill=False))
    ...
    if i_file > 3:
        ...
        if np.abs(np.average(delta)) > tolerance:
            status = 1
            print(
                'ERROR: z=%g difference in subhalo masses exceeding limits!' %
                z)
