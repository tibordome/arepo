...
def create_ics(path, filename='ics.hdf5'):
    ...
    gamma = FloatType(
        (1.0 + PolytropicIndex) / PolytropicIndex
    ...
    pos_first, pos_last = FloatType(
        0.1) + FloatType(0.5) * dx, Boxsize - FloatType(0.5) * dx

    ## set up grid
    Pos = np.zeros([NumberOfCells, 3], dtype=FloatType)
    Pos[:, 0] = np.linspace(pos_first,
                            pos_last,
                            NumberOfCells,
                            dtype=FloatType)
    Volume = FloatType(4.) * np.pi / FloatType(3.) * (
        (Pos[:, 0] + FloatType(0.5) * dx)**3 -
        (Pos[:, 0] - FloatType(0.5) * dx)**3)
    ...
    theta = np.sinc(
        Radius / Rscale /
        np.pi)  ## np.sinc(x) = np.sin(np.pi x)/(np.pi x) or 1 if x=0
    theta[theta < 1.e-12] = FloatType(1.e-12)
    K = Rscale**2 * FloatType(4.0) * np.pi * G / (
        (PolytropicIndex + FloatType(1.0)) *
        density_0**(FloatType(1.0) / PolytropicIndex - FloatType(1.0)))
    ## explicit n=1 case
...
if __name__ == '__main__':
    ...
    print(
        'examples/polytrope_1d_spherical/create.py: creating ICs in directory '
