...
print('examples/Gresho_2d/create.py: creating ICs in directory ' +
...
while d_ring * FloatType(i_ring) <= 0.71 * Boxsize:
    ...
    for i_cell in np.arange(IntType(2.0 * np.pi * i_ring)):
        ...
        phi += (2.0 * np.pi * i_ring) / IntType(
            2.0 * np.pi * i_ring) / FloatType(i_ring)
...
Vel[i_all_but_central,
    0] = RotationVelocity[i_all_but_central] * yPosFromCenter[
        i_all_but_central] / Radius[i_all_but_central]
Vel[i_all_but_central,
    1] = -RotationVelocity[i_all_but_central] * xPosFromCenter[
        i_all_but_central] / Radius[i_all_but_central]
Vel[:, 0] += velocity_0
...
Pressure[i2] = 9.0 + 12.5 * Radius[i2] * Radius[i2] - 20.0 * Radius[
