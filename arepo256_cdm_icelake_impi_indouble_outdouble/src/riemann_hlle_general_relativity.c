...
static void get_fluxes_from_state(struct state *st, double d, double sx, double sy, double sz, double t, struct fluxes *flux,
                                  double alp, double psi, double bx_turned_to_xface);
                                               ...
                                               struct fluxes *flux, struct geometry *geom, double facex, double facey, double facez)
{
  ...
  // but careful if we refine the velocity faces as coordinate velocities, for the moment this is correct since the vels contain the
  // primitive velocity and NOT the coordinate velocity
  ...
  double a_L = fmin(0., fmin(lambda_L[2], lambda_R[2]));
  double a_R = fmax(0., fmax(lambda_L[1], lambda_R[1]));

  // below we need the real signal velocities - that's important

  double sa_L = fmin(lambda_L[2], lambda_R[2]);
  double sa_R = fmax(lambda_L[1], lambda_R[1]);
  ...
  else if(comp_vel > sa_L && comp_vel < sa_R)
    {  // that means we are in the starred region
...
void get_fluxes_from_state(struct state *st, double d, double sx, double sy, double sz, double t, struct fluxes *flux, double alp,
                           double psi, double bx_turned_to_xface)
{
  //    double det = pow(psi,6.);
  //
  ...
  flux->energy = alp * (t * coord_vx + det * st->press * st->velx);  // note that there are different formulations possible -> probably
                                                                     // not a big difference if cons are consistent with prims
