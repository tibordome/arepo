 ...
 *
 * \file        src/mhd_nonideal/ohmic_diffusion.c
 * \date        10/2017
...
void face_add_ohmic_fluxes(const struct state *state_L, const struct state *state_R, const struct state *delta_time_L,
                           ...
                           double sqrtatime)
{
  double gradB, Bleft[3], Bright[3], deltaB[3];

#ifdef ONLY_OHMIC_DIFFUSION
  face_clear_fluxes(flux);
#endif

  // deltaB[0] = state_L->dcp[0] * state_L->grad->dB[0][0] + state_L->dcp[1] * state_L->grad->dB[0][1] + state_L->dcp[2] *
  // state_L->grad->dB[0][2]; deltaB[1] = state_L->dcp[0] * state_L->grad->dB[1][0] + state_L->dcp[1] * state_L->grad->dB[1][1] +
  // state_L->dcp[2] * state_L->grad->dB[1][2]; deltaB[2] = state_L->dcp[0] * state_L->grad->dB[2][0] + state_L->dcp[1] *
  // state_L->grad->dB[2][1] + state_L->dcp[2] * state_L->grad->dB[2][2];
  deltaB[0] = 0;
  deltaB[1] = 0;
  deltaB[2] = 0;
  ...
  // deltaB[0] = state_R->dcp[0] * state_R->grad->dB[0][0] + state_R->dcp[1] * state_R->grad->dB[0][1] + state_R->dcp[2] *
  // state_R->grad->dB[0][2]; deltaB[1] = state_R->dcp[0] * state_R->grad->dB[1][0] + state_R->dcp[1] * state_R->grad->dB[1][1] +
  // state_R->dcp[2] * state_R->grad->dB[1][2]; deltaB[2] = state_R->dcp[0] * state_R->grad->dB[2][0] + state_R->dcp[1] *
  // state_R->grad->dB[2][1] + state_R->dcp[2] * state_R->grad->dB[2][2];
  deltaB[0] = 0;
  deltaB[1] = 0;
  deltaB[2] = 0;
...
#ifndef OHMIC_HEATING
  double heat_L[3], heat_R[3], heat[3];
...
#ifdef MHD_CT
void face_add_ohmic_A_fluxes(int q, int qother, const struct state *st_L, const struct state *st_R, struct fluxes *flux, double atime,
                             double sqrtatime)
{
