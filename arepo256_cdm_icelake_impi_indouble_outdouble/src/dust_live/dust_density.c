          ...
          if(Todo[i] > 0)
            {
              ...
              if(DustParticle[i].NumNgb < (All.DesNumNgbDust - All.MaxNumNgbDeviationDust) ||
                 (DustParticle[i].NumNgb > (All.DesNumNgbDust + All.MaxNumNgbDeviationDust)))
                {
                  ...
                  if(DustParticle[i].NumNgb > 0)
                    {
                      DustParticle[i].Dhsmlrho *=
                          DTP(DustParticle[i].index).Hsml /
                          (NUMDIMS * DustParticle[i].NumNgb / (NORM_COEFF * pow(DTP(DustParticle[i].index).Hsml, 3)));
                  ...
                  if(DustParticle[i].NumNgb < (All.DesNumNgbDust - All.MaxNumNgbDeviationDust))
                    Left[i] = fmax(DTP(DustParticle[i].index).Hsml, Left[i]);
                  else
                    {
                      if(Right[i] != 0)
                        {
                  ...
                  if(iter >= MAXITER - 10)
                    {
                      printf("i=%d task=%d ID=%d Hsml=%g Left=%g Right=%g Ngbs=%g  Right-Left=%g\n   pos=(%g|%g|%g)\n", i, ThisTask,
                             (int)P[DustParticle[i].index].ID, DTP(DustParticle[i].index).Hsml, Left[i], Right[i],
                             (float)DustParticle[i].NumNgb, Right[i] - Left[i], P[DustParticle[i].index].Pos[0],
                             P[DustParticle[i].index].Pos[1], P[DustParticle[i].index].Pos[2]);
                      myflush(stdout);
                    }
                      ...
                      if(Right[i] == 0 && Left[i] > 0)
                        {
                          ...
                          if(fabs(DustParticle[i].NumNgb - All.DesNumNgbDust) < 0.5 * All.DesNumNgbDust)
                            {
                              fac = 1 - (DustParticle[i].NumNgb - All.DesNumNgbDust) / (NUMDIMS * DustParticle[i].NumNgb) *
                                            DustParticle[i].Dhsmlrho;
