...
#ifdef GFM
  if(P[i].Type == 4 ||
     P[i].Type == 5) /* for stars and BHs, count them like additional hydro cells because of their enrichment/accretion calculations */
    for(int bin = P[i].TimeBinGrav; bin <= All.HighestOccupiedTimeBin; bin++)
      if(domain_to_be_balanced[bin])
        w += domain_hydro_weight[bin];
#endif
...
void domain_init_sum_cost(void)
{
  ...
  for(int j = 0; j < GRAVCOSTLEVELS; j++)
...
{
  int nexport = 0, nimport = 0, task, loc_first_no;
  ...
  loc_DomainLeaveNode =
      (struct domain_cost_data *)mymalloc("loc_DomainLeaveNode", blocksize[ThisTask] * sizeof(struct domain_cost_data));
  memset(loc_DomainLeaveNode, 0, blocksize[ThisTask] * sizeof(struct domain_cost_data));

  listCost = (struct domain_cost_data *)mymalloc("listCost", NTopleaves * sizeof(struct domain_cost_data));

  int *no_place = (int *)mymalloc("no_place", NTopleaves * sizeof(int));
  memset(no_place, -1, NTopleaves * sizeof(int));

  for(int j = 0; j < NTask; j++)
    Send_count[j] = 0;

  /* find for each particle its top-leaf, and then add the associated cost with it */
      ...
      if(p < 0)
        {
          ...
          if(no < pivot_no)
            ...
            /* note: if blk = 0, then this case can not occur, since then always no < pivot_no */
            task = rmd + (no - pivot_no) / blk;
  ...
  export_node_data = (struct domain_cost_data *)mymalloc("export_node_data", nexport * sizeof(struct domain_cost_data));
  import_node_data = (struct domain_cost_data *)mymalloc("import_node_data", nimport * sizeof(struct domain_cost_data));
  ...
  for(int i = 0; i < nexport; i++)
    {
      if(listCost[i].no < pivot_no)
        ...
        /* note: if blk = 0, then this case can not occur, since then always no < pivot_no */
        task = rmd + (listCost[i].no - pivot_no) / blk;
...
void domain_combine_multipledomains(void)
{
  ...
  /* we first determine the grav-cost and hydro-cost separately for each timebin of all the domain-pieces that are available for a
  ...
  loc_bin_Cost  = (struct cost_data *)mymalloc_clear("loc_bin_Cost", sizeof(struct cost_data) * ndomains * TIMEBINS);
  glob_bin_Cost = (struct cost_data *)mymalloc_clear("glob_bin_Cost", sizeof(struct cost_data) * ndomains * TIMEBINS);
  ...
  // MPI_Allreduce(loc_bin_Cost + lowestOccupiedTimeBin * ndomains, glob_bin_Cost + lowestOccupiedTimeBin * ndomains, 2 * ndomains *
  // (All.HighestOccupiedTimeBin - loc_lowestOccupiedTimeBin + 1), MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
      ...
      if(domain_to_be_balanced[bin])
        {
          ...
          ngrav[bin] = (struct mydata *)mymalloc("ngrav[bin]", NTask * sizeof(struct mydata));

          RB_INIT(&queue_hydrocost[bin]);
          nhydro[bin] = (struct mydata *)mymalloc("nhydro[bin]", NTask * sizeof(struct mydata));
        }
    }

  RB_INIT(&queue_load);
  nload = (struct mydata *)mymalloc("nload", NTask * sizeof(struct mydata));
...
void domain_optimize_domain_to_task_mapping(void)
{
  ...
  int *count_per_task = (int *)mymalloc_clear("count_per_task", NTask * sizeof(int));
  ...
  struct domain_count_data *domain_count =
      (struct domain_count_data *)mymalloc("domain_count", NTask * sizeof(struct domain_count_data));
  ...
  int *new_task = (int *)mymalloc("new_task", NTask * sizeof(int));
  ...
  int *copy_DomainStartList = (int *)mymalloc("copy_DomainStartList", All.MultipleDomains * NTask * sizeof(int));
  int *copy_DomainEndList   = (int *)mymalloc("copy_DomainEndList", All.MultipleDomains * NTask * sizeof(int));
