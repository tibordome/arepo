...
static double getUthermEstimate(int cell)
{
  ...
  if(TimeBinSynchronized[P[cell].TimeBinHydro])
  ...
  else
    {
      double Vel[3];
      for(int k = 0; k < 3; k++)
        Vel[k] = SphP[cell].Momentum[k] / P[cell].Mass;
      Utherm = (SphP[cell].Energy / P[cell].Mass - 0.5 * (Vel[0] * Vel[0] + Vel[1] * Vel[1] + Vel[2] * Vel[2])) /
               (All.cf_atime * All.cf_atime);
#ifdef MHD
      double B[3];
      for(int k = 0; k < 3; k++)
        B[k] = SphP[cell].BConserved[k] / SphP[cell].Volume;
      ...
      if(current_ray->current_cell == -1 && SDATA(current_ray->original_cell).ShockSurface)
        {
          ...
          if(SphP[cell].Machnumber > All.CriticalMachnumber)
            {
#ifdef COSMIC_RAYS_MAGNETIC_OBLIQUITY
              acceleration_efficiency = All.AccelerationEfficiency * 0.5 *
                                        (tanh(-(current_ray->magnetic_obliquity_pre_shock - critical_obliquity) / delta) + 1);
#else
              acceleration_efficiency = All.AccelerationEfficiency;
#endif

              dt_cell =
                  (P[cell].TimeBinHydro ? (((integertime)1) << P[cell].TimeBinHydro) : 0) * All.Timebase_interval / All.cf_hubble_a;
              ...
              double dEnergy_tmp = (injection_weight(cell) - eth_tot_pre_shock) * acceleration_efficiency * dissipated_energy /
                                   total_energy / All.cf_atime; /* in physical units */

              double dEnergy = fmax(0., fmin(dEnergy_tmp, (getUthermEstimate(cell) - All.MinEgySpec) * P[cell].Mass));
              ...
              for(i = 0; i < current_ray->cell_count; i++)
                {
                  ...
                  if(task != ThisTask)
                  ...
                  else
                    {
                      ...
                      dEnergy_tmp = (injection_weight(cell) - eth_tot_pre_shock) * acceleration_efficiency * dissipated_energy /
                                    total_energy / All.cf_atime;

                      dEnergy = fmax(0., fmin(dEnergy_tmp, (getUthermEstimate(cell) - All.MinEgySpec) * P[cell].Mass));
                      lost_injected_energy += dEnergy_tmp - dEnergy;
          ...
          if(send_count[recvTask] > 0 || receive_count[recvTask] > 0)
            {
              ...
              MPI_Sendrecv(&data_export[send_offset[recvTask]], send_count[recvTask] * sizeof(struct data_exch), MPI_BYTE, recvTask, 0,
                           &data_import[receive_offset[recvTask]], receive_count[recvTask] * sizeof(struct data_exch), MPI_BYTE,
                           recvTask, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
        }
    }
  ...
  for(j = 0; j < nimport; j++)
    {
      ...
      double dEnergy_tmp = (injection_weight(data_import[j].cell) - data_import[j].tot_pre_shock) * acceleration_efficiency *
                           data_import[j].diss_per_tot_energy / All.cf_atime;
      double dEnergy = fmax(0., fmin(dEnergy_tmp, (SphP[data_import[j].cell].Utherm - All.MinEgySpec) * P[data_import[j].cell].Mass));
      lost_injected_energy += dEnergy_tmp - dEnergy;
