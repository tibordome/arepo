...
void test_cooling_metal_init(void)
{
  double meanweight;
...
void test_cooling_function(void)
{
  FILE *fp;
  ...
  XHe            = XHe_primordial +
        (XHe_solar - XHe_primordial) * MetallicityInSolar; /* this is an approximation that XHe scales linearly with metallicity */
  ...
  if(ThisTask == 0)
    {
      file_path_sprintf(buf, "%s/cooling_curve.txt", All.OutputDir);
...
#else
          metal_cooling =
              get_CoolingMetalRate(log10(AgnRadFlux), log10(MetallicityInSolar), log10(HydrogenNumberDensity), log10(temp));
#endif
#endif
#else
          metal_cooling = 0.0;
...
void test_cooling(void)
{
  ...
  double XHe            = XHe_primordial + (XHe_solar - XHe_primordial) *
                                    MetallicityInSolar; /* this is an approximation that XHe scales linearly with metallicity */
  ...
  if(ThisTask == 0)
    {
      file_path_sprintf(buf, "%s/cooling_test.txt", All.OutputDir);
