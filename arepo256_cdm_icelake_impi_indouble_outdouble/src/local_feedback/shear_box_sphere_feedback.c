...
void create_shear_box_particles(void)
{
  ...
  double SFDenThresh = All.LocalFeedbackSFDenThresh * PROTONMASS * All.UnitLength_in_cm * All.UnitLength_in_cm * All.UnitLength_in_cm /
                       All.UnitMass_in_g;
          ...
          if(rand < prob)
            {
              if(P[i].Mass < 2.0 * MinimumStarMass)
                ...
                {
                  printf(
                      "CMS_FEEDBACK: spawning star from cell; Cell: time = %f; id = %d; sfr = %f; prob = %f; mass = %f; height = %f\n",
                      All.Time, P[i].ID, sfr, prob, P[i].Mass, dz);
      ...
      if(P[i].Type == 4)
        {
          ...
          prob = 0.5 * (1.0 + erf((Age + dt - SNTimeDelay) / sqrt(2) / SNTimeSpread)) -
                 0.5 * (1.0 + erf((Age - SNTimeDelay) / sqrt(2) / SNTimeSpread));
          prob *= N_SN;
          ...
          if(rand < prob)
            {
              ...
              P[i].FeedbackDone += 1;
              printf(
                  "CMS_FEEDBACK: Feedback particle ID = %d; time = %f; rand = %f; prob = %f; N_SN = %f; Age = %f; Age-SNTD = %f; "
                  "|Age-SNTD| = %f; SNTD = %f\n",
          ...
          if(TempCellData[i].flag_feedback)
            {
              ...
              if(calc > -1)
                cell_radius = pow(TempCellData[i].NgbVolume * 3.0 / 4.0 / M_PI, 1.0 / 3.0);
              ...
              printf("CMS_FEEDBACK[%d]: id = %d; calc = %d; repeat_calc0 = %d; tot_vol = %f; Ncells = %d\n", ThisTask, P[i].ID, calc,
                     repeat_calc0, TempCellData[i].total_vol, TempCellData[i].Ncells);
                ...
                if(TempCellData[i].NgbVolume == 0.0)
                  {
                    printf("CMS_FEEDBACK[%d]: id = %d; NgbDistance = %g; NgbVolume = %g\n", ThisTask, P[i].ID,
                           TempCellData[i].NgbDistance, TempCellData[i].NgbVolume);
      ...
      if(TempCellData[i].flag_feedback)
        {
          ...
          double resid =
              sqrt(TempCellData[i].resid[0] * TempCellData[i].resid[0] + TempCellData[i].resid[1] * TempCellData[i].resid[1] +
                   TempCellData[i].resid[2] * TempCellData[i].resid[2]);
                  ...
                  if(r2 > 0.0)
                    {
                      ...
                      cell_b = (momentum[0] * pvec[0] + momentum[1] * pvec[1] + momentum[2] * pvec[2]) * fvol_cell_centered /
                               (mass + dM * fvol);
                      local_b += cell_b;
                      cell_c = 0.5 * fvol_cell_centered * fvol_cell_centered / (mass + dM * fvol);
                      local_c += cell_c;
                      // printf("CMS_FEEDBACK[%d]: Local %d: %e, %e, %e,
                      // %e\n",ThisTask,j,local_aterm1,local_b,local_c,local_tot_ekin_b); printf("CMS_FEEDBACK[%d]: Cell %d: %e, %e,
                      // %e, %e\n",ThisTask,P[j].ID,cell_aterm1,cell_b,cell_c,cell_tot_ekin_b); printf("CMS_FEEDBACK[%d]: Cell %d: %e
              ...
              if(calc == 2)
                {
                  ...
                  double pb2 = (SphP[j].Momentum[0] * SphP[j].Momentum[0] + SphP[j].Momentum[1] * SphP[j].Momentum[1] +
                  ...
                  // printf("CMS_FEEDBACK[%d]: Cell %d: %f, %f, %f %f %f, %f,
                  ...
                  pb2 = (SphP[j].Momentum[0] * SphP[j].Momentum[0] + SphP[j].Momentum[1] * SphP[j].Momentum[1] +
                         SphP[j].Momentum[2] * SphP[j].Momentum[2]);
                  ...
                  // SNEnergy*(1.0-fkin)*fvol,SNMassReturn*fvol,dkin,deltap,total_vol);
                }
            }  // end r2 < h2
        }
    }
