...
struct ShockFinderVariables SfVars;
...
static void shock_finder_warnings(void);
static void shock_finder_info(void);

void shock_finder(void)
{
...
void shock_finder_on_the_fly(void)
{
  TIMER_START(CPU_SHOCK_FINDER);
  ...
  exchange_primitive_variables_and_gradients();  // needed for calculate_divvel, also exchanges temperatures for on-the-fly shock
                                                 // finding
...
void calculate_additional_shock_data(void)
{
  int idx, i;
      ...
      if(recvTask < NTask)
        {
          if(Mesh_Send_count[recvTask] > 0 || Mesh_Recv_count[recvTask] > 0)
            {
              /* get the particles */
              MPI_Sendrecv(&tmpSDataExch[Mesh_Send_offset[recvTask]], Mesh_Send_count[recvTask] * sizeof(ShockData), MPI_BYTE,
                           recvTask, TAG_SHOCK_DATA, &SDataExch[Mesh_Recv_offset[recvTask]],
                           Mesh_Recv_count[recvTask] * sizeof(ShockData), MPI_BYTE, recvTask, TAG_SHOCK_DATA, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
            }
        }
    }
...
// allocate memory
void shock_finder_alloc_memory(void)
{
#ifndef SHOCK_FINDER_ON_THE_FLY
...
// free memory
void shock_finder_free_memory(void)
{
#ifndef SHOCK_FINDER_ON_THE_FLY
  myfree(SDataExch);
  myfree(SData);
          ...
          if(TimeBinSynchronized[P[q0].TimeBinHydro])
            {
              ...
              SDATA(q0).Divvel += facA * (c[0] * (GVelx->value0 - GVelx->value1) + c[1] * (GVely->value0 - GVely->value1) +
          ...
          if(TimeBinSynchronized[P[q1].TimeBinHydro])
            {
              ...
              SDATA(q1).Divvel += facB * (c[0] * (GVelx->value1 - GVelx->value0) + c[1] * (GVely->value1 - GVely->value0) +
...
void calculate_gradT(void)
{
  ...
  grad_min_value = (double *)mymalloc("grad_min_value", NumGas * sizeof(double));
  grad_max_value = (double *)mymalloc("grad_max_value", NumGas * sizeof(double));
...
#ifdef CHIMES
      SDATA(i).Temperature = chimes_convert_ucgs_to_temp(SphP[i].Utherm * All.UnitEnergy_in_cgs / All.UnitMass_in_g, i);
#else
#ifdef COOLING
...
#ifdef COSMIC_RAYS
      SDATA(i).CRpseudoTemperature = (SphP[i].Pressure + SphP[i].CR_Pressure) / SphP[i].Density * All.UnitEnergy_in_cgs /
                                     All.UnitMass_in_g * meanweight * PROTONMASS / BOLTZMANN;
#else
      SDATA(i).Temperature =
          SphP[i].Utherm * All.UnitEnergy_in_cgs / All.UnitMass_in_g * GAMMA_MINUS1 * meanweight * PROTONMASS / BOLTZMANN;
#endif
#endif  // CHIMES
    }
}
...
double shock_surface_area(int k)
{
  ...
  terminate(
      "termination in shock_finder.c: shock_surface_area: surface angle approximation in 2D not implemented. Use flag "
...
static inline double dcfR(double M) { return GAMMA_PLUS1 / (GAMMA_MINUS1 + 2 / (M * M)); }
...
double delta_M(double M)
{
  ...
  double result =
      2 / (GAMMA * GAMMA_MINUS1 * M * M * dcfR(M)) * ((2 * GAMMA * M * M - GAMMA_MINUS1) / GAMMA_PLUS1 - pow(dcfR(M), GAMMA));
...
double generated_thermal_energy_flux(int k)
{
  ...
  if(!SDATA(k).ShockSurface)
    ...
    {
      double gte_flux = delta_M(SphP[k].Machnumber) * 0.5 * SDATA(k).RhopreShock *
                        pow(SDATA(k).CpreShock * SphP[k].Machnumber, 3);  // rhopreshock: comoving, note: no factors for cpreshock
...
double dissipated_energy_in_erg_per_s(int k)
{
  ...
  double eflux_physical   = generated_thermal_energy_flux(k) / (All.Time * All.Time * All.Time) * All.UnitMass_in_g /
                          pow(All.UnitLength_in_cm, 3) * pow(All.UnitVelocity_in_cm_per_s, 3);
...
double calculate_machnumber_T_jump(double T_preshock, double T_postshock)
{
  ...
  solve_quadratic_equation(2 * GAMMA * (GAMMA - 1),
                           4 * GAMMA - (GAMMA - 1) * (GAMMA - 1) - (GAMMA + 1) * (GAMMA + 1) * T_postshock / T_preshock,
                           -2 * (GAMMA - 1), &solution1, &solution2);  // general gamma
 ...
 * \param rho2 the post shock density
 */
double calculate_machnumber_cosmic_rays_from_energy_equation(double pth1, double pth2, double pcr1, double pcr2, double rho1,
                                                             double rho2)
{
  assert(rho1 > 0);
  assert(rho2 > 0);
  ...
  double Msquare =
      2. * xs * xs / (xs * xs - 1.) * ((eps2 / ptot2 + 1.) * c2_squared / c1_squared / gammaEff2 - (eps1 / ptot1 + 1.) / gammaEff1);
 ...
 */
double generated_thermal_energy_flux_cosmic_rays(double eth1, double eth2, double ecr1, double ecr2, double rho1, double rho2,
                                                 double M, double c1)
{
  double xs = rho2 / rho1;
 ...
 */
double calculate_gamma_effective(double pcr, double pth) { return (All.GammaCR * pcr + GAMMA * pth) / (pcr + pth); }
 ...
 */
static void restore_pos_periodic(double pos[3], int particle_index) { periodic_wrap_point(pos, P[particle_index].Pos); }
 ...
 *  \result The index of the neighbouring cell
 */
int find_neighbouring_cell_pos_para(int k, double start_point[3], double dir[3], int previous, int task_of_previous, double *end_point,
                                    int *task, int *original_index, int *edge, int *boundary_reached)
{
  double dir_l = sqrt(dir[0] * dir[0] + dir[1] * dir[1] + dir[2] * dir[2]);
  myassert(dir_l > 0);
...
// note: the plus/minus solution of the pq-formula does not always correspond to the plus/minus solution of the abc-formula, the
// solutions may be swaped
...
static void shock_finder_info(void)
{
  mpi_printf("Total particle number: %lld\n", All.TotNumPart);
...
static void write_zoneflag_buffer(hid_t file_id, unsigned int nof_particles, int *zoneflag_buffer)
{
  ...
  // write
  status =
      my_H5Dwrite(zoneflag_dataset, H5T_NATIVE_INT, zoneflag_memspace, zoneflag_filespace, H5P_DEFAULT, zoneflag_buffer, "/Zoneflag");
...
static int get_particle_numbers(void)
{
  ...
  if(ThisTask % tasks_group == 0)  // it's a write task
    {
      if(All.NumFilesPerOutput == 1)
        {
          ...
          file_path_sprintf(filename, "%s/shocks%d_%03d.hdf5", All.OutputDir, All.ShockSubvolumeNum, RestartSnapNum);
#else
          file_path_sprintf(filename, "%s/shocks_%03d.hdf5", All.OutputDir, RestartSnapNum);
#endif
        }
      else if(NTask == All.NumFilesPerOutput)
        {
          int filenum = ThisTask;
          file_path_sprintf(filename, "%s/shocks_%03d_%03d.hdf5", All.OutputDir, RestartSnapNum, filenum);
        }
      else
        {
      ...
      SDIR(dataspace_id = my_H5Screate_simple(2, dims2d, NULL);
           ...
           status = my_H5Dclose(dataset_id, "/ShockDirection"); status = my_H5Sclose(dataspace_id, H5S_SIMPLE);)
      ...
      FANGLE(dataspace_id = my_H5Screate_simple(1, dims1d, NULL);
             ...
             status = my_H5Dclose(dataset_id, "/MaxFaceAngle"); status = my_H5Sclose(dataspace_id, H5S_SIMPLE);)
