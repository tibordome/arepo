                                               ...
                                               struct fluxes *flux)
{
  ...
  double a_L =
      fmin(lambda_L[2], lambda_R[2]);  // check that this is exactly what we need for HLLC - yes wo checking zero (that's important)
  ...
  // now we have the velocity of the contact disc. and compute the starred pressure (Mignone's error corrected, my energy formulation,
  // for left state, which should equal right state)
  ...
  if(a_L >= comp_vel)
    {  // F_L,  one could use get_fluxes here, but below we need lamstar
  ...
  if((lamstar < a_L || lamstar > a_R) && a_L < 0. && a_R > 0.)
