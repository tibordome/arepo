          ...
          if(WindParticle[i].CellID == 0)
            {
              ...
              if(iter >= MAXITER - 10)
                {
                  printf("i=%d task=%d hsml=%g nearest dist=%g pos=(%g|%g|%g)\n", i, ThisTask, STP(WindParticle[i].index).Hsml,
                         WindParticle[i].NearestDist, P[WindParticle[i].index].Pos[0], P[WindParticle[i].index].Pos[1],
                         P[WindParticle[i].index].Pos[2]);
                  myflush(stdout);
                }
              if(iter > MAXITER)
            ...
            /* note: NearestDist can be zero if we start from a snapshot, since a snapshot can contain wind and SPH particles at
               identical coordinates, since after wind particle creation only a gravity kick is applied, but no drift before the
               snapshot is written
             */
            STP(WindParticle[i].index).Hsml =
                (WindParticle[i].NearestDist > 0) ? (1.5 * WindParticle[i].NearestDist) : get_default_softening_of_particletype(4);
        }
