  ...
  for(idx = 0; idx < TimeBinsGravity.NActiveParticles; idx++)
    {
      ...
      for(k = 0; k < GFM_N_CHEM_ELEMENTS; k++)
        {
          if((k == element_index_Hydrogen) || (k == element_index_Helium) || (k == element_index_Nitrogen) ||
             (k == element_index_Neon))
            {
              continue;
            }
          ...
          double temperature_i =
              (mu * PROTONMASS) / BOLTZMANN * GAMMA_MINUS1 * utherm * All.UnitPressure_in_cgs / All.UnitDensity_in_cgs;
...
#ifdef GFM_DUST_LOWGROWTH
          // Low dust growth from McKinnon 2017
          ...
          double temperature_0 = 20.0;
          // Internal units corresponding to n_H = 100 cm^-3, assuming mu = 1.4.
          double rho_0 = (1.4 * 1.0 * PROTONMASS) / (All.UnitDensity_in_cgs * All.HubbleParam * All.HubbleParam);
          double rho_i = SphP[i].Density * All.cf_a3inv;
...
#error "This is not implemented!"
      ...
      if(SphP[i].Sfr == 0.0)
#endif
        {
          ...
          double S_i;
          double amu_masses[] = {1.0079, 4.0026, 12.0107, 14.0067, 15.9994, 20.1797, 24.305, 28.0855, 55.845};
              ...
              for(l = 0; l < GFM_N_CHEM_ELEMENTS; l++)
                {
                  ...
                  double gas_ni = SphP[i].Density * All.cf_a3inv * SphP[i].MetalsFraction[l] *
                                  (All.UnitDensity_in_cgs * All.HubbleParam * All.HubbleParam) / gas_mi;
                  double v_thr = pow(8.0 * H_s / gas_mi, 0.5);

                  double numerator =
                      (-8.0 * H_s * BOLTZMANN * T + 4.0 * BOLTZMANN * BOLTZMANN * T * T - 4.0 * H_s * gas_mi * v_thr * v_thr +
                       2.0 * BOLTZMANN * gas_mi * T * v_thr * v_thr + 0.5 * gas_mi * gas_mi * v_thr * v_thr * v_thr * v_thr);
                  double denominator = exp(0.5 * gas_mi * v_thr * v_thr / (BOLTZMANN * T)) * gas_mi * gas_mi;
                  /* Number of particles sputtered per unit time. */
                  double rate_sp = gas_ni * pow(gas_mi / (2.0 * 3.1415926 * BOLTZMANN * T), 1.5) * 4.0 *
                                   (3.1415926 * a_grain * a_grain) * S_i / H_s * BOLTZMANN * T * numerator / denominator *
                                   (2.0 * 3.1415926 / 2.0);
