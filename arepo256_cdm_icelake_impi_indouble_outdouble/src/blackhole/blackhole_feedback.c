...
#ifdef BH_THERMALFEEDBACK_ACC
  double accenergy          = 0;
  double energy_to_temp_fac = (4 / (8 - 5 * (1 - HYDROGEN_MASSFRAC))) * PROTONMASS / BOLTZMANN * GAMMA_MINUS1 * All.UnitEnergy_in_cgs /
                              All.UnitMass_in_g / (All.ReferenceGasPartMass * All.DesNumNgbBlackHole);
  double delta_temp;
  double delta_time = 0;
#endif
  ...
  for(idx = 0; idx < TimeBinsBHAccretion.NActiveParticles; idx++)
    {
      ...
      energy =
          All.BlackHoleFeedbackFactor * All.BlackHoleRadiativeEfficiency * mdot * dt * pow(CLIGHT / All.UnitVelocity_in_cm_per_s, 2);
          ...
          if(dt > 0)
            {
              ...
              for(k = 0; k < 3; k++)
                P[n].GravAccel[k] +=
                    -All.cf_atime * All.cf_atime * fac / dt * (P[n].Vel[k] - BPP(n).BH_SurroundingGasVel[k]) / All.cf_atime;
            }
        }
    }
}
