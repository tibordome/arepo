...
#if defined(MEASURE_POTMIN_AROUND_BH) || defined(BH_BONDI_CAPTURE)
  Tree_BhngbResultsImported =
      (struct bhresultsimported_data *)mymalloc("Tree_BhngbResultsImported", ncount * sizeof(struct bhresultsimported_data));
#endif
...
#error \
    "this doesn't work at present since we are not guaranteed to have access to velocities of particles in the tree if they are an imported TreePoint"
...
void blackhole_potmin_diagnostic(void)
{
  ...
  if(ThisTask == 0)
    {
      fprintf(FdBHDiag, "%g %d  %g %g %g   %12.7f %g  %g %g %g  %12.7f  %g %g %g %g  %g %g %g  %g %g %g \n", All.Time, task, my.pos[0],
              my.pos[1], my.pos[2], my.minpot, my.bh_hsml, my.bh_pos[0], my.bh_pos[1], my.bh_pos[2], my.bh_minpot, my.bh_rhotot,
              my.bh_minpotvel[0], my.bh_minpotvel[1], my.bh_minpotvel[2], my.Pos[0], my.Pos[1], my.Pos[2], my.Vel[0], my.Vel[1],
              my.Vel[2]);
      fflush(FdBHDiag);
    }
