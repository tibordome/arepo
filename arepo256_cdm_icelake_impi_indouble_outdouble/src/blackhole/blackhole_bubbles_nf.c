  ...
  if(total_num_activebh > 0)
    {
      ...
      radio_bubble = (struct localbubble_data *)mymalloc("radio_bubble", num_bubbles * sizeof(struct localbubble_data));
      ...
      for(i = 0; i < num_activebh; i++)
        {
          ...
          if(mtot * radio_bubble[l].u > BPP(radio_bubble[l].pindex).BH_RadioEgyFeedback)
                  ...
                  if(BPP(radio_bubble[l + rep].pindex).BH_RadioEgyFeedback > egy_thresh && radio_bubble[l + rep].mass > 0)
                    {
                      ...
                      BPP(radio_bubble[l + rep].pindex).BH_MPB_CumEgyLow += egy;
            ...
            {
              /* There is so much energy that even placing MAX_BUBBLES_PER_ACTIVE_HOLE bubbles with the nominal energy increase will
               * not get rid off it all. In this situation, we increase the amount of energy that is given per bubble.
               */
