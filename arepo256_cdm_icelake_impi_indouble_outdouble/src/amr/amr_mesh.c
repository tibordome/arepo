...
int amr_copy_dp(int i, tessellation* T)
{
  ...
  memcpy(T->DP + new_Ndp, T->DP + i, sizeof(point));

  int j;
  for(j = 0; j < 2 * NUMDIMS; j++)
    {
      T->DP[new_Ndp].neighbors[j] = -1;
    }

  if(T->DP[new_Ndp].task == ThisTask)
    {
      T->DP[new_Ndp].index += NumGas;
    }
