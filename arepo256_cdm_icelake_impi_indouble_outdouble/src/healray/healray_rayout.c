      ...
      for(i = 0; i < HRD.RayNumTrace; i++)
        {
          ...
          if(HRD.RayDebugMode)
            mpi_printf("id: %ld %d %ld %d %d %ld\n", tot_num_rays, HRD.RayNumTrace, target_list[i], task_list[i], idx_list[i],
                       HRD.TraceList[i]);
        }
  ...
  if(HRD.RayNumTrace)
    {
      ...
      mpi_distribute_items_to_tasks(frac, offsetof(struct frac_struct, task), &nfrac, &max_nfrac, sizeof(struct frac_struct),
                                    TAG_DENS_A);
          ...
          for(i = 0; i < HRD.RayNumTrace; i++)
            {
              ...
              if(HRD.RayDebugMode)
                mpi_printf("trace: %d %ld %d %g\n", i, HRD.TraceList[i], last_iter[i],
                           energy_frac[last_iter[i] * HRD.RayNumTrace + i]);
            }
