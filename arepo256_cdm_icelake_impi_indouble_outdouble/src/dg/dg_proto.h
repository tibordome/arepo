...
int dg_face_get_state(tessellation* T, CBV(a), int interface, int q, int p, int i, struct state* st,
                      double (*P_X)[NOF_BASE_FUNCTIONS]);
...
// void dg_compute_step(void);
void print_time_integration_info(void);
void ini_rk_coefficients(void);
 ...
 * dg_projection.c
 */
void ini_higher_order_quadrature(void);
...
void print_outer_quad_points_weights(void);
void print_weights(int cell);
void print_imported_weights(void);
void assert_imported_weights_positivity(const char* error);
void print_timebins(void);
