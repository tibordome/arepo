...
{  // todo: movable
  ...
  Outer_quad_points_x =
      (double(*)[NOF_OUTER_QUAD_POINTS])mymalloc("Outer_quad_points_x", 2 * NUMDIMS * Nof_outer_quad_points * sizeof(double));
  Outer_quad_points_y =
      (double(*)[NOF_OUTER_QUAD_POINTS])mymalloc("Outer_quad_points_y", 2 * NUMDIMS * Nof_outer_quad_points * sizeof(double));
  Outer_quad_points_z =
      (double(*)[NOF_OUTER_QUAD_POINTS])mymalloc("Outer_quad_points_z", 2 * NUMDIMS * Nof_outer_quad_points * sizeof(double));
  Outer_quad_points_weights =
      (double(*)[NOF_OUTER_QUAD_POINTS])mymalloc("Outer_quad_points_weights", 2 * NUMDIMS * Nof_outer_quad_points * sizeof(double));

  Inner_base_values =
      (double(*)[NOF_BASE_FUNCTIONS])mymalloc("Inner_base_values", Nof_base_functions * Nof_inner_quad_points * sizeof(double));
  Inner_base_dx_values =
      (double(*)[NOF_BASE_FUNCTIONS])mymalloc("Inner_base_dx_values", Nof_base_functions * Nof_inner_quad_points * sizeof(double));
  Inner_base_dy_values =
      (double(*)[NOF_BASE_FUNCTIONS])mymalloc("Inner_base_dy_values", Nof_base_functions * Nof_inner_quad_points * sizeof(double));
  Inner_base_dz_values =
      (double(*)[NOF_BASE_FUNCTIONS])mymalloc("Inner_base_dz_values", Nof_base_functions * Nof_inner_quad_points * sizeof(double));

  Outer_base_values = (double(*)[NOF_OUTER_QUAD_POINTS][NOF_BASE_FUNCTIONS])mymalloc(
      "Outer_base_values", Nof_base_functions * 2 * NUMDIMS * Nof_outer_quad_points * sizeof(double));
  Outer_fine_base_values = (double(*)[NOF_OUTER_FINE_QUAD_POINTS][NOF_BASE_FUNCTIONS])mymalloc(
      "Outer_fine_base_values", NOF_INTERFACES * NOF_OUTER_FINE_QUAD_POINTS * Nof_base_functions * sizeof(double));
 ...
 */
void dg_set_initial_conditions(void)
{
...
#error Not yet implemented in 3D!
  ...
  Aexch =
      (double(*)[NOF_BASE_FUNCTIONS][5])mymalloc_movable(&(Aexch), "Aexch", Mesh_nimport * Nof_base_functions * 5 * sizeof(double));
  double(*a1)[NOF_BASE_FUNCTIONS][5] =
      (double(*)[NOF_BASE_FUNCTIONS][5])mymalloc("a1", NumGas * Nof_base_functions * 5 * sizeof(double));
...
void dg_compute_step(void)
{
  ...
  Aexch =
      (double(*)[NOF_BASE_FUNCTIONS][5])mymalloc_movable(&(Aexch), "Aexch", Mesh_nimport * Nof_base_functions * 5 * sizeof(double));
  double(*w0)[NOF_BASE_FUNCTIONS][5] =
      (double(*)[NOF_BASE_FUNCTIONS][5])mymalloc("w0", NumGas * Nof_base_functions * 5 * sizeof(double));
  double(*a1)[NOF_BASE_FUNCTIONS][5] =
      (double(*)[NOF_BASE_FUNCTIONS][5])mymalloc("a1", NumGas * Nof_base_functions * 5 * sizeof(double));
  double(*a2)[NOF_BASE_FUNCTIONS][5] =
      (double(*)[NOF_BASE_FUNCTIONS][5])mymalloc("a2", NumGas * Nof_base_functions * 5 * sizeof(double));
      ...
      for(l = 0; l < Nof_base_functions; l++)
        {
          for(k = 0; k < 5; k++)
            {
              a1[i][l][k] = (1 - factor) * w0[i][l][k] + factor * a2[i][l][k] -
                            0.5 / All.DG_RK2_alpha * dt * R_inner[l][k];  // a_1 = (1-(1-1/(2alpha))/alpha) * w_0 +
                                                                          // (1-1/(2alpha))/alpha * a2 - 1/(2alpha) * dt * R_inner(a2)
            }
        }
    }
      ...
      for(l = 0; l < Nof_base_functions; l++)
        {
          for(k = 0; k < 5; k++)
            {
              a1[i][l][k] = 3. / 4. * w0[i][l][k] + 1. / 4. * a2[i][l][k] -
                            1. / 4. * dt * R_inner[l][k];  //  a_1 = 3./4. * w_0 + 1./4. * a2  - 1./4. * dt * R_inner(a2)
            }
        }
    }
      ...
      for(l = 0; l < Nof_base_functions; l++)
        {
          for(k = 0; k < 5; k++)
            {
              a2[i][l][k] = 1. / 3. * w0[i][l][k] + 2. / 3. * a1[i][l][k] -
                            2. / 3. * dt * R_inner[l][k];  //  a_2 = 1./3. * w_0 + 2./3. * a_1 - 2./3. * dt * R_inner(a1)
            }
        }
    }
  ...
  for(q = 0; q < Nof_inner_quad_points; q++)
    {
      ...
      DG3D(flux3(w, f3);)
#ifdef DG_EXTERNAL_ACCELERATION
      cell_coords_to_lab_coords(cell, GET_Inner_quad_points_x(q), GET_Inner_quad_points_y(q), GET_Inner_quad_points_z(q), &xlab, &ylab,
                                &zlab);
...
#ifdef DG_TURBULENCE
      SphP[cell].EgyDrive += pv[RHO] * (pv[VX] * acc[0] + pv[VY] * acc[1] + pv[VZ] * acc[2]) * rk_weight * dt * SphP[cell].Volume *
                             norm_fac_grav * GET_Inner_quad_points_weights(q);
#endif
#endif
      ...
      for(l = 0; l < Nof_base_functions; l++)
        {
          ...
          src_term[W_E]   = pv[RHO] * (pv[VX] * acc[0] + pv[VY] * acc[1] + pv[VZ] * acc[2]) * GET_Inner_base_values(q, l) *
                          GET_Inner_quad_points_weights(q);
 ...
 */
void calc_state_at_outer_quad_point(double a_cell[NOF_BASE_FUNCTIONS][5], int interface, int quad_point, double w[5],
                                    double (*P_X)[NOF_BASE_FUNCTIONS])
{
  int l, j;
...
void flux3(double w[5], double f[5]) { return; }
...
void exchange_weights(CBV(a))
{
  ...
  TIMER_START(CPU_DG_EXCHANGE);
  tmpAexch = (double(*)[(NOF_BASE_FUNCTIONS)][5])mymalloc("tmpAexch", Mesh_nexport * Nof_base_functions * 5 * sizeof(double));
      ...
      if(recvTask < NTask)
        {
          if(Mesh_Send_count[recvTask] > 0 || Mesh_Recv_count[recvTask] > 0)
            {
              /* exchange the data */
              MPI_Sendrecv(&tmpAexch[Mesh_Send_offset[recvTask]], Mesh_Send_count[recvTask] * Nof_base_functions * 5 * sizeof(double),
                           MPI_BYTE, recvTask, 0, &Aexch[Mesh_Recv_offset[recvTask]],
                           Mesh_Recv_count[recvTask] * Nof_base_functions * 5 * sizeof(double), MPI_BYTE, recvTask, 0, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
            }
        }
    }
 ...
 */
void dg_update_conserved_variables(void)
{
  int i;
