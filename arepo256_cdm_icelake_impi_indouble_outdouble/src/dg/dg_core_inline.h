...
inline static void lab_coords_to_cell_coords(int cell, double x_lab, double y_lab, double z_lab, double *x_cell, double *y_cell,
                                             double *z_cell);
inline static void cell_coords_to_lab_coords(int cell, double x_cell, double y_cell, double z_cell, double *x_lab, double *y_lab,
                                             double *z_lab);
 ...
 */
inline static double dg_sound_speed(int cell) { return sqrt(GAMMA * SphP[cell].Pressure / SphP[cell].Density); }
...
inline static double ideal_gas_pressure(double density, double utherm) { return density * utherm * (GAMMA - 1); }

inline static double ideal_gas_density(double pressure, double utherm) { return pressure / (utherm * (GAMMA - 1)); }

inline static double ideal_gas_utherm(double density, double pressure) { return pressure / (density * (GAMMA - 1)); }
...
#ifdef OLD_VERSION
  pv[PRES] =
      (w[W_E] - 0.5 * (w[W_PX] * w[W_PX] / w[W_RHO] + w[W_PY] * w[W_PY] / w[W_RHO] + w[W_PZ] * w[W_PZ] / w[W_RHO])) * (GAMMA - 1);
#else
