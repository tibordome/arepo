 ...
 * \date        MM/YYYY
 * \author
 * \brief       routines for adding the effective mass to the non-periodic PM grid
 * \details
 *
 *
...
void modgrav_add_amr_nodes_to_nonperiodic_grid(int grnr, fft_real *rhogrid, double to_slab_fac, fft_plan myplan, int fftsize)
{
  ...
  double overlap[3];
  double pm_cell_size = All.TotalMeshSize[grnr] / GRID_NP;
  ...
  to_slab_fac = GRID_NP / All.TotalMeshSize[grnr];
  ...
  for(no = All.MaxPart; no < All.MaxPart + Tree_NumNodes; no++)
    if(Nodes[no].mg_bitflag.amr_node == 1)
      {
        if(no < Tree_FirstNonTopLevelNode) /* top nodes are also exported, this simplifies the code and should not have a big impact on
                                              performance */
        ...
        if(Nodes[no].center[0] + 0.5 * Nodes[no].len < All.Corner[grnr][0] ||
           Nodes[no].center[0] - 0.5 * Nodes[no].len >= All.UpperCorner[grnr][0])
          continue;
        if(Nodes[no].center[1] + 0.5 * Nodes[no].len < All.Corner[grnr][1] ||
           Nodes[no].center[1] - 0.5 * Nodes[no].len >= All.UpperCorner[grnr][1])
          continue;
        if(Nodes[no].center[2] + 0.5 * Nodes[no].len < All.Corner[grnr][2] ||
           Nodes[no].center[2] - 0.5 * Nodes[no].len >= All.UpperCorner[grnr][2])
          continue;

        minslab = (int)floor(to_slab_fac * (Nodes[no].center[0] - All.Corner[grnr][0] - 0.5 * Nodes[no].len) + 0.5);
        if(minslab < 0)
          minslab = 0;
        else if(minslab >= GRID_NP)
          minslab = GRID_NP - 1;
        ...
        if(minslab < 0 || maxslab >= GRID_NP || minslab > maxslab)
          {
  ...
  for(i = 0; i < num_nodes_to_exchange; ++i)
    {
      ...
      for(ix[0] = minx[0]; ix[0] <= maxx[0]; ++ix[0])
        if(ix[0] >= myplan.first_slab_x_of_task[ThisTask] &&
           ix[0] < myplan.first_slab_x_of_task[ThisTask] + myplan.slabs_x_per_task[ThisTask])
            ...
            for(ix[2] = minx[2]; ix[2] <= maxx[2]; ++ix[2])
              {
                ...
                offset = ((large_array_offset)GRID2_NP) * (GRID_NP * (ix[0] - myplan.first_slab_x_of_task[ThisTask]) + ix[1]) + ix[2];
                if(offset < 0 || offset > fftsize)
                  {
                    printf("Task %i, offset %ui, fftsize %i, first_slab_of_task[ThisTask] %i, actual_x = [%i, %i, %i]\n", ThisTask,
                           offset, fftsize, myplan.first_slab_x_of_task[ThisTask], ix[0], ix[1], ix[2]);
      ...
      if(fabs(mass_for_node) > fabs(1.000001 * exchange_nodes[i].eff_mass))
        {
          ...
          printf("first slab %i, last slab %i \n", myplan.first_slab_x_of_task[ThisTask],
                 myplan.first_slab_x_of_task[ThisTask] + myplan.slabs_x_per_task[ThisTask] - 1);
