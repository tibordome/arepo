...
int FindTriangle(tessellation *T, int tt, int pp, int *degnerate_flag, int *nexttetra)
{
  ...
  if(pp0 == DPinfinity || pp1 == DPinfinity || pp2 == DPinfinity)
    terminate("we are in a triangle with an infinity point. tetra=%d  p=(%g|%g)\n", tt, p->x, p->y);
  ...
  if(status < 0)
    {
      ...
      if(ivol <= 0)
        terminate("flat or negatively oriented triangle found (ivol=%d)\n", ivol);
    }
  ...
  if(count_zeros >= 2)
    {
      ...
      printf("Ngas=%d\n", NumGas);
      printf("xyz, p=%d: (%g|%g)  index=%d task=%d ID=%" MYIDTYPE_PRI "  flags\n", (int)(pp0), p0->x, p0->y, p0->index, p0->task,
...
#ifdef AXISYMMETRY
/*! \brief Comuptes geometric correction factors when using axisymmetry.
 *
 *  Correction factors for cell volume and face areas.
 *
 *  \param[in, out] T pointer to tessellation.
 *
 *  \return void
  ...
  for(i = 0, nnew = ntri; i < ntri; i++)
    {
      if(trilist[i].owner < 0 || trilist[i].owner >= Mesh.Ndp)
        terminate("i=%d trilist[i].owner=%d\n", i, trilist[i].owner);
...
void write_voronoi_mesh(tessellation *T, char *fname, int writeTask, int lastTask)
{
  ...
  EdgeList     = (int *)mymalloc("EdgeList", MaxNel * sizeof(int));
  Nedges       = (int *)mymalloc("Nedges", NumGas * sizeof(int));
  NedgesOffset = (int *)mymalloc("NedgesOffset", NumGas * sizeof(int));
  whichtetra   = (int *)mymalloc("whichtetra", NumGas * sizeof(int));
  xyz_edges    = (float *)mymalloc("xyz_edges", T->Ndt * DIMS * sizeof(float));
  ngas_list    = (int *)mymalloc("ngas_list", sizeof(int) * NTask);
  nel_list     = (int *)mymalloc("nel_list", sizeof(int) * NTask);
  ndt_list     = (int *)mymalloc("ndt_list", sizeof(int) * NTask);
  ...
  if(ThisTask == writeTask)
    {
      ...
      for(task = writeTask + 1; task <= lastTask; task++)
        {
          tmp = (int *)mymalloc("tmp", sizeof(int) * ngas_list[task]);
      ...
      for(task = writeTask + 1; task <= lastTask; task++)
        {
          tmp = (int *)mymalloc("tmp", sizeof(int) * nel_list[task]);
      ...
      for(task = writeTask + 1; task <= lastTask; task++)
        {
          float *ftmp = mynew("ftmp", float, DIMS *ndt_list[task]);
          MPI_Recv(ftmp, sizeof(float) * DIMS * ndt_list[task], MPI_BYTE, task, TAG_N + 5, MPI_COMM_WORLD, &status);
          my_fwrite(ftmp, sizeof(float), DIMS * ndt_list[task], fd);
          myfree(ftmp);
        }
...
/*! \brief File output of an image of density and velocity fields
 *
 *  Writes images of density and velocity with a given number of pixels,
 *  specified by VORONOI_FIELD_DUMP_PIXELS_X and VORONOI_FIELD_DUMP_PIXELS_Y in
 *  binary files.
 *
 *  \param[in] num Index of file that is to be written
 *  \param[in] gradients_flag Flag whether to include gradients or not in dump
 */
void do_special_dump(int num, int gradients_flag)
{
  ...
  if(gradients_flag == 1)
    ...
    file_path_sprintf(buf, "%s/density_field_nograds_%03d", All.OutputDir, num);
  else
  ...
  float *dens    = (float *)mymalloc_clear("dens", pixels_x * pixels_y * sizeof(float));
  float *denssum = (float *)mymalloc_clear("denssum", pixels_x * pixels_y * sizeof(float));

#ifdef TRACER_FIELD
  float *tracer    = (float *)mymalloc_clear("tracer", pixels_x * pixels_y * sizeof(float));
  float *tracersum = (float *)mymalloc_clear("tracersum", pixels_x * pixels_y * sizeof(float));
#endif

#ifdef TRACER_MC
  float *tracerMC    = (float *)mymalloc_clear("tracerMC", pixels_x * pixels_y * sizeof(float));
  float *tracerMCsum = (float *)mymalloc_clear("tracerMCsum", pixels_x * pixels_y * sizeof(float));
#endif

#ifdef VORONOI_VELOCITY_FIELD_2D
  float *velx    = (float *)mymalloc_clear("velx", pixels_x * pixels_y * sizeof(float));
  float *velxsum = (float *)mymalloc_clear("velxsum", pixels_x * pixels_y * sizeof(float));
  float *vely    = (float *)mymalloc_clear("vely", pixels_x * pixels_y * sizeof(float));
  float *velysum = (float *)mymalloc_clear("velysum", pixels_x * pixels_y * sizeof(float));
#endif
  ...
  mesh_search_data *searchdata = (mesh_search_data *)mymalloc("searchdata", npixels * sizeof(mesh_search_data));
  ...
  for(int pix = 0; pix < npixels; pix++)
    {
      ...
      searchdata[pix].Pos[0] = WRAP_X(searchdata[pix].Pos[0]);
#endif
#ifdef VORONOI_FIELD_COMPENSATE_VY
      searchdata[pix].Pos[1] += All.Time * VORONOI_FIELD_COMPENSATE_VY;
      searchdata[pix].Pos[1] = WRAP_Y(searchdata[pix].Pos[1]);
#endif
    }
  ...
  mesh_search_data *send_searchdata = (mesh_search_data *)mymalloc("send_searchdata", npixels * sizeof(mesh_search_data));
  mesh_search_data *recv_searchdata = (mesh_search_data *)mymalloc("recv_searchdata", nimport * sizeof(mesh_search_data));
      ...
      if(recvTask < NTask)
        {
          if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
            MPI_Sendrecv(&send_searchdata[Send_offset[recvTask]], Send_count[recvTask] * sizeof(mesh_search_data), MPI_BYTE, recvTask,
                         TAG_DENS_A, &recv_searchdata[Recv_offset[recvTask]], Recv_count[recvTask] * sizeof(mesh_search_data),
  ...
  for(int i = 0; i < nimport; i++)
    {
      ...
      double dx = nearest_x(recv_searchdata[i].Pos[0] - SphP[li].Center[0]);
      double dy = nearest_y(recv_searchdata[i].Pos[1] - SphP[li].Center[1]);
...
#endif
#ifdef TRACER_MC
          tracerMC[pp] = (SphP[li].Density + SphP[li].Grad.drho[0] * dx + SphP[li].Grad.drho[1] * dy) * get_number_of_tracers(li) *
                         All.ReferenceTracerMCMass / P[li].Mass;
#endif
#ifdef VORONOI_VELOCITY_FIELD_2D
