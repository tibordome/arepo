    ...
    if(P[i].Type == 0)
      {
        ...
        if(calculated_flag == 0 && nGamma < 1.0e-2 * nHI) /* here photoionization cannot change the neutral fraction much */
          {
            ...
            if(fac > 0)
              nGamma_new =
                  (-(1 + fac * (1 - nHII - nGamma)) + sqrt(pow(1 + fac * (1 - nHII - nGamma), 2) + 4 * fac * nGamma)) / (2 * fac);
            else
              nGamma_new = nGamma;
        ...
        nHeII /= 1.0 + F * SphP[i].n_elec + D * SphP[i].n_elec + J * SphP[i].n_elec +
                 ((F * SphP[i].n_elec - E * SphP[i].n_elec) / (1.0 + E * SphP[i].n_elec) * D * SphP[i].n_elec);
