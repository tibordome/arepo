...
/*function to lorentz boost the RT variables into the reference frame of the cell*/
  ...
  for(int num1 = 0; num1 < MRT_BINS; num1++)
    {
      ...
      double modF =
          sqrt(sphp[i].Cons_RT_F[num1][0] * sphp[i].Cons_RT_F[num1][0] + sphp[i].Cons_RT_F[num1][1] * sphp[i].Cons_RT_F[num1][1] +
               sphp[i].Cons_RT_F[num1][2] * sphp[i].Cons_RT_F[num1][2]);
      ...
      for(int j = 0; j < 3; j++)
        {
          for(int k = 0; k < 3; k++)
            {
              if(j == k)
                pt[j][k] = 1.0 + ((1.0 - chi) / 2.0) +
                           ((3.0 * chi - 1.0) / 2.0) * n[j] *
                               n[k]; /*Addition 1.0 + comes due to the fact that you have I+D in transformation*/
              else
      ...
      double Ec = sphp[i].Cons_DensPhot[num1] -
                  2.0 * (vx * sphp[i].Cons_RT_F[num1][0] + vy * sphp[i].Cons_RT_F[num1][1] + vz * sphp[i].Cons_RT_F[num1][2]) /
                      c_internal_units / cspeed;
      ...
      if(isnan(sphp[i].Cons_RT_F[num1][0]) || isnan(sphp[i].Cons_DensPhot[num1]))
        terminate(
            "NAN ||| vx = %g \t vy = %g \t vz = %g \t pt = %g %g %g %g %g %g %g %g %g \n modF = %g, f = %g, chi = %g, cons_densphot = "
            "%g \n c_int = %g, c_tot = %g\n",
            vx, vy, vz, pt[0][0], pt[0][1], pt[0][2], pt[1][0], pt[1][1], pt[1][2], pt[2][0], pt[2][1], pt[2][2], modF, f, chi,
            sphp[i].Cons_DensPhot[num1], c_internal_units, cspeed);
    }
}

/* Add the source terms that arise due to the solving the RT equations in the comoving frame */
void do_comoving_frame_source_terms(void)
{
  mpi_printf_rt(0, "RT: Adding the comoving frame source terms...\n");
  int idx, i;
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      /* factor of 0.5 included because of spilt for RK time integration */
