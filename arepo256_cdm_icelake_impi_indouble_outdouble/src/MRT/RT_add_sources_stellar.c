...
void find_cells_photon_dump(int npart)
{
          ...
          if(Todo[i] > 0)
            {
              ...
              if(StarParticle[i].NumNgb < (All.DesNumNgb - All.MaxNumNgbDeviation) ||
                 (StarParticle[i].NumNgb > (All.DesNumNgb + All.MaxNumNgbDeviation)))
                {
                      ...
                      if(Right[i] == 0 && Left[i] > 0)
                        {
                          ...
                          if(fabs(StarParticle[i].NumNgb - All.DesNumNgb) < 0.5 * All.DesNumNgb)
                            {
                              fac = 1 - (StarParticle[i].NumNgb - All.DesNumNgb) / (NUMDIMS * StarParticle[i].NumNgb) *
                                            StarParticle[i].Dhsmlrho;
      ...
      if(ntot > 0)
        {
          ...
          if(iter > 0)
            mpi_printf_rt(
                0, "RT add sources: star ngb iteration %3d: need to repeat for %12lld particles. (previous iteration took %g sec)\n",
  ...
  mpi_printf_rt(0, "RT: active stars %lld, finding Hsml took = %g sec\n", npartall, timediff(t0, t1));
}
