...
#ifdef SINKS
void domain_resize_storage_sinks(int count_get_sinks)
{
  int max_sinksload, sinksload = NumSinks + count_get_sinks;
  MPI_Allreduce(&sinksload, &max_sinksload, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);

  if(max_sinksload >= All.MaxPartSinks - 0.5 * ALLOC_SINKS_ROOM * (All.TotNumGas / NTask))
    {
      All.MaxPartSinks = max_sinksload + ALLOC_SINKS_ROOM * (All.TotNumGas / NTask);
      reallocate_memory_maxpartsinks();
    }
}
#endif
...
#ifdef TRACER_MC
/*! \brief Changes memory allocation for tracer data if necessary
 *
 *  If the memory usage due to a net import or export of tracers changes
 *  above a certain tolerance, the tracer structures need to be reallocated.
 *
 *  \param[in] count_get_tracer How many particles are imported?
 *
 *  \return void
 */
...
void domain_exchange(void)
{
...
#endif
#ifdef SINKS
  int count_togo_sinks = 0, count_get_sinks = 0;
  int *count_sinks, *offset_sinks;
  int *count_recv_sinks, *offset_recv_sinks;
  struct sink_particle_data *SinksBuf;
  int *sinksbuf_offset, *sinksbuf_recv_offset;
#endif
#ifdef DUST_LIVE
...
#endif
#ifdef SINKS
  count_sinks          = (int *)mymalloc_movable(&count_sinks, "count_sinks", NTask * sizeof(int));
  offset_sinks         = (int *)mymalloc_movable(&offset_sinks, "offset_sinks", NTask * sizeof(int));
  count_recv_sinks     = (int *)mymalloc_movable(&count_recv_sinks, "count_recv_sinks", NTask * sizeof(int));
  offset_recv_sinks    = (int *)mymalloc_movable(&offset_recv_sinks, "offset_recv_sinks", NTask * sizeof(int));
  sinksbuf_offset      = (int *)mymalloc_movable(&sinksbuf_offset, "sinkbuf_offset", NTask * sizeof(int));
  sinksbuf_recv_offset = (int *)mymalloc_movable(&sinksbuf_recv_offset, "sinksbuf_recv_offset", NTask * sizeof(int));
#endif
#ifdef DUST_LIVE
...
#ifdef SINKS
  offset_sinks[0] = prec_offset;
  for(i = 1; i < NTask; i++)
    {
      offset_sinks[i] = offset_sinks[i - 1] + toGoSinks[i - 1];
      decrease[i] += toGoSinks[i - 1];
    }
  prec_offset = offset_sinks[NTask - 1] + toGoSinks[NTask - 1];
#endif
...
#endif
#ifdef SINKS
      count_togo_sinks += toGoSinks[i];
      count_get_sinks += toGetSinks[i];
#endif
#ifdef DUST_LIVE
...
#ifdef TRACER_MC
  tracerBuf =
      (struct tracer_linked_list *)mymalloc_movable(&tracerBuf, "tracerBuf", count_togo_tracer * sizeof(struct tracer_linked_list));
...
#ifdef SINKS
  SinksBuf =
      (struct sink_particle_data *)mymalloc_movable(&SinksBuf, "SinksBuf", count_togo_sinks * sizeof(struct sink_particle_data));
#endif
#ifdef DUST_LIVE
          ...
          if(P[n].Type == 0)
            {
              ...
              for(i = 0; i < ChimesGlobalVars.totalNumberOfSpecies; i++)
                sphAbundancesBuf[((offset_sph[target] + count_sph[target]) * ChimesGlobalVars.totalNumberOfSpecies) + i] =
                    SphP[n].ChimesGasVars.abundances[i];
            ...
            {
              partBuf[offset_sinks[target] + count_sinks[target]] = P[n];
              /*
              #ifdef TRACER_MC
                            partBuf[offset_BHs[target] + count_BHs[target]].TracerHead = count_tracer[target];
              #endif
              */
              keyBuf[offset_sinks[target] + count_sinks[target]]                     = Key[n];
              SinksBuf[offset_sinks[target] - offset_sinks[0] + count_sinks[target]] = SinkP[P[n].AuxDataID];
              count_sinks[target]++;
            }
#endif
#ifdef DUST_LIVE
          ...
          if(P[n].Type == 0)
            {
              ...
              if(n < NumGas - 1)
                {
                  SphP[n].ChimesGasVars.index = n;
                  chimes_move_abundances(n, NumGas - 1);
            ...
            {
              SinkP[P[n].AuxDataID]                = SinkP[NumSinks - 1];
              P[SinkP[NumSinks - 1].PID].AuxDataID = P[n].AuxDataID;
...
#ifdef SINKS
  domain_resize_storage_sinks(count_get_sinks);
#endif
...
#ifdef TRACER_MC
  mpi_printf("DOMAIN: Resizing tracer storage\n");
...
#endif
#ifdef SINKS
      count_recv_sinks[i] = toGetSinks[i];
      count_recv[i] -= toGetSinks[i];
#endif
#ifdef DUST_LIVE
...
#ifdef SINKS
  offset_recv_sinks[0] = prec_count;
  for(i = 1; i < NTask; i++)
    offset_recv_sinks[i] = offset_recv_sinks[i - 1] + count_recv_sinks[i - 1];
  prec_count += count_get_sinks;

  for(target = 0; target < NTask; target++)
    {
      sinksbuf_offset[target]      = offset_sinks[target] - offset_sph[NTask - 1] - count_sph[NTask - 1];
      sinksbuf_recv_offset[target] = NumSinks + offset_recv_sinks[target] - offset_recv_sph[NTask - 1] - count_recv_sph[NTask - 1];
    }
#endif
      ...
      if(target < NTask)
        {
...
#ifdef CHIMES
              MPI_Sendrecv(
                  sphAbundancesBuf + (offset_sph[target] * ChimesGlobalVars.totalNumberOfSpecies),
                  ...
                  count_recv_sph[target] * ChimesGlobalVars.totalNumberOfSpecies, MPI_DOUBLE, target, TAG_ABUNDATA, MPI_COMM_WORLD,
                  MPI_STATUS_IGNORE);
#endif  // CHIMES
          ...
          if(count_tracer[target] > 0 || count_recv_tracer[target] > 0)
            {
              MPI_Sendrecv(tracerBuf + offset_tracer[target], count_tracer[target] * sizeof(struct tracer_linked_list), MPI_BYTE,
                           target, TAG_TRACERDATA, tracerRecvBuf + offset_recv_tracer[target],
                           count_recv_tracer[target] * sizeof(struct tracer_linked_list), MPI_BYTE, target, TAG_TRACERDATA,
                           MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
#endif
#if defined(GFM) || defined(SFR_MCS)
          if(count_star[target] > 0 || count_recv_star[target] > 0)
            {
              MPI_Sendrecv(partBuf + offset_star[target], count_star[target] * sizeof(struct particle_data), MPI_BYTE, target,
                           TAG_PDATA_STAR, P + offset_recv_star[target], count_recv_star[target] * sizeof(struct particle_data),
                           MPI_BYTE, target, TAG_PDATA_STAR, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
              ...
              MPI_Sendrecv(starBuf + offset_star[target] - offset_sph[NTask - 1] - count_sph[NTask - 1],
                           ...
                           count_recv_star[target] * sizeof(struct star_particle_data), MPI_BYTE, target, TAG_STARDATA, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);

              MPI_Sendrecv(keyBuf + offset_star[target], count_star[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_STAR,
                           Key + offset_recv_star[target], count_recv_star[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_STAR,
                           MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
#endif
#ifdef BLACK_HOLES
          if(count_BHs[target] > 0 || count_recv_BHs[target] > 0)
            {
              MPI_Sendrecv(partBuf + offset_BHs[target], count_BHs[target] * sizeof(struct particle_data), MPI_BYTE, target,
              ...
              MPI_Sendrecv(keyBuf + offset_BHs[target], count_BHs[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_BH,
                           Key + offset_recv_BHs[target], count_recv_BHs[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_BH,
                           MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
#endif
#ifdef SINKS
          if(count_sinks[target] > 0 || count_recv_sinks[target] > 0)
            {
              MPI_Sendrecv(partBuf + offset_sinks[target], count_sinks[target] * sizeof(struct particle_data), MPI_BYTE, target,
                           TAG_PDATA_SINKS, P + offset_recv_sinks[target], count_recv_sinks[target] * sizeof(struct particle_data),
                           MPI_BYTE, target, TAG_PDATA_SINKS, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

              MPI_Sendrecv(SinksBuf + offset_sinks[target] - offset_sph[NTask - 1] - count_sph[NTask - 1],
                           count_sinks[target] * sizeof(struct sink_particle_data), MPI_BYTE, target, TAG_SINKSDATA,
                           SinkP + NumSinks + offset_recv_sinks[target] - offset_recv_sph[NTask - 1] - count_recv_sph[NTask - 1],
                           count_recv_sinks[target] * sizeof(struct sink_particle_data), MPI_BYTE, target, TAG_SINKSDATA,
                           MPI_COMM_WORLD, MPI_STATUS_IGNORE);

              MPI_Sendrecv(keyBuf + offset_sinks[target], count_sinks[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_SINKS,
                           Key + offset_recv_sinks[target], count_recv_sinks[target] * sizeof(peanokey), MPI_BYTE, target,
                           TAG_KEY_SINKS, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
#endif
#ifdef DUST_LIVE
          if(count_dust[target] > 0 || count_recv_dust[target] > 0)
            {
              MPI_Sendrecv(partBuf + offset_dust[target], count_dust[target] * sizeof(struct particle_data), MPI_BYTE, target,
                           TAG_PDATA_DUST, P + offset_recv_dust[target], count_recv_dust[target] * sizeof(struct particle_data),
                           MPI_BYTE, target, TAG_PDATA_DUST, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
              ...
              MPI_Sendrecv(keyBuf + offset_dust[target], count_dust[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_DUST,
                           Key + offset_recv_dust[target], count_recv_dust[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_DUST,
                           MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
#endif
      ...
      if(target < NTask)
        {
          ...
          if(count_recv_tracer[target] > 0)
            {
              MPI_Irecv(tracerRecvBuf + offset_recv_tracer[target], count_recv_tracer[target] * sizeof(struct tracer_linked_list),
          ...
          if(count_recv_star[target] > 0)
            {
              ...
              MPI_Irecv(Key + offset_recv_star[target], count_recv_star[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_STAR,
                        MPI_COMM_WORLD, &requests[n_requests++]);
            }
#endif
#ifdef BLACK_HOLES
          if(count_recv_BHs[target] > 0)
            {
              MPI_Irecv(P + offset_recv_BHs[target], count_recv_BHs[target] * sizeof(struct particle_data), MPI_BYTE, target,
                        TAG_PDATA_BH, MPI_COMM_WORLD, &requests[n_requests++]);
              ...
              MPI_Irecv(BHP + NumBHs + offset_recv_BHs[target] - offset_recv_star[NTask - 1] - count_recv_star[NTask - 1],
                        ...
                        &requests[n_requests++]);
#else
              MPI_Irecv(BHP + NumBHs + offset_recv_BHs[target] - offset_recv_sph[NTask - 1] - count_recv_sph[NTask - 1],
                        count_recv_BHs[target] * sizeof(struct bh_particle_data), MPI_BYTE, target, TAG_BHDATA, MPI_COMM_WORLD,
                        &requests[n_requests++]);
#endif
            }
#endif
#ifdef SINKS
          if(count_recv_sinks[target] > 0)
            {
              MPI_Irecv(P + offset_recv_sinks[target], count_recv_sinks[target] * sizeof(struct particle_data), MPI_BYTE, target,
                        TAG_PDATA_SINKS, MPI_COMM_WORLD, &requests[n_requests++]);

              MPI_Irecv(Key + offset_recv_sinks[target], count_recv_sinks[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_SINKS,
                        MPI_COMM_WORLD, &requests[n_requests++]);

              MPI_Irecv(SinkP + NumSinks + offset_recv_sinks[target] - offset_recv_sph[NTask - 1] - count_recv_sph[NTask - 1],
                        count_recv_sinks[target] * sizeof(struct sink_particle_data), MPI_BYTE, target, TAG_SINKSDATA, MPI_COMM_WORLD,
                        &requests[n_requests++]);
            }
#endif
#ifdef DUST_LIVE
          if(count_recv_dust[target] > 0)
            {
              MPI_Irecv(P + offset_recv_dust[target], count_recv_dust[target] * sizeof(struct particle_data), MPI_BYTE, target,
                        TAG_PDATA_DUST, MPI_COMM_WORLD, &requests[n_requests++]);

              MPI_Irecv(Key + offset_recv_dust[target], count_recv_dust[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_DUST,
                        MPI_COMM_WORLD, &requests[n_requests++]);
              ...
              MPI_Irecv(DustP + N_dust + offset_recv_dust[target] - offset_recv_star[NTask - 1] - count_recv_star[NTask - 1],
                        ...
                        &requests[n_requests++]);
#else
              MPI_Irecv(DustP + N_dust + offset_recv_dust[target] - offset_recv_sph[NTask - 1] - count_recv_sph[NTask - 1],
                        count_recv_dust[target] * sizeof(struct dust_particle_data), MPI_BYTE, target, TAG_DUSTDATA, MPI_COMM_WORLD,
                        &requests[n_requests++]);
#endif
            }
      ...
      if(target < NTask)
        {
          if(count_sph[target] > 0)
            {
              ...
              MPI_Isend(sphAbundancesBuf + (offset_sph[target] * ChimesGlobalVars.totalNumberOfSpecies),
                        count_sph[target] * ChimesGlobalVars.totalNumberOfSpecies, MPI_DOUBLE, target, TAG_ABUNDATA, MPI_COMM_WORLD,
                        &requests[n_requests++]);
#endif
          ...
          if(count_tracer[target] > 0)
            {
              MPI_Isend(tracerBuf + offset_tracer[target], count_tracer[target] * sizeof(struct tracer_linked_list), MPI_BYTE, target,
          ...
          if(count_star[target] > 0)
            {
              ...
              MPI_Isend(starBuf + offset_star[target] - offset_sph[NTask - 1] - count_sph[NTask - 1],
                        count_star[target] * sizeof(struct star_particle_data), MPI_BYTE, target, TAG_STARDATA, MPI_COMM_WORLD,
                        &requests[n_requests++]);

              MPI_Isend(keyBuf + offset_star[target], count_star[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_STAR,
                        MPI_COMM_WORLD, &requests[n_requests++]);
            }
#endif
#ifdef BLACK_HOLES
          if(count_BHs[target] > 0)
            {
              ...
              MPI_Isend(BHsBuf + offset_BHs[target] - offset_sph[NTask - 1] - count_sph[NTask - 1],
                        ...
                        &requests[n_requests++]);
#endif
              MPI_Isend(keyBuf + offset_BHs[target], count_BHs[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_BH,
                        MPI_COMM_WORLD, &requests[n_requests++]);
            }
#endif
#ifdef SINKS
          if(count_sinks[target] > 0)
            {
              MPI_Isend(partBuf + offset_sinks[target], count_sinks[target] * sizeof(struct particle_data), MPI_BYTE, target,
                        TAG_PDATA_SINKS, MPI_COMM_WORLD, &requests[n_requests++]);

              MPI_Isend(SinksBuf + offset_sinks[target] - offset_sph[NTask - 1] - count_sph[NTask - 1],
                        count_sinks[target] * sizeof(struct sink_particle_data), MPI_BYTE, target, TAG_SINKSDATA, MPI_COMM_WORLD,
                        &requests[n_requests++]);

              MPI_Isend(keyBuf + offset_sinks[target], count_sinks[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_SINKS,
                        MPI_COMM_WORLD, &requests[n_requests++]);
            }
#endif
#ifdef DUST_LIVE
          if(count_dust[target] > 0)
            {
              MPI_Isend(partBuf + offset_dust[target], count_dust[target] * sizeof(struct particle_data), MPI_BYTE, target,
                        TAG_PDATA_DUST, MPI_COMM_WORLD, &requests[n_requests++]);
              ...
              MPI_Isend(dustBuf + offset_dust[target] - offset_sph[NTask - 1] - count_sph[NTask - 1],
                        ...
                        &requests[n_requests++]);
#endif
              MPI_Isend(keyBuf + offset_dust[target], count_dust[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_DUST,
                        MPI_COMM_WORLD, &requests[n_requests++]);
            }
#endif
...
#ifdef CHIMES
  myMPI_Alltoallv(sphAbundancesBuf, count_sph, offset_sph, sphAbundancesRecvBuf, count_recv_sph, offset_recv_sph,
                  ChimesGlobalVars.totalNumberOfSpecies * sizeof(double), 0, MPI_COMM_WORLD);
#endif
...
#ifdef TRACER_MC
  myMPI_Alltoallv(tracerBuf, count_tracer, offset_tracer, tracerRecvBuf, count_recv_tracer, offset_recv_tracer,
  ...
  myMPI_Alltoallv(starBuf, count_star, starbuf_offset, StarP, count_recv_star, starbuf_recv_offset, sizeof(struct star_particle_data),
                  0, MPI_COMM_WORLD);
  ...
  myMPI_Alltoallv(BHsBuf, count_BHs, bhbuf_offset, BHP, count_recv_BHs, bhbuf_recv_offset, sizeof(struct bh_particle_data), 0,
                  MPI_COMM_WORLD);
...
#ifdef SINKS
  myMPI_Alltoallv(partBuf, count_sinks, offset_sinks, P, count_recv_sinks, offset_recv_sinks, sizeof(struct particle_data), 0,
                  MPI_COMM_WORLD);

  myMPI_Alltoallv(SinksBuf, count_sinks, sinksbuf_offset, SinkP, count_recv_sinks, sinksbuf_recv_offset,
                  sizeof(struct sink_particle_data), 0, MPI_COMM_WORLD);

  myMPI_Alltoallv(keyBuf, count_sinks, offset_sinks, Key, count_recv_sinks, offset_recv_sinks, sizeof(peanokey), 0, MPI_COMM_WORLD);
#endif

#ifdef DUST_LIVE
  myMPI_Alltoallv(partBuf, count_dust, offset_dust, P, count_recv_dust, offset_recv_dust, sizeof(struct particle_data), 0,
                  MPI_COMM_WORLD);

  myMPI_Alltoallv(dustBuf, count_dust, dustbuf_offset, DustP, count_recv_dust, dustbuf_recv_offset, sizeof(struct dust_particle_data),
                  0, MPI_COMM_WORLD);
...
#ifdef CHIMES
  /* Loop through received gas particles
      ...
      if(count_recv_sph[target] > 0)
        {
          for(i = 0; i < count_recv_sph[target]; i++)
            {
              SphP[offset_recv_sph[target] + i].ChimesGasVars.index = offset_recv_sph[target] + i;
              chimes_set_pointers(offset_recv_sph[target] + i);
              for(abunIndex = 0; abunIndex < ChimesGlobalVars.totalNumberOfSpecies; abunIndex++)
                SphP[offset_recv_sph[target] + i].ChimesGasVars.abundances[abunIndex] =
                    sphAbundancesRecvBuf[((offset_recv_sph[target] - offset_recv_sph[0] + i) * ChimesGlobalVars.totalNumberOfSpecies) +
                                         abunIndex];
            }
        }
    }
...
#endif
#ifdef SINKS
  for(i = 0; i < count_get_sinks; i++)
    {
      SinkP[NumSinks + i].PID               = offset_recv_sinks[0] + i;
      P[offset_recv_sinks[0] + i].AuxDataID = NumSinks + i;
    }
#endif
#ifdef DUST_LIVE
...
#endif
#ifdef TRACER_MC
  myfree_movable(tracerRecvBuf);
  myfree_movable(tracerBuf);
#endif
  myfree_movable(sphBuf);
  myfree_movable(partBuf);
#ifdef DUST_LIVE
  myfree_movable(dustbuf_recv_offset);
  myfree_movable(dustbuf_offset);
  myfree_movable(offset_recv_dust);
  myfree_movable(count_recv_dust);
  myfree_movable(offset_dust);
  myfree_movable(count_dust);
#endif
#ifdef SINKS
  myfree_movable(sinksbuf_recv_offset);
  myfree_movable(sinksbuf_offset);
  myfree_movable(offset_recv_sinks);
  myfree_movable(count_recv_sinks);
  myfree_movable(offset_sinks);
  myfree_movable(count_sinks);
#endif
#ifdef BLACK_HOLES
  myfree_movable(bhbuf_recv_offset);
  myfree_movable(bhbuf_offset);
  myfree_movable(offset_recv_BHs);
  myfree_movable(count_recv_BHs);
  ...
  myfree_movable(count_recv_star);
  myfree_movable(offset_star);
  myfree_movable(count_star);
#endif
#ifdef TRACER_MC
  myfree_movable(offset_recv_tracer);
  myfree_movable(count_recv_tracer);
  myfree_movable(offset_tracer);
  myfree_movable(count_tracer);
#endif
  myfree_movable(offset_recv_sph);
  myfree_movable(offset_recv);
  myfree_movable(count_recv_sph);
  myfree_movable(count_recv);
  myfree_movable(offset_sph);
  myfree_movable(offset);
  myfree_movable(count_sph);
  myfree_movable(count);
  ...
  for(int i = 0; i < NumPart; i++)
    {
      if(P[i].Type == 1 && (P[i].TracerHead >= 0 || P[i].NumberOfTracers > 0))
        terminate("i=%d P[i].Type=%d  P[i].TracerHead=%d  P[i].NumberOfTracers=%d", i, P[i].Type, P[i].TracerHead,
                  P[i].NumberOfTracers);
    }
  check_tracer_lists();
#endif
