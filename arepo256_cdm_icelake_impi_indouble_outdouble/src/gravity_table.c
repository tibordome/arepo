...
void grav_table_find_grav_acceleration(double xi, double yi, double zi, double *sp_acc)
{
  ...
  /*Exceptions*/
  if(GravT[j * nz + k].R > cyl_Ri || GravT[(j + 1) * nz + k].R < cyl_Ri || GravT[j * nz + k + 1].R > cyl_Ri ||
     GravT[(j + 1) * nz + k + 1].R < cyl_Ri || GravT[j * nz + k].z > abzi || GravT[j * nz + k + 1].z < abzi ||
     GravT[(j + 1) * nz + k].z > abzi || GravT[(j + 1) * nz + k + 1].z < abzi || ((j + 1) * nz + k + 1) > All.NGravityTableBins)
