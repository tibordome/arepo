 ...
 *
 * \file        src/growing_disk_potential.c
 * \date        MM/YYYY
 * \author
 * \brief
 * \details
...
void get_disk_forces(double RR, double zz, double *f_R, double *f_z)
{
  ...
  *f_R =
      tanh(RR / 0.01) * ((force_R[j * (Nbin + 1) + i]) * (1 - u) * (1 - v) + (force_R[j * (Nbin + 1) + (i + 1)]) * (u) * (1 - v) +
                         (force_R[(j + 1) * (Nbin + 1) + i]) * (1 - u) * (v) + (force_R[(j + 1) * (Nbin + 1) + (i + 1)]) * (u) * (v));

  *f_z =
      tanh(zz / 0.01) * ((force_z[j * (Nbin + 1) + i]) * (1 - u) * (1 - v) + (force_z[j * (Nbin + 1) + (i + 1)]) * (u) * (1 - v) +
                         (force_z[(j + 1) * (Nbin + 1) + i]) * (1 - u) * (v) + (force_z[(j + 1) * (Nbin + 1) + (i + 1)]) * (u) * (v));
}

double get_disk_mass(double time) { return 5.8 * sqrt(time / 2.0); }
...
void growing_disk_init(void)
{
  ...
  force_R = (double *)mymalloc("force_R", (Nbin + 1) * (Nbin + 1) * sizeof(double));
  force_z = (double *)mymalloc("force_z", (Nbin + 1) * (Nbin + 1) * sizeof(double));
      ...
      for(i = 1; i <= n - m; i++)
        {
          ...
          w  = c[i + 1] - d[i];
          if((den = ho - hp) == 0.0)
            terminate("Error in routine polint");
          den  = w / den;
          d[i] = hp * den;
          c[i] = ho * den;
        }
...
static double *vector(long nl, long nh)
{
  ...
  if(!v)
    terminate("allocation failure in vector()");
  return v - nl + NR_END;
}

static void free_vector(double *v, long nl, long nh)
/* free a double vector allocated with vector() */ { free((FREE_ARG)(v + nl - NR_END)); }
  ...
  if((ax = fabs(x)) < 8.0)
    {
      y = x * x;
      ans1 =
          x * (72362614232.0 + y * (-7895059235.0 + y * (242396853.1 + y * (-2972611.439 + y * (15704.48260 + y * (-30.16036606))))));
