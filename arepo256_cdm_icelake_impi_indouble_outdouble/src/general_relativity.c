...
/* TODO or TOCHECK
 ...
 -> ACHTUNG bei der Zeitintegration haben wir womoeglich ueberall einen Fehler 2 drin, so dass das nicht auffaefllt aber sich einfach
 die Zeit anders entwickelt. Das koennte dann aber beim moving mesh zu problem fuehren, je nachdem wie es da entwickelt wird.
...
// static double BH_x = 27.392*0.5-0.105353846154; //15.;   // this will be used as center if we use a fixed numerical metric that is
// read in static double BH_y = 27.392*0.5-0.105353846154; //15.;   // -> so it will be overwritten static double BH_z
// = 27.392*0.5-0.105353846154; //15.;
...
void get_metric_general_relativity(double x, double y, double z, double *alp, double *psi, double *bx, double *by, double *bz)
{
  ...
  *psi = 1. + 2. * mbh / r + (3. * mbh * mbh + a * a * mu * mu) / (2. * r * r) + (mbh * mbh - a * a) * mbh / (2. * r * r * r) +
         (mbh * mbh - a * a) * (mbh * mbh - a * a) / (16. * r * r * r * r);  // that's psi^4

  *psi = sqrt(*psi);
  *psi = sqrt(*psi);  // since the above formula gives psi^4

  double bphi = (2. * a * mbh * capr) / (sigma * (capr * capr + a * a) + 2. * a * a * mbh * capr * sin(theta) * sin(theta));
...
void compute_source_terms_general_relativity(struct particle_data *P, struct sph_particle_data *SphP, int i, double *srcsx,
                                             double *srcsy, double *srcsz, double *srctau)
{
  ...
  *srcsx = 2. * alp * dpsi[0] / psi * (sii[0] + sii[1] + sii[2]) + SphP[i].Momentum[0] / SphP[i].Volume * dbx[0] +
           SphP[i].Momentum[1] / SphP[i].Volume * dby[0] + SphP[i].Momentum[2] / SphP[i].Volume * dbz[0] -
           (SphP[i].Energy / SphP[i].Volume + P[i].Mass / SphP[i].Volume) * dalp[0];

  *srcsy = 2. * alp * dpsi[1] / psi * (sii[0] + sii[1] + sii[2]) + SphP[i].Momentum[0] / SphP[i].Volume * dbx[1] +
           SphP[i].Momentum[1] / SphP[i].Volume * dby[1] + SphP[i].Momentum[2] / SphP[i].Volume * dbz[1] -
           (SphP[i].Energy / SphP[i].Volume + P[i].Mass / SphP[i].Volume) * dalp[1];

  *srcsz = 2. * alp * dpsi[2] / psi * (sii[0] + sii[1] + sii[2]) + SphP[i].Momentum[0] / SphP[i].Volume * dbx[2] +
           SphP[i].Momentum[1] / SphP[i].Volume * dby[2] + SphP[i].Momentum[2] / SphP[i].Volume * dbz[2] -
  ...
  *srctau = Sxx * Kxx + Sxy * Kxy + Sxz * Kxz + Syx * Kxy + Syy * Kyy + Syz * Kyz + Szx * Kxz + Szy * Kyz + Szz * Kzz -
            SphP[i].Momentum[0] / SphP[i].Volume / ps4 * dalp[0] - SphP[i].Momentum[1] / SphP[i].Volume / ps4 * dalp[1] -
                                                   ...
                                                   struct pv_update_data *pvd)
{
  ...
  /*if ( iter == nmax ) {
    printf( "GENERAL_RELATIVITY: Max iter reached in in con2prim Newton method, rel=%g, prec=%g, f=%g, df=%g, p=%g, nmax=%i, p0=%g,
  i=%i\n", rel, prec, f, df, p, nmax, p0, i ); printf( "GENERAL_RELATIVITY: cons: d=%g, s2=%g, t=%g, p0=%g\n",d,s2,t,p0 );
...
void update_primitive_variables_general_relativity_org(struct particle_data *P, struct sph_particle_data *SphP, int i,
                                                       struct pv_update_data *pvd)
{
  /* here we assume that the EoS is available in this routine either as table or analytically
   for the momemt this routine works only for analytic ideal gas EoS
  ...
  if(t < 0.0)
    {  // reset to the previous value by recalculating cons from prim
...
ATMLABEL:
  if(p < 0.0 || r < rhomax_gr * atm_fct * thr_fct)
    {
      r  = rhomax_gr * atm_fct;
      vx = 0.0;
      vy = 0.0;
...
void compute_conserved_quantities_from_ICs_general_relativity(int i)
{
  ...
  double Enthalpy =
      1. + GAMMA * SphP[i].Pressure / SphP[i].Density / (GAMMA - 1.);  //+ SphP[i].Utherm + SphP[i].Pressure / SphP[i].Density;
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      double dt_cell = (P[i].TimeBinHydro ? (((integertime)1) << P[i].TimeBinHydro) : 0) *
  ...
  for(i = 0; i < oned_metric->nsize; i++)
    {
      ...
      /*if(fscanf(file, "%lf %lf %lf %lf %lf %lf", oned_metric->r + i, oned_metric->alp + i, oned_metric->psi + i, oned_metric->bx + i,
         oned_metric->by + i, oned_metric->bz + i ) != 6)
         {
           fprintf(stderr, "error in metric file at element %d\n", i);
           free(oned_metric->r);
           free(oned_metric->alp);
      ...
      // fscanf(file, "%lf %lf %lf %lf %lf %lf", oned_metric->r + i, oned_metric->alp + i, oned_metric->psi + i, //oned_metric->bx + i,
      // oned_metric->by + i, oned_metric->bz + i )

      // printf(" was %g %g %g %g %g %g\n", *oned_metric->r, *oned_metric->alp, *oned_metric->psi, *oned_metric->bx, *oned_metric->by,
      // *oned_metric->bz );
    }
...
int read_fixed_numerical_1d_metric(void)
{
  ...
  if(fscanf(file, "%d %lf %lf %lf", &oned_metric->nsize, &oned_metric->r0, &oned_metric->dr, &oned_metric->rmax) !=
     4)  // todo is d correct?
    {
      fprintf(stderr, "error in metric 1d file format");
      free(oned_metric);
  ...
  for(i = 0; i < NumGas; i++)
    {
      ...
      double r = (SphP[i].Center[0] - BH_x) * (SphP[i].Center[0] - BH_x) + (SphP[i].Center[1] - BH_y) * (SphP[i].Center[1] - BH_y) +
  ...
  if(ThisTask == 0)
    {
      ...
      fprintf(FdGR, "%g %g %g %g %g %g %g %g %g %g %g %g %g %g\n", All.Time, rhomaxAll, rhominAll, umaxAll, vmaxAll, mrestAll, eintAll,
              ucenter, minlapseAll, psimaxAll, tausumAll, taumaxAll, tauminAll, rstmaxAll);
      myflush(FdGR);
    }
}
