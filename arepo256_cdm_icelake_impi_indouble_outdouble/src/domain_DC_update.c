...
void domain_exchange_and_update_DC(void)
{
  ...
  send_trans_data = (struct aux_trans_data *)mymalloc("send_trans_data", nexport * sizeof(struct aux_trans_data));
  recv_trans_data = (struct aux_trans_data *)mymalloc("recv_trans_data", nimport * sizeof(struct aux_trans_data));
  ...
  local_trans_data = (struct local_aux_trans_data *)mymalloc("local_trans_data", NumGas * sizeof(struct local_aux_trans_data));
  ...
  for(ii = 0, jj = 0; ii < nimport && jj < NumGas;)
    {
      if(recv_trans_data[ii].ID < local_trans_data[jj].ID)
        {
          /* this particle has been eliminated */
          recv_trans_data[ii].new_index = -1;
          ii++;
        }
      else if(recv_trans_data[ii].ID > local_trans_data[jj].ID)
        jj++;
      else
        {
          recv_trans_data[ii].new_index = local_trans_data[jj].new_index;
          ii++;
          jj++;
        }
    }
  ...
  send_transscribe_data =
      (struct aux_transscribe_data *)mymalloc("send_transscribe_data", nexport * sizeof(struct aux_transscribe_data));
  recv_transscribe_data =
      (struct aux_transscribe_data *)mymalloc("recv_transscribe_data", nimport * sizeof(struct aux_transscribe_data));
  ...
  connection *tmpDC = (connection *)mymalloc("tmpDC", nexport * sizeof(connection));
  ...
  int last = -1;
  for(ii = 0, jj = 0; ii < NumGas && jj < Nvc; ii++)
    {
      int k = local_trans_data[ii].new_index;
      ...
      else if(P[k].ID == DC[jj].ID)
        {
          SphP[k].first_connection = jj;

          while(jj < Nvc)
            {
              SphP[k].last_connection = jj;
