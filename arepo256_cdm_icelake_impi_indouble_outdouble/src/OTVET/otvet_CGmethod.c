            ...
            if(SphP[j].n_gamma[i] > 0)
              {
                double R = sqrt(SphP[j].Grad_ngamma[0][i] * SphP[j].Grad_ngamma[0][i] +
                                ...
                                SphP[j].Grad_ngamma[2][i] * SphP[j].Grad_ngamma[2][i]) /
                           (SphP[j].n_gamma[i] * Kappa[j]);
...
void otvet_matrix_multiply(double *in, double *out, double *sum)
{
  ...
  All.BunchSize =
      (int)((All.BufferSize * 1024 * 1024) /
            (sizeof(data_index) + sizeof(struct data_nodelist) + sizeof(struct radtransferdata_in) +
             sizeof(struct radtransferdata_out) + smax(sizeof(struct radtransferdata_in), sizeof(struct radtransferdata_out))));
          ...
          if(recvTask < NTask)
            {
              if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
                {
                  /* get the particles */
                  MPI_Sendrecv(&OtvetDataIn[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct radtransferdata_in), MPI_BYTE,
                               recvTask, TAG_OTVET_A, &OtvetDataGet[Recv_offset[recvTask]],
                               Recv_count[recvTask] * sizeof(struct radtransferdata_in), MPI_BYTE, recvTask, TAG_OTVET_A,
          ...
          if(recvTask < NTask)
            {
              if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
                {
                  /* send the results */
                  MPI_Sendrecv(&OtvetDataResult[Recv_offset[recvTask]], Recv_count[recvTask] * sizeof(struct radtransferdata_out),
                               MPI_BYTE, recvTask, TAG_OTVET_B, &OtvetDataOut[Send_offset[recvTask]],
                               Send_count[recvTask] * sizeof(struct radtransferdata_out), MPI_BYTE, recvTask, TAG_OTVET_B,
                               MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                }
            }
        }
...
#endif
          {
            printf("1 + sum + rate= %g   sum=%g rate=%g i =%d\n", 1 + dt * c_light * ainv * Kappa[i] + sum[i], sum[i],
                   dt * c_light * ainv * Kappa[i], i);
            terminate("11111111");
          }
                  ...
                  if(r > 0 && (r < h_i || r < h_j))
                    {
                      ...
                      double tensor = (ET_ij[0] * dx * dx + ET_ij[1] * dy * dy + ET_ij[2] * dz * dz + 2.0 * ET_ij[3] * dx * dy +
                                       2.0 * ET_ij[4] * dy * dz + 2.0 * ET_ij[5] * dz * dx);
...
void ot_get_sigma(void)
{
  ...
  if(ThisTask == 0)
    for(i = 0; i < OT_N_BINS; i++)
      printf("OTVET SIGMA: %g %g | %g %g | %g %g\n", otvet_sigma_HI[i] / fac, G_HI[i] / fac_two, otvet_sigma_HeI[i] / fac,
             G_HeI[i] / fac_two, otvet_sigma_HeII[i] / fac, G_HeII[i] / fac_two);
