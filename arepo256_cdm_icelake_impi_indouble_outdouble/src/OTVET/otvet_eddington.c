...
void otvet_eddington(void)
{
  ...
  All.BunchSize  = (int)((All.BufferSize * 1024 * 1024) /
                        (sizeof(data_index) + sizeof(struct data_nodelist) + sizeof(struct eddingtondata_in) +
                         sizeof(struct eddingtondata_out) + smax(sizeof(struct eddingtondata_in), sizeof(struct eddingtondata_out))));
      ...
      for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
        {
          ...
          if(recvTask < NTask)
            if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
              /* get the particles */
              MPI_Sendrecv(&EddingtonDataIn[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct eddingtondata_in), MPI_BYTE,
                           recvTask, TAG_HYDRO_A, &EddingtonDataGet[Recv_offset[recvTask]],
                           Recv_count[recvTask] * sizeof(struct eddingtondata_in), MPI_BYTE, recvTask, TAG_HYDRO_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
        }
      ...
      for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
        {
          ...
          if(recvTask < NTask)
            if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
              MPI_Sendrecv(&EddingtonDataResult[Recv_offset[recvTask]], Recv_count[recvTask] * sizeof(struct eddingtondata_out),
                           MPI_BYTE, recvTask, TAG_HYDRO_B, &EddingtonDataOut[Send_offset[recvTask]],
                           Send_count[recvTask] * sizeof(struct eddingtondata_out), MPI_BYTE, recvTask, TAG_HYDRO_B, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
        }
      ...
      if(recvTask < NTask)
        if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
          MPI_Sendrecv(&EddingtonResultsImported[Recv_offset[recvTask]],
                       ...
                       &tmp_results[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct eddingtonresultsimported_data),
...
#ifdef SMUGGLE_RADPRESS_OPT_THICK
      trace1 = SphP[i].vector_n[0] * SphP[i].vector_n[0] + SphP[i].vector_n[1] * SphP[i].vector_n[1] +
               SphP[i].vector_n[2] * SphP[i].vector_n[2];
      trace1 = sqrt(trace1);
                      ...
                      if(*nexport >= All.BunchSize)
                        {
                          ...
                          *nexport = nexport_save;
                          if(nexport_save == 0)
                            terminate("error eddington, buffer too small\n"); /* in this case, the buffer is too small to process even
                                                                                 a single particle */
