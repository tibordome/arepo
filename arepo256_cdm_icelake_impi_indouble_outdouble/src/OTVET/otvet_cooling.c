 ...
 *
 * \file        src/OTVET/otvet_cooling.c
 * \date        MM/YYYY
 * \author
 * \brief
 * \details
...
#if defined(OTVET_COOLING_HEATING)
...
double otvet_DoHeating(int i, double dt_internal)
{
  ...
  Lambda_Heat = otvet_get_heating_rate(i);

  du = Lambda_Heat * dt_internal / hubble_a /
       (SphP[i].Density * a3inv); /*LVS: rt_DoHeating.c in arepo has no 1/hubble_a factor here?? */
...
double otvet_DoCooling(int i, double dt_internal)
{
  ...
  /* do the cooling */
  lambda = otvet_get_cooling_rate(i, utherm);
  ...
  if(fabs(du) < 0.2 * utherm)
  ...
  else
    {
      ...
      /* bracketing */
      while(u_lower - u_old - ratefact * otvet_get_cooling_rate(i, u_lower) > 0)
        {
          u_upper /= 1.1;
          u_lower /= 1.1;
      ...
      do
        {
          ...
          if(u - u_old - ratefact * otvet_get_cooling_rate(i, u) > 0)
            u_upper = u;
          else
            u_lower = u;
...
double otvet_get_heating_rate(int i)
{
  ...
  for(j = 0, rate = 0; j < OT_N_BINS; j++)
    {
      n_gamma = SphP[i].n_gamma[j] / P[i].Mass * a3inv;

      if(nu[j] >= 13.6)
        rate += nHI * c_light * otvet_sigma_HI[j] * G_HI[j] * n_gamma;

#ifdef OTVET_INCLUDE_HE
      if(nu[j] >= 24.6)
        rate += nHeI * c_light * otvet_sigma_HeI[j] * G_HeI[j] * n_gamma;

      if(nu[j] >= 54.4)
        rate += nHeII * c_light * otvet_sigma_HeII[j] * G_HeII[j] * n_gamma;
#endif
    }
...
double otvet_GetCoolingTime(int i, double u_old, double rho, double *ne_guess)
{
  ...
  LambdaNet = otvet_get_heating_rate(i) + otvet_get_cooling_rate(i, u);
