...
void blackhole_disk_vorticity(void)
{
  ...
  All.BunchSize  = (int)((All.BufferSize * 1024 * 1024) /
                        (sizeof(data_index) + sizeof(struct data_nodelist) + sizeof(struct blackholedata_in) +
                         sizeof(struct blackholedata_out) + smax(sizeof(struct blackholedata_in), sizeof(struct blackholedata_out))));
      ...
      for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
        {
          ...
          if(recvTask < NTask)
            if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
              MPI_Sendrecv(&BlackholeDataIn[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct blackholedata_in), MPI_BYTE,
                           recvTask, TAG_DENS_A, &BlackholeDataGet[Recv_offset[recvTask]],
                           Recv_count[recvTask] * sizeof(struct blackholedata_in), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
        }
      ...
      for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
        {
          ...
          if(recvTask < NTask)
            if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
              MPI_Sendrecv(&BlackholeDataResult[Recv_offset[recvTask]], Recv_count[recvTask] * sizeof(struct blackholedata_out),
                           MPI_BYTE, recvTask, TAG_DENS_B, &BlackholeDataOut[Send_offset[recvTask]],
                           Send_count[recvTask] * sizeof(struct blackholedata_out), MPI_BYTE, recvTask, TAG_DENS_B, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
        }
