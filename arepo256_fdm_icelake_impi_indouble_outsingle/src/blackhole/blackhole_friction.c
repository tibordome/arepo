 ...
 *
 * \file        src/blackhole/blackhole_friction.c
 * \date        MM/YYYY
 * \author
 * \brief
 * \details
  ...
  for(idx = 0; idx < TimeBinsBHAccretion.NActiveParticles; idx++)
    {
      ...
      if(BPP(i).BH_MinPot_ActiveM >= BH_REPOSITION_POTMIN_TRUST_THRESHOLD * BPP(i).BH_MinPot_TotalM)
        {
          if(BPP(i).BH_MinPotTime - BPP(i).BH_MinPotTime_Previous > 0 && BPP(i).BH_MinPotTime >= 0 &&
             BPP(i).BH_MinPotTime_Previous >= 0)
            {
      ...
      if(BPP(i).BH_MinPot_ActiveM >= BH_REPOSITION_POTMIN_TRUST_THRESHOLD * BPP(i).BH_MinPot_TotalM)
        {
          ...
          if((BPP(i).BH_MinPot < 0.5 * BHPOTVALUEINIT) && (BPP(i).BH_MinPot != 0) &&
             (BPP(i).BH_MinPot_Extended < 0.5 * BHPOTVALUEINIT) && (BPP(i).BH_MinPot_Extended != 0))
            {
          ...
          if((BPP(i).BH_MinPot < 0.5 * BHPOTVALUEINIT) && (BPP(i).BH_MinPot != 0) &&
             (BPP(i).BH_MinPot_Extended < 0.5 * BHPOTVALUEINIT) && (BPP(i).BH_MinPot_Extended != 0) &&
             (BPP(i).BH_MinPot_Extended < BPP(i).BH_MinPot))
            {
              for(k = 0; k < 3; k++)
                P[i].Pos[k] = BPP(i).BH_MinPotPos_Extended[k];
            }
  ...
  for(int idx = 0; idx < TimeBinsBHAccretion.NActiveParticles; idx++)
    {
      ...
      if((BPP(i).BH_MinPot < 0.5 * BHPOTVALUEINIT) && (BPP(i).BH_MinPot != 0) && (BPP(i).BH_MinPot_Extended < 0.5 * BHPOTVALUEINIT) &&
         (BPP(i).BH_MinPot_Extended != 0) && (BPP(i).BH_MinPot_Extended > BPP(i).BH_MinPot))
        {
          for(int k = 0; k < 3; k++)
