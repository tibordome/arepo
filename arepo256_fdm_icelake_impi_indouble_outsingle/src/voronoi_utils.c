...
#if defined(VORONOI) && !defined(TWODIMS) && !defined(ONEDIMS) && defined(TETRA_INDEX_IN_FACE)
                              ...
                              unsigned int *nof_polygon_elements)
{
  ...
  /* 6 doubles represent one line segment */
  assert(nof_elements % 6 == 0);
 ...
 *
 *  \param polygon Array of points of the polygon: x1, y1, x2, y2, ..., has to be sorted counterclockwise and closed (x_n == x_0 && y_n
 * == y_0) \param nof_elements Number of elements in the array
 *
 */
static double polygon_area(double *polygon, unsigned int nof_elements)
{
...
/* quicksort partitioning function, helper for qs_sort */
static int qs_partition(double *A, int p, int r, double *B)
{
  double x = A[r];
  double tmp;
...
/* quick-sorts the points of the polygon with respect to phi */
static void qs_sort(double *A, int p, int r, double *B)
{
  int q;
...
/** calculate the phi coordinate of a point */
static double calc_phi(double x, double y)
{
