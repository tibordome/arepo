          ...
          for(cone = 0; cone < FLD_NCONES; cone++)
            {
              ...
              TIMER_STOP(CPU_FLD);
              TIMER_START(CPU_FLD_MATRIX);
              fld_radtransfer(gamma_new[cone], dt);
              TIMER_STOP(CPU_FLD_MATRIX);
              TIMER_START(CPU_FLD);
            }
  ...
  for(i = 0; i < NumGas; i++)
    {
      ...
      if(P[i].ID >= FLD_LOWER_BOUNDARY_MINID && P[i].ID <= FLD_LOWER_BOUNDARY_MAXID)
        {
          SphP[i].w[2 * NUMDIMS] +=
              dt * c_light * 2 * amr_area[Mesh.DP[i].level] / (4. + 3 * SphP[i].Kappa_P * amr_area[Mesh.DP[i].level]) / SphP[i].Volume;
        }
#endif
      ...
      if(P[i].Type == 0 && P[i].Mass != 0 && P[i].ID != 0)
        {
          if(!gsl_finite(gamma_new[0][i]) || !gsl_finite(u_new[i]))
            {
              terminate("FLD: bad radiation update P[i].Mass=%g dtime=%g ID=%d Lambda=%g Kappa_diff=%g i=%d", P[i].Mass, dt, P[i].ID,
                        SphP[i].Lambda, SphP[i].Kappa_diff, i);
            }
      ...
      if(P[i].Type == 0 && P[i].Mass != 0 && P[i].ID != 0)
        {
          ...
          for(k = 0; k < 2; k++)
            {
              ...
              if(!gsl_finite(dvel))
                terminate("FLD: bad radiation pressure dvel=%g P[i].Mass=%g dtime=%g ID=%d Grad[k]=%g Lambda=%g k=%d", dvel, P[i].Mass,
                          dt, P[i].ID, SphP[i].Grad.dngamma[k], SphP[i].Lambda, k);
            }
          // add kin. energy
          SphP[i].Energy += 0.5 * P[i].Mass * (P[i].Vel[0] * P[i].Vel[0] + P[i].Vel[1] * P[i].Vel[1] + P[i].Vel[2] * P[i].Vel[2]);
          ...
          MyFloat rel1 =
              dt * SphP[i].Lambda * (2. * SphP[i].Kappa_P / SphP[i].Kappa_R - 1.) *
              (P[i].Vel[0] * SphP[i].Grad.dngamma[0] + P[i].Vel[1] * SphP[i].Grad.dngamma[1] + P[i].Vel[2] * SphP[i].Grad.dngamma[2]);
          ...
          SphP[i].Utherm =
              (SphP[i].Energy / P[i].Mass - 0.5 * (P[i].Vel[0] * P[i].Vel[0] + P[i].Vel[1] * P[i].Vel[1] + P[i].Vel[2] * P[i].Vel[2]));
        }
    }
      ...
      if(SphP[i].n_gamma > 0)
        {
          double beta = 1e-4;
#ifndef FLD_CONES
          double R = (sqrt(SphP[i].Grad.dngamma[0] * SphP[i].Grad.dngamma[0] + SphP[i].Grad.dngamma[1] * SphP[i].Grad.dngamma[1] +
                           SphP[i].Grad.dngamma[2] * SphP[i].Grad.dngamma[2]) +
                      beta) /
                     (SphP[i].n_gamma * SphP[i].Kappa_R);
#else
          double R = (sqrt(SphP[i].Grad.dngamma[0] * SphP[i].Grad.dngamma[0] + SphP[i].Grad.dngamma[1] * SphP[i].Grad.dngamma[1] +
                           SphP[i].Grad.dngamma[2] * SphP[i].Grad.dngamma[2]) +
                      beta) /
                     (SphP[i].n_gamma * SphP[i].Kappa_R) * FLD_NCONES;
#endif
  ...
  for(i = 0; i < Mesh.Nvf; i++)
    {
...
#ifdef FLD_TEST_BOUNDARY
      if(DP[VF[i].p1].ID >= FLD_LOWER_BOUNDARY_MINID && DP[VF[i].p1].ID <= FLD_LOWER_BOUNDARY_MAXID &&
         DP[VF[i].p1].ID == DP[VF[i].p2].ID)
        {
      ...
      else if(DP[VF[i].p1].ID >= FLD_UPPER_BOUNDARY_MINID && DP[VF[i].p1].ID <= FLD_UPPER_BOUNDARY_MAXID &&
              DP[VF[i].p1].ID == DP[VF[i].p2].ID)
        {
          int particle = DP[VF[i].p1].index;
          if(particle >= NumGas)
          ...
          if((Kappa_i + Kappa_j) > 0)
            {
              ...
              if(!gsl_finite(SphP[i].w[ind[0]]) || !gsl_finite(SphP[i].w[ind[1]]) || !gsl_finite(SphP[i].w[ind[2]]) ||
                 !gsl_finite(SphP[i].w[ind[3]]) || !gsl_finite(SphP[i].w[ind[4]]) || !gsl_finite(SphP[i].w[ind[5]]))
                {
                  terminate("blubb");
                }
#else
