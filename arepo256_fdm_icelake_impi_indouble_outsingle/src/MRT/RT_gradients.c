 ...
 *
 * Initializes the gradients for only the RT variables.
 */
...
void init_gradients_RT(void)
{
      ...
      for(j = 0; j < DIMS; j++)
        {
          ...
          for(int k = 0; k < DIMS; k++)
            gradient_init_RT(&SphP[0].PT[num1][j][k], &RTPrimExch[0].PT[num1][j][k], SphP[0].RTGrad.dPT[num1][j][k],
  ...
  mpi_printf_rt(0, "INIT RT: %d/%d Gradients used.\n", N_Grad_RT, MAXRTGRADIENTS);
}
...
void gradient_init_RT(MyFloat *addr, MyFloat *addr_exch, MySingle *addr_grad, int type)
{
  if(N_Grad_RT == MAXRTGRADIENTS)
    {
      mpi_printf_rt(0, "Failed to register gradient, maximum of %d already reached\n", MAXGRADIENTS);
