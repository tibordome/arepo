...
void healray_update_cells(void);
void healray_restore_mesh(void);
void healray_test_error(void);
void healray_sanity_and_logging(void);
void healray_free(void);
      ...
      for(i = 0; i < NumGas; i++)
        {
          ...
          error = log10(fabs(SphP[i].HeatRate - heat_rate) / heat_rate);
          tot_error += error;
        }
  ...
  if(!HRD.RayMode)
    {
      ...
      if(HRD.TotEnergyInit)
        tot_energy_err = fabs(HRD.TotEnergyInit - tot_energy_absorb - tot_energy_esc) / HRD.TotEnergyInit;
      else
        tot_energy_err = 0.;

      if(tot_energy_err > 1e-10)
        terminate("HEALRAY: Unusually large error in total energy: %g. Something is wrong!", tot_energy_err);
    }
