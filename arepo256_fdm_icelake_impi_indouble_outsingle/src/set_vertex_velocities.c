...
static void validate_vertex_velocities_1d(void);
#endif
      ...
      if(SphP[i].Density > 0)
        {
          ...
          acc[0] += -SphP[i].Grad.dsgstPressure[0] / SphP[i].Density;
          acc[1] += -SphP[i].Grad.dsgstPressure[1] / SphP[i].Density;
          acc[2] += -SphP[i].Grad.dsgstPressure[2] / SphP[i].Density;
#endif
      ...
      if(d > 0 && fraction > 0)
        {
          ...
          double vmax = fmax(All.cf_atime * get_sound_speed(i), vel);
          if(v > vmax)
            v = vmax;
#endif
...
#ifdef ONEDIMS_SPHERICAL
void validate_vertex_velocities_1d(void)
{
      ...
      for(k = 0; k < 2; k++)
        {
          ...
          if((ID1 >= BOUNDARY_STICKY_MINID && ID1 < BOUNDARY_STICKY_MAXID) ||
             (ID2 >= BOUNDARY_STICKY_MINID && ID2 < BOUNDARY_STICKY_MAXID))
            {
