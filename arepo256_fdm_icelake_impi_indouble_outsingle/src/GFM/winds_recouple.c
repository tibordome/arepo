...
static void particle2in(data_in *in, int i, int firstnode)
{
  ...
  for(k = 0; k < GFM_RPROCESS_CHANNELS; k++)
    {
      in->MassRProcess[k] = STP(WindParticle[i].index).MassRProcess[k];
    }
#endif
#ifdef REFINEMENT_HIGH_RES_GAS
...
#endif
#ifdef REFINEMENT_CGM
  in->HighResMassCGM = STP(WindParticle[i].index).HighResMassCGM;
#endif
...
#endif
#ifdef GFM_RPROCESS_CHANNELS
  massrprocess = in->MassRProcess;
#endif
#ifdef REFINEMENT_HIGH_RES_GAS
  highresmass = in->HighResMass;
#endif
#ifdef REFINEMENT_CGM
  highresmasscgm = in->HighResMassCGM;
#endif
#ifdef TRACER_MC
      ...
      if(P[j].ID == cellID)
        {
          ...
          /* compute thermal energy variation due to wind particle thermal energy */
...
#ifdef GFM_RPROCESS_CHANNELS
          for(k = 0; k < GFM_RPROCESS_CHANNELS; k++)
            SphP[j].MassRProcess[k] += massrprocess[k];
#endif
#ifdef REFINEMENT_HIGH_RES_GAS
          SphP[j].HighResMass += highresmass;
#endif
#ifdef REFINEMENT_CGM
          SphP[j].HighResMassCGM += highresmasscgm;
#endif
