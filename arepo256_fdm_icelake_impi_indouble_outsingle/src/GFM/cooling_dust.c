...
double get_CoolingDustRate(double logT, double dust_to_gas_ratio, double ne, char dust_cool)
{
/* MRN grain distribution */
  ...
  if(dust_cool == 1)
    {
      if((logT > 5) && (logT < 9.0))
        return (dust_to_gas_ratio / 0.0075) * pow(10.0, 0.01575072 * logT * logT * logT * logT - 0.41930701 * logT * logT * logT +
                                                            3.87861885 * logT * logT - 13.66242274 * logT - 9.09918663);
      else
        return 0.0;
    }
  else
  ...
  if(dust_cool == 1)
    {
      ...
      double fac       = (dust_to_gas_ratio * m_p) / (HYDROGEN_MASSFRAC * 4.0 / 3.0 * 3.141592 * a_cm * a_cm * a_cm * rho_grain) * ne;
      return fac * H_ne;
    }
  else
    return 0.0;
