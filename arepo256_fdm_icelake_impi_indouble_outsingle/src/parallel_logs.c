...
void parallel_log_allocate(struct plog_data *data, int extendable)
{
  ...
  if(data->extendable)
    {
      data->text         = (char *)mymalloc_movable(&data->text, "pLogText", data->size);
      data->text_lengths = (int *)mymalloc_movable(&data->text_lengths, "pLogTextLengths", data->count * sizeof(int));
    }
  else
    {
      data->text         = (char *)mymalloc("pLogText", data->size);
      data->text_lengths = (int *)mymalloc("pLogTextLengths", data->count * sizeof(int));
    }
}
  ...
  if(data->offset + len > data->size || data->text_count == data->count)
    {
      if(!data->extendable)
      ...
      else
        {
          ...
          data->text         = (char *)myrealloc_movable(data->text, data->size);
          data->text_lengths = (int *)myrealloc_movable(data->text_lengths, data->count * sizeof(int));
        }
    }

  memcpy(&data->text[data->offset], text, len);
          ...
          if(text_size > 0)
            {
              ...
              temp.text         = (char *)mymalloc("text", text_size);
              temp.text_lengths = (int *)mymalloc("textlengths", text_count * sizeof(int));
