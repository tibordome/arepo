 ...
 *
 * \file        src/OTVET/otvet_chem_ps2011.c
 * \date        MM/YYYY
 * \author
 * \brief
 * \details
 ...
 * You must use this function if using the SMUGGLE_RADPRESS_OPT_THICK option.
 */
...
#if defined(OTVET) && defined(OTVET_CHEMISTRY_PS2011)
 ...
 */
int otvet_rate_ODEs(double t, const double y[], double f[], void *params)
{
...
void otvet_update_chemistry(void)
{
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
...
#ifdef OTVET_NOCOLLISION_IONIZATION
      gamma = 0.;
#endif
      ...
      nHI    = SphP[i].HI;
      nHII   = SphP[i].HII;
      nGamma = SphP[i].n_gamma[0] / P[i].Mass * All.cf_a3inv / nH;
      sigma  = otvet_sigma_HI[0];

      int calculated_flag = 0;

      double d_nGamma = -c_light * sigma * nH * dt * nHI * nGamma; /* linear estimate of the photo abundance change */

      if(fabs(d_nGamma) <= 0.05 * nGamma) /* we are nearly in photoionization equilibrium, and/or the photon density is so large that
                                             it should change little */
        {
      ...
      if(calculated_flag == 0 && nGamma < 1.0e-2 * nHI) /* here photoionization cannot change the neutral fraction much */
        {
          ...
          if(fac > 0)
            {
              /* nGamma_new = (-(1 + fac * (1 - nHII - nGamma)) + sqrt(pow(1 + fac * (1 - nHII - nGamma), 2) + 4 * fac * nGamma)) / (2
               * * fac); */
      ...
      if(calculated_flag == 0) /* if no accurate solution has been found yet, integrate the problem with an ODE solver */
        {
          ...
          gsl_odeiv_system sys = {otvet_rate_ODEs, NULL, 2, coeff};
          ...
          nGamma_new = y[1];
          if(isnan(nGamma_new))
            terminate("LVS5: nGamma_new is nan\n");
        }
      ...
      SphP[i].n_gamma[0] = nGamma_new / All.cf_a3inv * P[i].Mass * nH;
      ...
      nHeII /= 1.0 + F * SphP[i].Ne + D * SphP[i].Ne + J * SphP[i].Ne +
               ((F * SphP[i].Ne - E * SphP[i].Ne) / (1.0 + E * SphP[i].Ne) * D * SphP[i].Ne);
  ...
  if(ThisTask == 0)
    {
      fprintf(FdOTVET, "%g %g ", All.Time, mean_nHI_all / All.BoxSize / All.BoxSize / All.BoxSize);
#ifndef OTVET_INCLUDE_HE
      fprintf(FdOTVET, "\n");
#else
  ...
  if(ThisTask == 0)
    {
      printf("Ionization balance solver: fraction of particle requiring ODE: %g, average number of iterations=%g\n",
