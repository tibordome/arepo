...
static void particle2in(data_in *in, int i, int firstnode)
{
  ...
  //  in->total_dens = TempCellData[i].total_dens;

  in->total_dens = P[i].MeanDensity;
      ...
      if(calc == 0)
        {
          ...
          //	  TempCellData[i].total_dens = out->total_dens;

          P[i].MeanDensity = out->total_dens;
      ...
      if(P[i].Type == 4)
        {
          ...
          if(P[i].TagSNe == 0 || (P[i].TagSNe == 1 && P[i].FeedbackDone > 0))
            continue;
#endif
...
#ifdef MRT_LOCAL_FEEDBACK
          if(P[i].TagSNe == 1 && Age + dt > SNTimeDelay && P[i].FeedbackDone == 0)
            prob = 1.0;
          else
            prob = -1.0;
#endif
      ...
      if(P[i].Type == 0)
        {
          ...
          if(rand < prob)
            {
              TempCellData[i].flag_feedback = 1;
              pl_fprintf(&plog, "CMS_FEEDBACK[%d]: center t,x,y,z,ty: %f %f %f %f %d\n", ThisTask, All.Time, SphP[i].Center[0],
                         SphP[i].Center[1], SphP[i].Center[2], P[i].Type);
              printf("CMS_FEEDBACK[%d]: center t,x,y,z: %f %f %f %f %d\n", ThisTask, All.Time, SphP[i].Center[0], SphP[i].Center[1],
                     SphP[i].Center[2], P[i].Type);
            }
        }
#endif
#endif
                  ...
                  if(TempCellData[i].flag_feedback && (TempCellData[i].Ncells < 32 || TempCellData[i].Ncells > 32))
                    {
                      ...
                      else if(TempCellData[i].Ncells > 32)
                        {
                          TempCellData[i].h_h -= TempCellData[i].dh;
...
static int find_feedback_cells_evaluate(int target, int mode, int thread_id)
{
  ...
  double total_vol, fkin, fcr, SNEnergy, SNMassReturn, deltap, SNMetalReturn, injected_photons, total_dens;
  ...
  double local_vol_sum, local_aterm1, local_b, local_c, local_tot_ekin_b, local_dens_sum;
