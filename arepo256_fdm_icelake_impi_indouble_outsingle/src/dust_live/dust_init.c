...
#ifdef DL_RADIATION
  init_dust_radiation();
  mpi_printf("DUST_LIVE: Radiation coefficients initialized.\n");
#endif
}
...
void spline_interpolate_condeff(int elem)
{
  ...
  if((elem == element_index_Carbon) || (elem == element_index_Oxygen) || (elem == element_index_Magnesium) ||
     (elem == element_index_Silicon) || (elem == element_index_Iron))
    {
          ...
          for(int k = 0; k < GFM_N_MASS_BINS; k++)
            {
              ...
              GSD.AGB_CondEff_spline[i][elem][k] = fmin(1.0, pow(10.0, result));
            }
        }
    }
  else
          ...
          for(int k = 0; k < GFM_N_MASS_BINS; k++)
            {
              if((GSD.AGB_CondEff_spline[i][j][k] > 1.0) || (GSD.AGB_CondEff_spline[i][j][k] < 0.0))
                terminate("Invalid AGB condensation efficiency %g for element %s i=%d j=%d k=%d!", GSD.AGB_CondEff_spline[i][j][k],
                          ElementNames[j], i, j, k);
            }
        }
    }
...
void init_grain_size_distribution(void)
{
      ...
      for(int j = 0; j <= k; j++)
        {
          ...
          GSD.I_2_kj_Prefac[k][j] = ((aj - ajp1) * (ak - akp1) *
                                     (2 * pow(aj, 2) + 2 * aj * ajp1 + 2 * pow(ajp1, 2) + 3 * aj * (ak + akp1) +
                                      3 * ajp1 * (ak + akp1) + 2 * (pow(ak, 2) + ak * akp1 + pow(akp1, 2)))) /
                                    6.;
  ...
  GSD.SNIINumFrac = integrate_imf(log10(All.SNII_MinMass_Msun), log10(All.SNII_MaxMass_Msun), INTEGRATE_IMF_NUMBER, NULL);
#endif
  ...
  /* If refinement naturally sets a maximum dust particle mass, we can see
   * whether the enclosed mass constraint for dust-dust neighbor searches uses
   * reasonable values. */
#ifdef DL_REFINEMENT
  /* All.TargetGasMass has not yet been set, so compute it manually */
  double TargetGasMass          = All.TargetGasMassFactor * All.ReferenceGasPartMass;
  double min_enclosed_mass      = (All.DesNumNgbDust - All.MaxNumNgbDeviationDust) * All.DustTargetFrac * TargetGasMass;
  double max_enclosed_mass      = (All.DesNumNgbDust + All.MaxNumNgbDeviationDust) * All.DustTargetFrac * TargetGasMass;
  double max_dust_particle_mass = All.DustMaxFrac * TargetGasMass;

  /* Need to avoid the situation where adding or removing one dust particle
   * from search kernel oscillates us between having too much and too little
   * mass in the kernel, even if we have a high-mass dust neighbor. */
  if((0.5 * (max_enclosed_mass - min_enclosed_mass) < max_dust_particle_mass) && (ThisTask == 0))
    {
      mpi_printf("DUST_LIVE: min_enclosed_mass=%g, max_enclosed_mass=%g, max_dust_particle_mass=%g\n", min_enclosed_mass,
                 max_enclosed_mass, max_dust_particle_mass);
      terminate(
          "DUST_LIVE: Set new values for All.DesNumNgbDust and All.MaxNumNgbDeviationDust so one dust particle's mass is not so big "
          "relative to difference in allowed enclosed kernel masses!\n");
    }
#endif
#endif
}
...
{
  char setname[MAXLEN_PATH];
  hid_t file_id = my_H5Fopen(All.GrainRPPath, H5F_ACC_RDONLY, H5P_DEFAULT);
  hid_t dataset;

  int n_size;
  sprintf(setname, "/NSizes");
  dataset = my_H5Dopen(file_id, setname);
  my_H5Dread(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &n_size, setname);
  my_H5Dclose(dataset, setname);

  int n_lam;
  sprintf(setname, "/NWavelengths");
  dataset = my_H5Dopen(file_id, setname);
  my_H5Dread(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &n_lam, setname);
  my_H5Dclose(dataset, setname);

  double *sizes = (double *)mymalloc("size", n_size * sizeof(double));
  sprintf(setname, "/Sizes");
  dataset = my_H5Dopen(file_id, setname);
  my_H5Dread(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, sizes, setname);
  my_H5Dclose(dataset, setname);

  double *lams = (double *)mymalloc("lams", n_lam * sizeof(double));
  sprintf(setname, "/Wavelengths");
  dataset = my_H5Dopen(file_id, setname);
  my_H5Dread(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, lams, setname);
  my_H5Dclose(dataset, setname);

  double *Q_pr_gra = (double *)mymalloc("Q_pr_gra", n_size * n_lam * sizeof(double));
  sprintf(setname, "/Q_pr_gra");
  dataset = my_H5Dopen(file_id, setname);
  my_H5Dread(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, Q_pr_gra, setname);
  my_H5Dclose(dataset, setname);

  double *Q_pr_sil = (double *)mymalloc("Q_pr_sil", n_size * n_lam * sizeof(double));
  sprintf(setname, "/Q_pr_sil");
  dataset = my_H5Dopen(file_id, setname);
  my_H5Dread(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, Q_pr_sil, setname);
  my_H5Dclose(dataset, setname);

  double *Q_abs_gra = (double *)mymalloc("Q_abs_gra", n_size * n_lam * sizeof(double));
  sprintf(setname, "/Q_abs_gra");
  dataset = my_H5Dopen(file_id, setname);
  my_H5Dread(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, Q_abs_gra, setname);
  my_H5Dclose(dataset, setname);

  double *Q_abs_sil = (double *)mymalloc("Q_abs_sil", n_size * n_lam * sizeof(double));
  sprintf(setname, "/Q_abs_sil");
  dataset = my_H5Dopen(file_id, setname);
  my_H5Dread(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, Q_abs_sil, setname);
  my_H5Dclose(dataset, setname);

  /* Do interpolation in log space.  User can exponentiate the result to get
   * real interpolated value. */
  for(int i = 0; i < n_size; i++)
    sizes[i] = log10(sizes[i]);
  ...
  for(int i = 0; i < n_size; i++)
    {
      for(int j = 0; j < n_lam; j++)
        {
          int idx        = i * n_lam + j;
          Q_pr_gra[idx]  = log10(Q_pr_gra[idx]);
          Q_pr_sil[idx]  = log10(Q_pr_sil[idx]);
          Q_abs_gra[idx] = log10(Q_abs_gra[idx]);
          Q_abs_sil[idx] = log10(Q_abs_sil[idx]);
        }
    }

  /* From GSL documentation: data points are assumed to be stored in a 1D
   * with the column-major ordering z_ij = za[j*xsize + i].  Here, i = grain
   * size and j = wavelength.  This is the opposite of how the data is read
   * in from HDF5. */
  double *Q_pr_gra2  = (double *)mymalloc("Q_pr_gra2", n_size * n_lam * sizeof(double));
  double *Q_pr_sil2  = (double *)mymalloc("Q_pr_sil2", n_size * n_lam * sizeof(double));
  double *Q_abs_gra2 = (double *)mymalloc("Q_abs_gra2", n_size * n_lam * sizeof(double));
  double *Q_abs_sil2 = (double *)mymalloc("Q_abs_sil2", n_size * n_lam * sizeof(double));
  for(int i = 0; i < n_size; i++)
    {
      for(int j = 0; j < n_lam; j++)
        {
          int idx          = i * n_lam + j;
          int idx2         = j * n_size + i;
          Q_pr_gra2[idx2]  = Q_pr_gra[idx];
          Q_pr_sil2[idx2]  = Q_pr_sil[idx];
          Q_abs_gra2[idx2] = Q_abs_gra[idx];
          Q_abs_sil2[idx2] = Q_abs_sil[idx];
        }
    }

  const gsl_interp2d_type *tbi = gsl_interp2d_bilinear;

  DustRP_gra.spline_Q_pr = gsl_spline2d_alloc(tbi, n_size, n_lam);
  DustRP_gra.a_acc       = gsl_interp_accel_alloc();
  DustRP_gra.lam_acc     = gsl_interp_accel_alloc();

  DustRP_sil.spline_Q_pr = gsl_spline2d_alloc(tbi, n_size, n_lam);
  DustRP_sil.a_acc       = gsl_interp_accel_alloc();
  DustRP_sil.lam_acc     = gsl_interp_accel_alloc();

  gsl_spline2d_init(DustRP_gra.spline_Q_pr, sizes, lams, Q_pr_gra2, n_size, n_lam);
  gsl_spline2d_init(DustRP_sil.spline_Q_pr, sizes, lams, Q_pr_sil2, n_size, n_lam);

  DustRP_gra.spline_Q_abs = gsl_spline2d_alloc(tbi, n_size, n_lam);
  DustRP_gra.a_acc_abs    = gsl_interp_accel_alloc();
  DustRP_gra.lam_acc_abs  = gsl_interp_accel_alloc();

  DustRP_sil.spline_Q_abs = gsl_spline2d_alloc(tbi, n_size, n_lam);
  DustRP_sil.a_acc_abs    = gsl_interp_accel_alloc();
  DustRP_sil.lam_acc_abs  = gsl_interp_accel_alloc();

  gsl_spline2d_init(DustRP_gra.spline_Q_abs, sizes, lams, Q_abs_gra2, n_size, n_lam);
  gsl_spline2d_init(DustRP_sil.spline_Q_abs, sizes, lams, Q_abs_sil2, n_size, n_lam);
  ...
  my_H5Fclose(file_id, All.GrainRPPath);

  /* Calculate some helpers for the MRT bins that are useful when calculating
      ...
      if(b < UV_BINS)
        {
          ...
          DRT.meanE[b] = 0.5 * (nu[b] + nu[b + 1]); /* eV */
#else
          /* If a single UV bin, assume 13.6 eV. */
          DRT.meanE[b] = 13.6; /* eV */
#endif
          /* For the UV bins, the units are 1.0e63 photons.  Need to
           * account for energy too. */
          DRT.photons2energy[b] = 1.0e63 * DRT.meanE[b] * ELECTRONVOLT_IN_ERGS / All.UnitEnergy_in_cgs;
        }
      else
        {
          /* If an IR bin, assume 0.25 eV (5 micron). */
          DRT.meanE[b] = 0.25; /* eV */
          /* For the IR bin, the units are already internal energy. */
          DRT.photons2energy[b] = 1.0;
        }

      /* Calculate wavelength in cgs units, then convert to microns. */
      DRT.lam[b] = (PLANCK * c_speed * All.UnitVelocity_in_cm_per_s) / (DRT.meanE[b] * ELECTRONVOLT_IN_ERGS);
      DRT.lam[b] *= 1.0e4; /* um */
    }
}
#endif
