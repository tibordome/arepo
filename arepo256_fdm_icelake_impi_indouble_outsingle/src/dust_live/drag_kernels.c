...
#endif
#ifdef DL_WINDS
      DustParticle[i].WindMassRate = out->WindMassRate;
      DustParticle[i].WindVel      = out->WindVel;
#endif
      for(int k = 0; k < 3; k++)
        {
...
#if defined(DL_WINDS) && defined(GFM_BIPOLAR_WINDS)
#if(GFM_BIPOLAR_WINDS == 3)
          DustParticle[i].DensGasAngMomentum[k] = out->DensGasAngMomentum[k];
#else
          DustParticle[i].GroupVel[k]     = out->GroupVel[k];
          DustParticle[i].GroupGravAcc[k] = out->GroupGravAcc[k];
#endif
#endif
        }
    }
          ...
          if(r2 < h2)
            {
...
#endif
              double SphP_temp =
                  (mu * PROTONMASS) / BOLTZMANN * GAMMA_MINUS1 * SphP[j].Utherm * All.UnitPressure_in_cgs / All.UnitDensity_in_cgs;
#ifdef USE_SFR
              if(SphP[j].Sfr > 0.0)
                SphP_temp = 1.0e4;
#endif
...
#ifdef DL_SNE_DESTRUCTION
              double n_cgs =
                  SphP[j].Density * All.cf_a3inv * All.UnitDensity_in_cgs * All.HubbleParam * All.HubbleParam / PROTONMASS; /* cm^-3 */
              /* Just to avoid potentially small, negative values in MassMetallicity. */
              double Z_mod   = fmax(SphP[j].MassMetallicity / P[j].Mass / 0.0127, 0.0) + 0.039;
              ...
              if(SphP[j].Sfr > 0.0)
                {
                  /* CloudFrac is anyways zero if not star-forming */
              ...
              if(SphP[j].Sfr > 0.0)
                {
                  /* SphP.MassLoading and SphP.WindVel entries are only
                   * up-to-date for star-forming cells */
                  out.WindMassRate += (NORM_COEFF * h3 * wk) * (SphP[j].Sfr * SphP[j].MassLoading);
                  out.WindVel += weight_fac * SphP[j].WindVel;
                }
#endif

              for(k = 0; k < 3; k++)
                {
                  out.LocalGasVelocity[k] += weight_fac * SphP[j].Momentum[k] / P[j].Mass;
...
#if defined(DL_WINDS) && defined(GFM_BIPOLAR_WINDS)
#if(GFM_BIPOLAR_WINDS == 3)
                  out.DensGasAngMomentum[k] += weight_fac * SphP[j].DensGasAngMomentum[k];
#else
                  out.GroupVel[k] += weight_fac * SphP[j].GroupVel[k];
                  out.GroupGravAcc[k] += weight_fac * SphP[j].GroupGravAcc[k];
#endif
#endif
                }
            }
