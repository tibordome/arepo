...
double ADM_CoolingRateFromU(double u, double rho, double *ne_guess)
{
  ...
  *ne_guess  = ADM_gs.ne;
  ...
  /* Bremsstrahlung */
  LambdaFF = gff * ADM_gs.nHp * ADM_gs.ne * 3.66e-27 * pow(All.ADMFineStructureConstant / 1e-2, 3) *
             pow(511.0 / All.ADMElectronMassInkeV, 1.5) * pow(Tplasma, 0.5);

  /* Compton cooling off the dark CMB */
  LambdaCmptn = ADM_gs.ne * 1.915e-37 * (Tplasma - TdarkCMB) * pow(511 / All.ADMElectronMassInkeV, 3.0) *
                pow(All.ADMFineStructureConstant / 1e-2, 2) * pow(TdarkCMB, 4);
...
void ADM_cool_cell(int i)
{
  ...
  unew       = ADM_DoCooling(fmax(All.MinEgySpec, SphP[i].Utherm), dens * All.cf_a3inv, dtcool, &ne);
  SphP[i].Ne = ne;
