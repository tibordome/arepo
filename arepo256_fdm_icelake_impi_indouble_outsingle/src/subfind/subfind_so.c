...
double subfind_overdensity(void)
{
  ...
  if(All.ComovingIntegrationOn)  // TODO fix?
    z = 1 / All.Time - 1;
  else
    z = 0;
              ...
              if(Group[i].Nsubs > 0)
                {
                  ...
                  switch(rep)
                    {
                      case 0:
                        Right[i] = Left[i] = GroupCatLocal[i].R_Mean200;  // previously GroupCat[Group[i].GrNr]
                        break;
                      case 1:
                        Right[i] = Left[i] = GroupCatLocal[i].R_TopHat200;
                        break;
                      case 2:
                        Right[i] = Left[i] = GroupCatLocal[i].R_Crit200;
                        break;
                      case 3:
                        Right[i] = Left[i] = GroupCatLocal[i].R_Crit500;
                        break;
                    }
#else
                  if(mainstep == 0)
...
#ifdef SUBFIND_EXTENDED_PROPERTIES
      double *egypot = (double *)mymalloc("egypot", Ngroups * sizeof(double));
...
#define FACT2 0.86602540 /* sqrt(3)/2 */
          ...
          if(no < Tree_MaxPart) /* single particle */
            {
              ...
              if(mainstep == 0)
                    ...
                    if(P[p].Type == i)
                      {
                        ...
                        if((P[p].Type == 4) && (STP(p).BirthTime < 0))
                          LenType[GFM_WINDS_SAVE_PARTTYPE]++;
                        else
#endif
                          LenType[i]++;
#ifdef GFM_WINDS
                        if((P[p].Type == 4) && (STP(p).BirthTime < 0))
                          {
                            MassType[4] -= P[p].Mass;  // subtract wind mass from stellar mass
                            MassType[0] += P[p].Mass;  // add wind mass to gas mass
                            // LenType[i]--; // do not do this: wind is written as parttype 4 (star) for LenType
                          }
#endif
                      }
              ...
              else if(mainstep == 1)
                {
                  ...
                  int ptype = P[p].Type;
#ifdef GFM_WINDS
                  /* count wind as gas for mass, but not for LenType to be consistent with the rest of the code */
                  if(P[p].Type == 4 && STP(p).BirthTime < 0)
                    ptype = 0;
#endif
              ...
              else if(mainstep == 2)
                {
                  ...
                  // Jtot[0] = Jdm[0] + Jgas[0] + Jstars[0];
                  // Jtot[1] = Jdm[1] + Jgas[1] + Jstars[1];
                  // Jtot[2] = Jdm[2] + Jgas[2] + Jstars[2];
