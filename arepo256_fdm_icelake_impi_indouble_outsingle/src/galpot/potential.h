...
{
 public:
  Stupid2DArray(int n1_, int n2_) : n1(n1_), n2(n2_) { arr = new double[n1 * n2]; }
  ~Stupid2DArray() { delete[] arr; }
...
class AxisymmetricMultipolePotential : public AxisymmetricPotential
{
 public:
  const double G;
  const double pi;
  AxisymmetricMultipolePotential(double G, const AxisymmetricDensity &, int nr, int npoly = 6, int ngauss = 8);
  ...
  virtual double Phi(const Vec3 &x, double t = 0.0) { return AxisymmetricPotential::Phi(x); }
  virtual Vec3 dPhidx(const Vec3 &x, double t = 0.0) { return AxisymmetricPotential::dPhidx(x); }
...
class BGSBUBarDensity : public AxisymmetricDensity
{
 public:
  ...
  const double rho0;
  BGSBUBarDensity(double rho0_ = 0.069, double alf_ = 1.75, double bet_ = 3.5, double a0_ = 1.2, double qm_ = 0.75, double rmin = 0,
                  double rmax = 0)
      : AxisymmetricDensity(rminmax(rmin, a0_ * 1e-3), rminmax(rmax, a0_ * 1e2)), alf(alf_), bet(bet_), a0(a0_), qm(qm_), rho0(rho0_)
  {
  }
  double operator()(double R, double z) const
  {
    ...
    if(a < a0)
      return rho0 * pow(a / a0, -alf);
    return rho0 * pow(a / a0, -bet);
  }

 private:
  double rminmax(double choice1, double choice2) { return choice1 != 0 ? choice1 : choice2; }
};

class BGSBUBarPotential : public AxisymmetricMultipolePotential
{
 public:
  BGSBUBarPotential(double G, const BGSBUBarDensity &rho_in, int nr = 100, int npoly = 6, int ngauss = 8)
      : AxisymmetricMultipolePotential(G, rho_in, nr, npoly, ngauss), rho_priv(rho_in)
  {
  }
...
class ExpBarDensity : public AxisymmetricDensity
{
 public:
  ...
  ExpBarDensity(double rho0_ = 0.2, double a0_ = 1.5, double q_ = 0.5, double rmin = 0, double rmax = 0)
      : AxisymmetricDensity(rminmax(rmin, a0_ * 1e-3), rminmax(rmax, a0_ * 1e2)), rho0(rho0_), a0(a0_), q(q_)
  {
  }
  double operator()(double R, double z) const
  {
...
{
 public:
  ExpBarPotential(double G, const ExpBarDensity &rho_in, int nr = 100, int npoly = 6, int ngauss = 8)
      : AxisymmetricMultipolePotential(G, rho_in, nr, npoly, ngauss), rho_priv(rho_in)
  {
  }
  double rho(const Vec3 &x) const;
...
class ExpDiskDensity : public AxisymmetricDensity
{
 public:
  ...
  ExpDiskDensity(double Sigma0_ = 1.83e8, double zd_ = 9., double Rd_ = 36., double rmin = 0, double rmax = 0)
      : AxisymmetricDensity(rminmax(rmin, Rd_ * 1e-3), rminmax(rmax, Rd_ * 1e2)), Sigma0(Sigma0_), Rd(Rd_), zd(zd_)
  {
  }
  double operator()(double R, double z) const { return (Sigma0 / (2. * zd)) * exp(-abs(z) / zd - R / Rd); }
...
{
 public:
  ExpDiskPotential(double G, const ExpDiskDensity &rho_in, int nr = 100, int npoly = 6, int ngauss = 8)
      : AxisymmetricMultipolePotential(G, rho_in, nr, npoly, ngauss), rho_priv(rho_in)
  {
  }
  double rho(const Vec3 &x) const;
...
class DPLDensity : public AxisymmetricDensity
{
 public:
  ...
  DPLDensity(double rho0_ = 8.54e3, double a_ = 196., double alpha_ = 1., double beta_ = 3., double rmin = 0, double rmax = 0)
      : AxisymmetricDensity(rminmax(rmin, a_ * 1e-3), rminmax(rmax, a_ * 1e2)), rho0(rho0_), a(a_), alpha(alpha_), beta(beta_)
  {
  }
  double operator()(double R, double z) const
  {
    double eps = 0.01;
    double r   = sqrt(eps * eps + R * R + z * z); /*added a softening eps rjs*/
...
class McMillanBulgeDensity : public AxisymmetricDensity
{
 public:
  ...
  const double rho0;
  McMillanBulgeDensity(double rho0_ = 9.84e7, double alf_ = 1.8, double acut_ = 21., double a0_ = 0.75, double qm_ = 0.5,
...
{
 public:
  McMillanBulgePotential(double G, const McMillanBulgeDensity &rho_in, int nr = 100, int npoly = 6, int ngauss = 8)
      : AxisymmetricMultipolePotential(G, rho_in, nr, npoly, ngauss), rho_priv(rho_in)
  {
  }
  double rho(const Vec3 &x) const;
...
class ModifiedMcMillanBulgeDensity : public AxisymmetricDensity
{
 public:
  const double alf, a0, acut, qm;
  const double rho0;
  ModifiedMcMillanBulgeDensity(double rho0_ = 9.84e7, double alf_ = 1.8, double a0_ = 1.0, double acut_ = 21., double qm_ = 0.5,
                               double rmin = 0., double rmax = 0.)
      : AxisymmetricDensity(rminmax(rmin, acut_ * 1e-3), rminmax(rmax, acut_ * 1e2)),
        alf(alf_),
        a0(a0_),
...
{
 public:
  ModifiedMcMillanBulgePotential(double G, const ModifiedMcMillanBulgeDensity &rho_in, int nr = 100, int npoly = 6, int ngauss = 8)
      : AxisymmetricMultipolePotential(G, rho_in, nr, npoly, ngauss), rho_priv(rho_in)
  {
  }
  double rho(const Vec3 &x) const;
...
{
 public:
  DPLPotential(double G, const DPLDensity &rho_in, int nr = 100, int npoly = 6, int ngauss = 8)
      : AxisymmetricMultipolePotential(G, rho_in, nr, npoly, ngauss), rho_priv(rho_in)
  {
  }
  double rho(const Vec3 &x) const;
...
{
 public:
  SpiralArmPotential(double Sr0in, double St0in, double SNarmsin, double Sphirin, double Salphain, double SHzin, double Sp0in,
                     double Srsin, double Gin);
...
{
 public:
  MovingNFWPotential(double G, const DPLDensity &rho_in, int nr, int npoly, int ngauss, const Vec3 x0_, const Vec3 v_)
      : AxisymmetricMultipolePotential(G, rho_in, nr, npoly, ngauss), rho_priv(rho_in), x0(x0_), v(v_)
  {
  }
