          ...
          do
            {
              ...
              double tetra_vol =
                  calculate_tetra_volume(p0, p1, p2, p3);  // more geom. precision if calculate exact intersection vol, but not needed
...
double get_phi(point *dp0, point *dp1, point *dp2, point *dp_ref)
{
  ...
  // calculate phi
  phi = 0.5 *
        ((A[1][0] + A[0][0]) * (dp1->x - dp0->x) + (A[1][1] + A[0][1]) * (dp1->y - dp0->y) + (A[1][2] + A[0][2]) * (dp1->z - dp0->z));
  phi += 0.5 *
         ((A[2][0] + A[1][0]) * (dp2->x - dp1->x) + (A[2][1] + A[1][1]) * (dp2->y - dp1->y) + (A[2][2] + A[1][2]) * (dp2->z - dp1->z));
  phi += 0.5 *
         ((A[0][0] + A[2][0]) * (dp0->x - dp2->x) + (A[0][1] + A[2][1]) * (dp0->y - dp2->y) + (A[0][2] + A[2][2]) * (dp0->z - dp2->z));
  return phi;
}
...
{
  double det = A[0][2] * A[1][1] * A[2][0] - A[0][1] * A[1][2] * A[2][0] - A[0][2] * A[1][0] * A[2][1] + A[0][0] * A[1][2] * A[2][1] +
               A[0][1] * A[1][0] * A[2][2] - A[0][0] * A[1][1] * A[2][2];
  *x = ((A[1][2] * A[2][1] - A[1][1] * A[2][2]) * b[0] + (-A[0][2] * A[2][1] + A[0][1] * A[2][2]) * b[1] +
        (A[0][2] * A[1][1] - A[0][1] * A[1][2]) * b[2]) /
       det;
  *y = ((-A[1][2] * A[2][0] + A[1][0] * A[2][2]) * b[0] + (A[0][2] * A[2][0] - A[0][0] * A[2][2]) * b[1] +
        (-A[0][2] * A[1][0] + A[0][0] * A[1][2]) * b[2]) /
       det;
  *z = ((A[1][1] * A[2][0] - A[1][0] * A[2][1]) * b[0] + (-A[0][1] * A[2][0] + A[0][0] * A[2][1]) * b[1] +
        (A[0][1] * A[1][0] - A[0][0] * A[1][1]) * b[2]) /
       det;
}

/** Get fluxes due to mesh motion - upwind scheme */
void get_A_fluxes(int k, int q, int qother, double face_normal_vel, double face_velx, struct state *st_L, struct state *st_R,
                  struct fluxes *flux)
{
...
void do_mhd_ct_source_terms(void)
{
  int idx, i;
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
...
int not_both_active(point *dp_ref, point *dp0)
{
  ...
  if((dp_ref->task == ThisTask) & (dp0->task == ThisTask))
    {
      if(SphP[i_ref].TimeLastBUpdate != SphP[i0].TimeLastBUpdate)
        return 1;
    }

  if((dp_ref->task != ThisTask) & (dp0->task == ThisTask))
    {
      if(PrimExch[i_ref].TimeLastBUpdate != SphP[i0].TimeLastBUpdate)
        return 1;
    }

  if((dp_ref->task == ThisTask) & (dp0->task != ThisTask))
    {
      if(SphP[i_ref].TimeLastBUpdate != PrimExch[i0].TimeLastBUpdate)
        return 1;
    }

  if((dp_ref->task != ThisTask) & (dp0->task != ThisTask))
    {
      if(PrimExch[i_ref].TimeLastBUpdate != PrimExch[i0].TimeLastBUpdate)
        return 1;
    }
