...
typedef struct tessellation_data
{
  ...
  int Ndt;
  int MaxNdt;        /*!< number of Delaunay tetrahedra */
...
int ngb_treefind_voronoi(tessellation *T, MyDouble searchcenter[3], MyFloat hsml, int target, int origin, int *startnode, int mode,
                         int *nexport, int *nsend_local, int id);
...
void face_get_viscous_fluxes(struct state_face *st_face, struct fluxes *flux, struct geometry *geom, double dyn_visc,
                             double bulk_visc);
#ifdef SECOND_DERIVATIVES
