...
#endif /* #ifndef FRACTION_TP */
/* fraction of particles selected for sphere
...
/*! \brief Routine that specifies what to do with local data.
 *
...
/*! \brief Routine that specifies what to do with imported data.
 *
 *  Part of the generic communication pattern.
 *
 *  \return void
...
/*! \brief Computes the two-point function.
 *
 *  \return void
 */
void twopoint(void)
{
  ...
  state_buffer = (char *)mymalloc("state_buffer", gsl_rng_size(random_generator));
...
/*! \brief Counts the pairs in a sphere.
 *
 *  \param[in] target Index in DataGet or P and SphP array.
 *  \param[in] mode Flag specifying whether to work on local or imported data.
...
/*! \brief Saves two-point correlation function to file.
 *
 *  \return void
 */
void twopoint_save(void)
{
  ...
  if(ThisTask == 0)
    {
      if(Argc < 6)
        file_path_sprintf(buf, "%s/correl_%03d.txt", All.OutputDir, RestartSnapNum);
      else
        {
          printf("Writing twopoint to %s\n", Argv[5]);
          file_path_sprintf(buf, "%s/correl_%03d.txt", Argv[5], RestartSnapNum);
        }
