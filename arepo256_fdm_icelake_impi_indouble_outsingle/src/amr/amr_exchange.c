  ...
  if(T->NexpListNode > 1)
    {
      ...
      for(j = 1; j < T->NexpListNode; j++)
        {
          if(T->expListNode[j].index != T->expListNode[new_nexp - 1].index ||
             T->expListNode[j].task != T->expListNode[new_nexp - 1].task)
            {
      ...
      if(recvTask < NTask)
        {
          if(T->Node_Send_count[recvTask] > 0 || T->Node_Recv_count[recvTask] > 0)
            {
              /* get the particles */
              MPI_Sendrecv(&nodeExchOut[T->Node_Send_offset[recvTask]], T->Node_Send_count[recvTask] * sizeof(node_exchange), MPI_BYTE,
                           recvTask, TAG_DENS_A, &nodeExchIn[T->Node_Recv_offset[recvTask]],
                           T->Node_Recv_count[recvTask] * sizeof(node_exchange), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
            }
        }
    }
      ...
      if(recvTask < NTask)
        {
          if(Mesh_Send_count[recvTask] > 0 || Mesh_Recv_count[recvTask] > 0)
            {
              /* get the particles */
              MPI_Sendrecv(&dpExchOut[Mesh_Send_offset[recvTask]], Mesh_Send_count[recvTask] * sizeof(point_exchange), MPI_BYTE,
                           recvTask, TAG_DENS_A, &dpExchIn[Mesh_Recv_offset[recvTask]],
                           Mesh_Recv_count[recvTask] * sizeof(point_exchange), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
            }
        }
    }
  ...
  if(T->Ndp + Mesh_nimport > T->MaxNdp)
    {
      ...
      T->DP     = (point *)myrealloc_movable(T->DP, T->MaxNdp * sizeof(point));
    }
  ...
  for(j = 0; j < Mesh_nimport; j++)
    {
      ...
      while(1)
        {
          if(th >= Ngb_MaxPart + Ngb_MaxNodes ||
             (th >= Ngb_MaxPart && th < Ngb_FirstNonTopLevelNode)) /* we are dealing with a ghost node or top node */
            {
              subnode = (((unsigned char)((xxb & mask) >> (shiftx--))) | ((unsigned char)((yyb & mask) >> (shifty--))) |
                         ((unsigned char)((zzb & mask) >> (shiftz--))));
...
{
  TIMER_START(CPU_AMR_EXCHANGE);
  amr_node_data *tmpExchExport, *tmpExchImport;
  int i;
  int place;
  int ngrp, recvTask;
  tessellation *T = &Mesh;
      ...
      if(recvTask < NTask)
        {
          if(Mesh.Node_Send_count[recvTask] > 0 || Mesh.Node_Recv_count[recvTask] > 0)
            {
              /* exchange the data */
              MPI_Sendrecv(&tmpExchExport[Mesh.Node_Send_offset[recvTask]], Mesh.Node_Send_count[recvTask] * sizeof(amr_node_data),
                           MPI_BYTE, recvTask, TAG_DENS_A, &tmpExchImport[Mesh.Node_Recv_offset[recvTask]],
                           Mesh.Node_Recv_count[recvTask] * sizeof(amr_node_data), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
            }
        }
    }
