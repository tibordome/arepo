...
/* will only be compiled in 2D case */
#if defined(AMR) && defined(TWODIMS) && !defined(ONEDIMS) && !defined(LONG_X) && !defined(LONG_Y)
...
#ifdef TRACER_FIELD
  tracer    = (float *)mymalloc("tracer", pixels_x * pixels_y * sizeof(float));
...
#ifdef TRACER_MC
  tracerMC    = (float *)mymalloc("tracerMC", pixels_x * pixels_y * sizeof(float));
  tracerMCsum = (float *)mymalloc("tracerMCsum", pixels_x * pixels_y * sizeof(float));
...
#ifdef VORONOI_VELOCITY_FIELD_2D
  velx    = (float *)mymalloc("velx", pixels_x * pixels_y * sizeof(float));
  velxsum = (float *)mymalloc("velxsum", pixels_x * pixels_y * sizeof(float));
  vely    = (float *)mymalloc("vely", pixels_x * pixels_y * sizeof(float));
  velysum = (float *)mymalloc("velysum", pixels_x * pixels_y * sizeof(float));
...
#endif
#ifdef TRACER_MC
                  tracerMC_L = (SphP[li].Density + SphP[li].Grad.drho[0] * l_dx + SphP[li].Grad.drho[1] * l_dy) *
                               get_number_of_tracers(li) * All.ReferenceTracerMCMass / P[li].Mass;
#endif
#ifdef VORONOI_VELOCITY_FIELD_2D
