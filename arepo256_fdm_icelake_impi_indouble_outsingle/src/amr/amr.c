...
void amr_treemodifylength(int delta_NgbMaxPart)
{
  ...
  if(Mesh.allocated > 2)
    {
      Mesh.Refflag =
          (int *)myrealloc_movable(Mesh.Refflag, (Ngb_MaxPart + delta_NgbMaxPart + Mesh.nodes_total + Mesh.Nghost_dp) * sizeof(int));
      memmove(&Mesh.Refflag[Ngb_MaxPart + delta_NgbMaxPart], &Mesh.Refflag[Ngb_MaxPart],
              sizeof(int) * Mesh.nodes_total + Mesh.Nghost_dp);
    }
#endif
}
...
double get_cell_radius(int i) { return 0.5 * amr_length[Mesh.DP[i].level]; }
