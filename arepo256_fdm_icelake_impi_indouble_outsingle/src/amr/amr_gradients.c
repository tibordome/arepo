...
#include "../proto.h"
    ...
    !defined(TVD_SLOPE_LIMITER_MINBEE)
              ...
              for(k = 0; k < N_Grad; k++)
                {
                  ...
                  if(All.ComovingIntegrationOn &&
                     (grad_elements[k].type == GRADIENT_TYPE_VELX || grad_elements[k].type == GRADIENT_TYPE_VELY ||
              ...
              for(dir = 0; dir < 3; dir++)
                {
                  ...
                  for(k = 0; k < N_Grad; k++)
                    {
                      MySingle *data = (MySingle *)(((char *)(&(SphP[node].Grad))) + grad_elements[k].offset_grad);

#ifdef TVD_SLOPE_LIMITER
                      MySingle *dataUl = (MySingle *)(((char *)(&(SphP[node].GradUl))) + grad_elements[k].offset_grad);
#endif
                  ...
                  for(side = -1; side <= 1; side += 2)
                    {
                      ...
                      MySingle *grad_vx = (MySingle *)(((char *)(&(SphP[node].Grad))) + GVelx->offset_grad);
                      MySingle *grad_vy = (MySingle *)(((char *)(&(SphP[node].Grad))) + GVely->offset_grad);
                      MySingle *grad_vz = (MySingle *)(((char *)(&(SphP[node].Grad))) + GVelz->offset_grad);
  ...
  if(Mesh.DP[node_other].level < Mesh.DP[node].level)
    {
      ...
      dx = NEAREST_X(pos[0] - Mesh.DP[node_other].x);
      dy = NEAREST_Y(pos[1] - Mesh.DP[node_other].y);
      dz = NEAREST_Z(pos[2] - Mesh.DP[node_other].z);
