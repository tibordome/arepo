...
/* spread sfr sources */
#if defined(RT_ADVECT) && defined(RT_SPREAD_SOURCE) && defined(USE_SFR) && defined(RT_HEALPIX_NSIDE)
              ...
              if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
                {
                  ...
                  MPI_Sendrecv(&SourceDataIn[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct sourcedata_in), MPI_BYTE,
                               recvTask, TAG_INJECT_A, &SourceDataGet[Recv_offset[recvTask]],
                               Recv_count[recvTask] * sizeof(struct sourcedata_in), MPI_BYTE, recvTask, TAG_INJECT_A, MPI_COMM_WORLD,
                               &status);
                }
            }
        }
