      ...
      if(DP[dp_upwind].task == ThisTask && DP[dp_upwind].index < NumGas)
        {
          ...
          if(Flag_Computed[cell_upwind])
            {
              SphP[i].ColumnDensity0 = SphP[cell_upwind].ColumnDensity1 + (RayEntryDistance[i] - RayExitDistance[cell_upwind]) *
                                                                              SphP[cell_upwind].Density * SphP[cell_upwind].nHI;
          ...
          if(DP[dp_upwind].task != ThisTask) /* the upstream cell is a ghost cell from another processor */
            {
              ...
              if(ColumnExch[cell_upwind].Flag_Computed)
                {
                  SphP[i].ColumnDensity0 =
                      ColumnExch[cell_upwind].ColumnDensity1 + (RayEntryDistance[i] - ColumnExch[cell_upwind].RayExitDistance) *
                                                                   ColumnExch[cell_upwind].Density * ColumnExch[cell_upwind].nHI;
  ...
  for(i = 0; i < NumGas; i++)
    {
      ...
      SphP[i].SourceDistance = sqrt(pow(SphP[i].Center[0] - source_pos[0], 2) + pow(SphP[i].Center[1] - source_pos[1], 2) +
                                    pow(SphP[i].Center[2] - source_pos[2], 2));
    }
      ...
      if(recvTask < NTask)
        {
          if(Mesh_Send_count[recvTask] > 0 || Mesh_Recv_count[recvTask] > 0)
            {
              /* get the particles */
              MPI_Sendrecv(&tmpColumnExch[Mesh_Send_offset[recvTask]], Mesh_Send_count[recvTask] * sizeof(struct columnexch), MPI_BYTE,
                           recvTask, TAG_DENS_A, &ColumnExch[Mesh_Recv_offset[recvTask]],
                           Mesh_Recv_count[recvTask] * sizeof(struct columnexch), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
            }
        }
    }
