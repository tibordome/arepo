...
void source_particle_create_list(void)
{
      ...
      if(recvTask < NTask)
        {
          if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
            {
              /* get the particles */
              MPI_Sendrecv(&SourcePartList[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct source_particle_data),
                           MPI_BYTE, recvTask, TAG_DENS_A, &SourcePartListGlobal[Recv_offset[recvTask]],
                           Recv_count[recvTask] * sizeof(struct source_particle_data), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
            }
        }
    }
...
void source_particle_update_list(void)
{
  struct source_particle_data *SourcePartList;
  SourcePartList = (struct source_particle_data *)mymalloc("SourcePartList", All.MaxPartSources * sizeof(struct source_particle_data));
...
double get_circumstellar_alpha_viscosity(double x, double y, double z, double rho, double press)
{
  ...
  if(All.TotPartSources == 0)
    {
      printf("total particles sources: %d\n", All.TotPartSources);
      terminate("I better stop!");
    }

  for(int k = 0; k < All.TotPartSources; k++)
    {
      if(SourcePartListGlobal[k].SurfaceTemp == 0)
        continue;
  ...
  r = sqrt(r2);
  h = All.SofteningTable[SourcePartListGlobal[ParentStarIndex].SofteningType];
  ...
  if(OmegaKep == 0.0)
    {
      printf("OmegaKep = %g \n", OmegaKep);
      printf("OmegaKep contributions: %g, %g, %g \n", All.G, SourcePartListGlobal[ParentStarIndex].Mass, fac);
      terminate("I better stop!");
    }
  ...
  double Csnd2;
#ifndef ISOTHERM_EQS
  Csnd2 = GAMMA * press / rho;
#else   // #ifndef ISOTHERM_EQS
  Csnd2       = All.IsoSoundSpeed * All.IsoSoundSpeed;
#endif  // #ifndef ISOTHERM_EQS #else
  return All.AlphaCoefficient * Csnd2 / OmegaKep;
...
#else  // #ifdef CENTRAL_MASS_POTENTIAL
#ifdef BINARY_POTENTIAL
  CentralMass = 1.0 - (All.BinaryMassRatio) / (1 + All.BinaryMassRatio);
#else   // #ifdef BINARY_POTENTIAL
  CentralMass = 1.0;
#endif  // #ifdef BINARY_POTENTIAL #else
#endif  // #ifdef CENTRAL_MASS_POTENTIAL #else
...
#endif  // #ifdef LOCALLY_ISOTHERM_DISK

  if(radius < DRin)
    {
      damping = (DRin - radius) / (DRin - All.inner_radius);
      // damping = sin(0.5 * M_PI * (DRin - radius)/(DRin - All.inner_radius));
      // damping = exp(-8 * (radius - All.inner_radius) * (radius - All.inner_radius)/(DRin - All.inner_radius) / (DRin -
      // All.inner_radius));
  ...
  if(radius > DRout)
    {
      ...
      // damping = exp(-8 * (radius - All.outer_radius) * (radius - All.outer_radius)/(DRout - All.outer_radius) / (DRout -
      // All.outer_radius));
