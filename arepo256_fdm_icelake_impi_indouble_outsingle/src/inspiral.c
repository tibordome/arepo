...
static void inspiral_compute_acceleration(void)
{
  struct wd wd1, wd2;
...
static void apply_friction_force(void)
{
  int idx;
  for(idx = 0; idx < TimeBinsGravity.NActiveParticles; idx++)
    {
      ...
      if(P[i].Type == 0)
        {
          double ekin_old = 0.5 *
                            (SphP[i].Momentum[0] * SphP[i].Momentum[0] + SphP[i].Momentum[1] * SphP[i].Momentum[1] +
...
void do_inspiral_source_terms_first_half(void) { apply_friction_force(); }
