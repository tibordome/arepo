  ...
  for(k = 1; k < GFM_N_CHEM_ELEMENTS; k++)
    {
      ...
      mpi_printf("TURBULENT_METALDIFFUSION: Finished loop with: iter=%d  delta1=%g delta1/delta0=%g  max-rel-change=%g\n", iter,
                 delta1, delta1 / delta0, glob_max_rel_change);
...
void mm_matrix_multiply(double *in, double *out, double *diag, double theta) { mm_matrix_multiply_isotropic(in, out, diag, theta); }
