...
int domain_determineTopTree(void)
{
  ...
  list_cost    = (double *)mymalloc_movable(&list_cost, "list_cost", sizeof(double) * NumPart);
  list_sphcost = (double *)mymalloc_movable(&list_sphcost, "listsph_cost", sizeof(double) * NumPart);
#ifdef AMR
  list_level                = (int *)mymalloc_movable(&list_level, "list_level", sizeof(int) * NumPart);
  ...
  RB_INIT(&queue_load);
  nload     = (struct mydata *)mymalloc("nload", limitNTopNodes * sizeof(struct mydata));
      ...
      for(struct mydata *np = RB_MIN(mytree, &queue_load); np != NULL; np = RB_NEXT(mytree, &queue_load, np))
        {
          ...
          if((np->workload > limit || (NTopleaves < All.MultipleDomains * NTask && count == 0) ||
              topNodes[np->topnode_index].Level >= All.MinLevelTopLeaf) &&
  ...
  if(NTopleaves < All.MultipleDomains * NTask)
    terminate("NTopleaves = %d < All.MultipleDomains * NTask = %d * %d = %d", NTopleaves, All.MultipleDomains, NTask,
  ...
  for(int k = 0; k < n; k++)
    {
      ...
      for(int j = 0; j < 8; j++)
        {
          int sub_ = topNodes[i].Daughter + j;
          worklist[k * 8 + j] =
              fac_work * topNodes[sub_].Cost + fac_worksph * topNodes[sub_].SphCost + fac_load * topNodes[sub_].Count;

          if(worklist[k * 8 + j] != 0)
            non_zero++;

#ifdef AMR
          levellist[k * 8 + j] = topNodes[sub_].MinCellLevel;
#endif
        }
    }
