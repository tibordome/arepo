...
/* will only be compiled in 3D case */
...
/*! \brief get the line segments of a Voronoi cell
 *
 *  \param sphp_index The index of the Voronoi cell
 *  \param dp_index The index of the corresponding Delaunay point
 *  \param segments (out) The array in which the line segments are stored
 *  \param nof_elements (out) The number of elements written in segments during this function call
 *  \param max_elements The maximum size of the segments array
 *
 *  Warning: The correspondance sphp_index == dp_index holds only for a global timestep!
 *
 */
...
#endif
      T->VF = (face *)myrealloc_movable(T->VF, T->MaxNvf * sizeof(face));
      ...
      if(prev != t && next != t)
        {
          ...
          VolFac = h * darea / 20.;
          Pxx    = VolFac * (2. * Dx * Dx + 2. * Ex * Ex + 2. * Fx * Fx + Dx * Ex + Dx * Ex + Dx * Fx + Dx * Fx + Ex * Fx + Ex * Fx);
          Pyy    = VolFac * (2. * Dy * Dy + 2. * Ey * Ey + 2. * Fy * Fy + Dy * Ey + Dy * Ey + Dy * Fy + Dy * Fy + Ey * Fy + Ey * Fy);
          Pzz    = VolFac * (2. * Dz * Dz + 2. * Ez * Ez + 2. * Fz * Fz + Dz * Ez + Dz * Ez + Dz * Fz + Dz * Fz + Ez * Fz + Ez * Fz);
          Pxy    = VolFac * (2. * Dx * Dy + 2. * Ex * Ey + 2. * Fx * Fy + Dx * Ey + Dy * Ex + Dx * Fy + Dy * Fx + Ex * Fy + Ey * Fx);
          Pxz    = VolFac * (2. * Dx * Dz + 2. * Ex * Ez + 2. * Fx * Fz + Dx * Ez + Dz * Ex + Dx * Fz + Dz * Fx + Ex * Fz + Ez * Fx);
          Pyz    = VolFac * (2. * Dy * Dz + 2. * Ey * Ez + 2. * Fy * Fz + Dy * Ez + Dz * Ey + Dy * Fz + Dz * Fy + Ey * Fz + Ez * Fy);
      ...
      if(TimeBinSynchronized[P[DP[p1].index].TimeBinHydro])
        {
          ...
          for(row = 0; row < 3; row++)
            for(col = 0; col < 3; col++)
              SphP[DP[p1].index].MomentOfInertia[row][col] += tensor1[row][col];
#endif
        }
    }
      ...
      if(TimeBinSynchronized[P[DP[p2].index].TimeBinHydro])
        {
          ...
          for(row = 0; row < 3; row++)
            for(col = 0; col < 3; col++)
              SphP[DP[p2].index].MomentOfInertia[row][col] += tensor2[row][col];
#endif
        }
    }
      ...
      for(k = 0; k < 2; k++)
        {
          ...
          if(DP[q].task == ThisTask)
            {
              /* this is a local ghost point */
              if(DP[q].index >= NumGas)
                p -= NumGas;
              ...
              if(Narea >= MaxNarea)
                {
                  ...
                  AreaList = (struct area_list_data *)myrealloc_movable(AreaList, MaxNarea * sizeof(struct area_list_data));
...
#endif
          T->DT  = (tetra *)myrealloc_movable(T->DT, T->MaxNdt * sizeof(tetra));
          T->DTC = (tetra_center *)myrealloc_movable(T->DTC, T->MaxNdt * sizeof(tetra_center));
          T->DTF = (char *)myrealloc_movable(T->DTF, T->MaxNdt * sizeof(char));
  ...
  if(flag == 3)
    {
      ...
      int *ttlist = (int *)mymalloc_movable(&ttlist, "ttlist", count * sizeof(int));
          ...
          if(ret_exact > 0) /* facet is illegal, because point lies inside */
            {
              ...
              else if(non_convex == 1)
                {
                  /* we might be able to make a 3-2 flip, or we deal with a convex edge on the outer hull */
                  ...
                  if(DT[tt].t[i] == DT[qq].t[j])
                    {
                      /* this means there is exactly one tetrahedron between
                       * them, i.e. we have found the third partner for the flip */
                      int ww;
                  ...
                  else
                    {
                      if(DT[DT[tt].t[i]].p[DT[tt].s[i]] == DPinfinity && DT[DT[qq].t[j]].p[DT[qq].s[j]] == DPinfinity)
                        {
                          /* this should not occur since we have embedded the
                           * points into a convex big triangle */
                          terminate("inconsistency: convex edge between points=%d %d on outer hull found",
                                    (int)(DT[tt].p[access_triangles[tip_index][convex_edge]]),
                                    (int)(DT[tt].p[access_triangles[tip_index][convex_edge < 2 ? convex_edge + 1 : 0]]));
                        }
                    }
                }
              else if(non_convex == 2)
                {
                  /* we might be able to make a 4-4 flip */
                  int i = convex_edge + 2;
                  if(i >= 3)
                    i -= 3;
                  ...
                  int j;
                  for(j = 0; j < 4; j++)
                    if(DT_[tt].p[i] == DT_[qq].p[j])
                      break;

                  if(DT_[DT_[tt].t[i]].p[DT_[tt].s[i]] == DT_[DT_[qq].t[j]].p[DT_[qq].s[j]])
                    {
                      /* ok, so we really have 4 tetra. The opposite points match up */
                      to_check[n_faces_to_check++] = tt;
                      to_check[n_faces_to_check++] = qq;
                      to_check[n_faces_to_check++] = DT_[tt].t[i];
                      to_check[n_faces_to_check++] = DT_[qq].t[j];
                      DTF[tt]                      = 0;
                      DTF[qq]                      = 0;
                      DTF[DT_[tt].t[i]]            = 0;
                      DTF[DT_[qq].t[j]]            = 0;
...
int convex_edge_test(tessellation *T, int tt, int tip, int *edgenr)
{
  ...
  if(status >= 0)
    {
      ...
      if(u > INSIDE_EPS && v > INSIDE_EPS && (1 - (u + v)) < -INSIDE_EPS)
        {
          /* 3-2 flip allowed since edge 1 is reflex */
          *edgenr = 1;
          return 1;
        }
      ...
      if(u < -INSIDE_EPS && v > INSIDE_EPS && (1 - (u + v)) > INSIDE_EPS)
        {
          /* 3-2 flip allowed since edge 2 is reflex */
          *edgenr = 2;
          return 1;
        }
  ...
  if(flag0 >= 0 && flag1 >= 0 && flag2 < 0)
    {
      /* 3-2 flip allowed since edge 0 is reflex */
      *edgenr = 0;
      return 1;
    }
  ...
  if(flag0 >= 0 && flag1 >= 0 && flag2 == 0)
    {
      /* 4-4 flip around edge 0 may be possible */
      *edgenr = 0;
      return 2;
    }
  ...
  if(flag0 >= 0 && flag1 == 0 && flag2 >= 0)
    {
      /* 4-4 flip around edge 2 may be possible */
      *edgenr = 2;
      return 2;
    }
  ...
  if(flag0 == 0 && flag1 >= 0 && flag2 >= 0)
    {
      /* 4-4 flip around edge 1 may be possible */
      *edgenr = 1;
      return 2;
    }
...
void make_an_edge_split(tessellation *T, int tt0, int edge_nr, int count, int pp, int *ttlist)
{
  ...
  tlist       = (tetra **)mymalloc("tlist", count * sizeof(tetra *));
  t_orig_list = (tetra **)mymalloc("t_orig_list", count * sizeof(tetra *));
  i_list      = (int *)mymalloc("i_list", sizeof(int) * count);
  j_list      = (int *)mymalloc("j_list", sizeof(int) * count);
  k_list      = (int *)mymalloc("k_list", sizeof(int) * count);
  l_list      = (int *)mymalloc("l_list", sizeof(int) * count);
 ...
 */
int get_tetra(tessellation *T, point *p,
              /** The number of moves necessary to find it. */
              int *moves, int ttstart,
              /** The return value from InTetra, specifying whether
                  the point is inside or on the edge/face. */
              int *flag,
              /** The edge/face number on the tetrahedron containing
                  the point, in case flag is >1. */
              int *edgeface_nr)
{
...
int InTetra(tessellation *T, int tt, point *p, int *edgeface_nr, int *nexttetra)
{
   ...
   * indicate the point is outside the volume. */
  /* XXX Actually it shouldn't do this anymore because we now do box tests instead */
...
#endif
#ifndef LONGIDS
      printf("task=%d: we are in a tetraeder with an infinity point. tetra=%d, coordinates of point=(%g|%g|%g) ID=%" MYIDTYPE_PRI "\n",
  ...
  if(status < 0)
    {
      ...
      if(ivol <= 0)
        terminate("flat or negatively oriented tetrahedron found (ivol=%d) tt=%d", ivol, tt);
    }
  ...
  if((ivol = Orient3d_Exact(p0, p1, p2, p3)) <= 0)
    terminate("flat or negatively oriented tetrahedron found (vol=%d)\n", ivol);
  ...
  if(count_zeros > 2)
    {
      ...
      printf("task=%d pp0=%ld pp1=%ld pp2=%ld pp3=%ld p=%ld IDs=(%" MYIDTYPE_PRI "%" MYIDTYPE_PRI "%" MYIDTYPE_PRI "%" MYIDTYPE_PRI
             "%" MYIDTYPE_PRI ") pos_0=(%g|%g|%g) pos_1=(%g|%g|%g) pos_2=(%g|%g|%g) pos_3=(%g|%g|%g) pos=(%g|%g|%g)\n",
             ThisTask, p0 - DP, p1 - DP, p2 - DP, p3 - DP, p - DP, p0->ID, p1->ID, p2->ID, p3->ID, p->ID, p0->x, p0->y, p0->z, p1->x,
             p1->y, p1->z, p2->x, p2->y, p2->z, p3->x, p3->y, p3->z, p->x, p->y, p->z);
...
void update_circumcircle(tessellation *T, int tt)
{
  ...
  if(status < 0)
    {
      /* resort to exact circum-sphere calculation */
...
double calculate_tetra_volume(point *p0, point *p1, point *p2, point *p3)
{
  ...
  if(isInfinity(p0) || isInfinity(p1) || isInfinity(p2) || isInfinity(p3))
    return +1;  // XXX why is volume +1 here, shouldn't it be something undefined?
...
void set_integers_for_pointer(point *p)
{
  ...
  if(p->xx < 1.0 || p->xx >= 2.0 || p->yy < 1.0 || p->yy >= 2.0 || p->zz < 1.0 || p->zz >= 2.0)
    terminate("invalid coordinate range: (%g, %g, %g) (%g, %g, %g)", p->x, p->y, p->z, p->xx, p->yy, p->zz);
 ...
 *  \param p2 Point 3 of tetrahedron.
 *  \param p3 Point 4 of tetrahedron.
 *  \param p Point to be checked if it is in circumsphere.
 *
 *  \return (-1, 0, 1); -1: in sphere, 0: on surface, 1: outside.
 */
...
#else
  get_integers_for_point(p, pB_ixyz, pB_xyz);
  double dx = pB_xyz[0] - (pA_xyz[0] + x[0]);
  double dy = pB_xyz[1] - (pA_xyz[1] + x[1]);
  double dz = pB_xyz[2] - (pA_xyz[2] + x[2]);
#endif
...
void write_voronoi_face_vertex_indices(tessellation *T, char *fname1, char *fname2, int writeTask, int lastTask)
{
  ...
  VertexEntries = (int *)mymalloc("VertexEntries", MaxEntries * sizeof(int));
  FaceNormals   = (float *)mynew("FaceNormals", float, MaxFaces);
  ...
  /* I/O */
  Nvertices_list = (int *)mymalloc("Nvertices_list", sizeof(int) * NTask);
  Nentries_list  = (int *)mymalloc("Nentries_list", sizeof(int) * NTask);
  Nsort_list     = (int *)mymalloc("Nsort_list", sizeof(int) * NTask);
  Nnormals_list  = (int *)mymalloc("Nnormals_list", sizeof(int) * NTask);
      ...
      for(task = writeTask + 1; task <= lastTask; task++)
        {
          ...
          tmp      = (int *)mymalloc("tmp", sizeof(int) * Nentries_list[task]);
...
void write_voronoi_face_vertex_coordinates(tessellation *T, char *fname, int writeTask, int lastTask)
{
  ...
  VertexCoordinates = (float *)mymalloc("VertexCoordinates", MaxEntries * 3 * sizeof(float));
  ...
  if(ThisTask == writeTask)
    {
      ...
      for(task = writeTask + 1; task <= lastTask; task++)
        {
          tmp = (float *)mymalloc("tmp", 3 * sizeof(float) * Nvertices_list[task]);
