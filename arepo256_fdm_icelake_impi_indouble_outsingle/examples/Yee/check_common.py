#!/usr/bin/env python3
""" @package ./examples/Yee/check_common.py
...
print('Yee_2d: checking simulation output in directory ' +
...
while True:
    ...
    RotationVelocity = xPosFromCenter * Velocity[:,
                                                 1] - yPosFromCenter * Velocity[:,
                                                                                0]
    ...
    Temperature_ref = Tinf - (
        (gamma - 1.0) * beta * beta /
        (8.0 * np.pi * np.pi * gamma) * np.exp(1.0 - Radius**2))
    exponent = 1.0 / (gamma - 1.0)
    Density_ref = Temperature_ref**exponent
    Vel_ref = np.zeros([NumberOfCells, 3], dtype=FloatType)
    Vel_ref[:, 0] = -0.5 * yPosFromCenter * beta / np.pi * np.exp(
        0.5 * (1.0 - Radius**2))
    Vel_ref[:, 1] = 0.5 * xPosFromCenter * beta / np.pi * np.exp(
        0.5 * (1.0 - Radius**2))
    RotationVelocity_ref = xPosFromCenter * Vel_ref[:,
                                                    1] - yPosFromCenter * Vel_ref[:,
                                                                                  0]
    # rotation velocity undefined where Radius == 0 (see above)
    RotationVelocity_ref[i_select] /= Radius[i_select]
    RotationVelocity_ref[~i_select] = np.nan
    ...
    L1_vel = np.average(abs_delta_vel[i_select], weights=Volume[i_select])
    ...
    print('\t tolerance: %g for %d cells per dimension' %
          (DeltaMaxAllowed, CellsPerDimension))
    ...
    if makeplots:
        ...
        pc = ax.pcolormesh(Edges1d,
                           Edges1d,
                           Density[cells].reshape((Nplot, Nplot)),
                           rasterized=True)
        ...
        plt.text(0.5,
                 0.92,
                 r'$t=%4.1f,\ L_1=%5.1e$' % (Time, L1_dens),
                 ha='center',
                 size=12,
